...
// each eased frame here via mogoiPebblePresent.
...
// State + frame tick are pushed every frame by mogoiPebblePresent (the shared
...
// mogoiPebblePresent -> g_idle_add (GTK is main-thread only).
typedef struct {
    int x, y, state;
...
// mogoiPebbleSpawn creates the window only — the shared Go loop drives motion
// + repaint via mogoiPebblePresent.
...
func (s *pebbleServiceLinux) createWindow() error { C.mogoiPebbleSpawn(); return nil }
...
func (s *pebbleServiceLinux) destroyWindow() { C.mogoiPebbleClose() }

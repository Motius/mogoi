...
func TestVerifyBrainTokenServerError(t *testing.T) {
	...
	// URL ("doesn't look like a Mogoi brain") — it should invite a retry.
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if strings.Contains(err.Error(), "doesn't look like a Mogoi brain") {
...
func TestVerifyBrainTokenNotABrain(t *testing.T) {
	...
	t.Cleanup(srv.Close)
	err := verifyBrainToken(context.Background(), enrollJWT(t, wsURLFor(srv)), "")
	if err == nil || !strings.Contains(err.Error(), "doesn't look like a Mogoi brain") {

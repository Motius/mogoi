...
const (
	...
	trayMenuMuteID     = 7
	trayMenuAccountID  = 8
	// Brand icon resources compiled into rsrc_windows_amd64.syso from mogoi.rc.
...
func createTrayIcon() bool {
	className, _ := syscall.UTF16PtrFromString("MogoiSidecarTray")
	...
	trayNID.HIcon = hIcon
	tip, _ := syscall.UTF16FromString("MOGOI Sidecar")
...
func showTrayMenu(hwnd uintptr) {
	...
	// Header — Mogoi + current state (disabled info line).
	header := "Mogoi"
	...
	appendTrayCheck(hMenu, "Pause Mogoi", trayMenuPauseID, ts.Paused)
	...
	appendTrayItem(hMenu, "Quit Mogoi", trayMenuCloseID)
...
// trayFooterText builds the "brain online · sidecar 2/2 · :1846" health line.

...
with **six patches**, all carried by `mogoi.patch`: a Win32 one for the open
...
a browser-controller accessor in a file of our own, `mogoi_native.go`, and a
Cocoa `terminate` that stops the run loop only when a window actually owns it.

`mogoi.patch` must carry EVERY vendored edit. `vendor-webview.sh` deletes the
vendor directory and copies pristine upstream over it, keeping only
`MOGOI_PATCH.md`, `mogoi.patch` and `.gitattributes` — so an edit made
...
`scripts/vendor-webview.sh`, re-applies `mogoi.patch`, and -- only if the patch
...
Regenerate `mogoi.patch`: diff a pristine copy of the new
`libs/webview/include/webview.h` against the `SW_HIDE` edit above (search for
`PATCHED (mogoi)`), update the hunk, and re-run the script.
...
## The browser-controller accessor (`mogoi_native.go`)
...
`mogoiTrayRun()`. Everything before that point owns the loop and terminates

...
@interface MogoiSubView : NSView {
@public
    SubWin* sw;
}
@end

@implementation MogoiSubView
...
static void sub_create_impl(SubWin* sw) {
    ...
    MogoiSubView* view = [[MogoiSubView alloc] initWithFrame:frame];
    view->sw = sw;
...
static SubWin* mogoiSubCreate(int seedX, int seedY) {
...
static void mogoiSubPresent(SubWin* sw, int x, int y, int r, int g, int b, int state, unsigned long long tick) {
...
// mogoiSubMonitorRight returns the main screen's right edge (single-display
// assumption — multi-monitor anchor-at-cursor is a follow-up).
static int mogoiSubMonitorRight(void) {
    NSScreen* main = [[NSScreen screens] firstObject];
    if (!main) return 0;
    return (int)(main.frame.origin.x + main.frame.size.width);
}

static void mogoiSubDestroy(SubWin* sw) {
    dispatch_async(dispatch_get_main_queue(), ^{
...
func (s *subPebbleServiceDarwin) Spawn(spec SubPebbleSpec) error {
	...
	entry.task.Store("")
	entry.result.Store("")
	entry.monitorRight.Store(int32(C.mogoiSubMonitorRight()))
...
func (s *subPebbleServiceDarwin) createOverlayWindow(entry *subPebbleEntry) error {
	...
	sw := C.mogoiSubCreate(C.int(cx), C.int(cy))
	if sw == nil {
		return fmt.Errorf("mogoiSubCreate returned nil")
	}

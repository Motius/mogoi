...
static Hotkey* mogoiHotkeyCreate(unsigned int mods, unsigned long keysym, unsigned long long id) {
...
// mogoiHotkeyRun blocks until stopped, firing the Go callback on each KeyPress.
static void mogoiHotkeyRun(Hotkey* hk) {
...
// mogoiHotkeyStop unblocks the run loop (write to the self-pipe — thread-safe,
// no XLib). Safe to call from a different goroutine than mogoiHotkeyRun.
static void mogoiHotkeyStop(Hotkey* hk) {
    if (hk && hk->stopfd[1] >= 0) { char c = 1; ssize_t n = write(hk->stopfd[1], &c, 1); (void)n; }
}

// mogoiHotkeyFree ungrabs + closes. Must run on the same goroutine as
// mogoiHotkeyRun (after it returns), since it touches XLib.
static void mogoiHotkeyFree(Hotkey* hk) {
    if (!hk) return;
    if (hk->dpy) {
...
func startHotkeyListener(keyspec string, onFire func()) (func(), error) {
	...
	go func() {
		...
		hk := C.mogoiHotkeyCreate(C.uint(mods), C.ulong(keysym), C.ulonglong(id))
		if hk == nil {

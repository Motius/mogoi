...
const (
	...
	pebbleBubbleBodyY0  = 84  // body text top, after the MOGOI eyebrow row
...
// drawBubbleText renders the riso "MOGOI" eyebrow + state-specific body
...
func (s *pebbleServiceWindows) drawBubbleText(memDC uintptr, state PebbleState, bubbleY1 int32) {
	...
	// Eyebrow row — uppercase mono "MOGOI" in speak-blue. Negative height in
	...
	nullTerm := int32(-1)
	eyebrowStr, _ := syscall.UTF16PtrFromString("MOGOI")
...
func repairBubbleTextAlpha(pixels []uint32, bubbleY1 int32) {
	...
	// just above the eyebrow row (62) so the MOGOI label is repaired too.

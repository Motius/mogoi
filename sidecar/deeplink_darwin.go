...
// macOS URL-open receiver for mogoi:// deep links. Unlike Linux/Windows,
...
static MogoiURLHandler* mogoiURLHandler = nil;
...
// macOS a mogoi:// click while the app is closed DOES cold-boot the full
...
// the Carbon header.
static void mogoiInstallURLHandler(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (mogoiURLHandler != nil) {
            return;
        }
        mogoiURLHandler = [[MogoiURLHandler alloc] init];
        [[NSAppleEventManager sharedAppleEventManager]
            setEventHandler:mogoiURLHandler

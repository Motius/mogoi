...
func TestPanelSpawnHandler_HappyPath(t *testing.T) {
	...
	h := makePanelSpawnHandler(svc, "ws://localhost:1846/sidecar/connect", stubPanelToken("tok-123"))

	res, err := h(map[string]any{
		"id":            "pebble",
		"url":           "http://localhost:1846/pebble.html",
		"title":         "MOGOI",
		"frameless":     true,
		"transparent":   true,
		"always_on_top": true,
...
func TestPanelSpawnHandler_InjectsTokenAndKeepsFragment(t *testing.T) {
	...
	if _, err := h(map[string]any{"id": "ans", "url": "http://localhost:1846/#/_answer_42"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

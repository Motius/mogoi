...
// candor: Approve/Deny inline for ordinary external actions, "Review in Mogoi"
...
//   - each button's `arguments` is a mogoi://n?id=..&kind=..&a=.. URI;
//   - clicking launches this exe with that URI (the mogoi:// scheme + the AUMID
...
const (
	notifyAUMID         = "Mogoi.Sidecar" // app id toasts attribute to
...
// ── setup: AUMID + mogoi:// scheme (idempotent, every startup) ──────────────
...
func windowsSetupNotifications() {
	...
	regSetString(`Software\Classes\AppUserModelId\`+notifyAUMID, "DisplayName", "Mogoi")
	if exe != "" {
		regSetString(`Software\Classes\AppUserModelId\`+notifyAUMID, "IconUri", exe)
	}

	// mogoi:// URI scheme → this exe, so a toast button's protocol activation
	// launches us with the action URI.
	regSetString(`Software\Classes\mogoi`, "", "URL:Mogoi Protocol")
	regSetString(`Software\Classes\mogoi`, "URL Protocol", "")
	if exe != "" {
		regSetString(`Software\Classes\mogoi\shell\open\command`, "", `"`+exe+`" "%1"`)
	}
}
...
func windowsForwardProtocolLaunch() bool {
	...
	if cls, err := syscall.UTF16PtrFromString("MogoiSidecarTray"); err == nil {
...
// It validates the tag, parses mogoi://n?id=..&kind=..&a=.., and dispatches.
...
func notifyURI(id, kind, action, nonce string) string {
	u := "mogoi://n?id=" + url.QueryEscape(id) + "&kind=" + url.QueryEscape(kind) + "&a=" + url.QueryEscape(action)
	if nonce != "" {

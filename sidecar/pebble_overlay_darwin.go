...
// goroutine and pushes each eased frame here via mogoiPebblePresent. This file
...
// mogoiPebblePresent (the shared Go runtime drives motion); drawRect: reads them.
...
@interface MogoiPebbleView : NSView
@end

@implementation MogoiPebbleView
...
// mogoiPebbleSpawnImpl creates the overlay window only — the shared Go loop
// drives motion + repaint via mogoiPebblePresent. Runs on the main thread.
static void mogoiPebbleSpawnImpl(void) {
    ...
    gPebbleView = [[MogoiPebbleView alloc] initWithFrame:frame];
    ...
    // frame, which dispatch_async's mogoiPebblePresentImpl onto the main queue.
}

// mogoiPebblePresentImpl applies one eased frame on the main thread: the shared
...
// (Y-flip), and redraw.
static void mogoiPebblePresentImpl(int x, int y, int state, unsigned long long tick,
...
static void mogoiPebbleCloseImpl(void) {
    if (gPebbleWindow) {
...
// mogoiPebbleSpawn creates the window only.
void mogoiPebbleSpawn(void) {
    dispatch_async(dispatch_get_main_queue(), ^{ mogoiPebbleSpawnImpl(); });
}

// mogoiPebblePresent pushes one eased frame from the Go runtime. text may be
// NULL/empty (drawRect: falls back to the per-state placeholder).
void mogoiPebblePresent(int x, int y, int state, unsigned long long tick,
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        mogoiPebblePresentImpl(x, y, state, tick, eye, blinded, answerOverflow, alphaPct, body);
        if (copy) free(copy);
    });
}

void mogoiPebbleClose(void) {
    dispatch_async(dispatch_get_main_queue(), ^{ mogoiPebbleCloseImpl(); });
}
*/
import "C"
...
func (s *pebbleServiceDarwin) createWindow() error { C.mogoiPebbleSpawn(); return nil }
...
func (s *pebbleServiceDarwin) destroyWindow() { C.mogoiPebbleClose() }

...
static const char kMogoiPanelDelegateKey;
...
static int mogoiInstallPanelExtNav(void* wkwebview) {
    ...
    if (wkwebview) {
        ...
        id d = v.UIDelegate;
        if (!d) return 2;
        objc_setAssociatedObject(v, &kMogoiPanelDelegateKey, d,
...
func installPanelExternalNav(wv webview.WebView) bool {
	...
	switch C.mogoiInstallPanelExtNav(ctrl) {
	case 0:
		return true
	case 1:

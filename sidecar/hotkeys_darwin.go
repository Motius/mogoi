...
// mogoiHotkeyRemove. modMask is an NSEventModifierFlags subset; keyCode is the
...
static void* mogoiHotkeyAdd(unsigned long modMask, unsigned short keyCode, unsigned long long hotkeyID) {
...
static void mogoiHotkeyRemove(void* p) {
    if (!p) return;

...
// was an endless reconnect loop in ~/.mogoi/sidecar.log. verifyBrainToken
...
// or the URL answers but isn't a Mogoi brain.
//
...
func verifyBrainToken(ctx context.Context, raw, brainOverride string) error {
	...
	if strings.TrimSpace(brainURL) == "" {
		return fmt.Errorf("This token doesn't name a brain to connect to. Run 'mogoi enroll \"<device-name>\"' again and paste the fresh token.")
	}
	...
	case resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden:
		return fmt.Errorf("The brain at %s rejected this token — it may be revoked or belong to a different brain. Run 'mogoi enroll \"<device-name>\"' again and paste the fresh token.", host)
	...
	default:
		return fmt.Errorf("The server at %s doesn't look like a Mogoi brain (unexpected response %s).", host, resp.Status)
	}
	...
	if err := json.NewDecoder(io.LimitReader(resp.Body, 1<<20)).Decode(&out); err != nil || out.AccessToken == "" {
		...
		return fmt.Errorf("The server at %s doesn't look like a Mogoi brain (unexpected response body).", host)
	}
	return nil
}

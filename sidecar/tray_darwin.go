...
//
// The menu mirrors the Windows tray (design: motius-tray.html §00): a
...
// NSApplication delegate (set in mogoiTraySetup) so that webview_go's panel
...
@interface MogoiTrayTarget : NSObject <NSApplicationDelegate>
...
@end
@implementation MogoiTrayTarget
...
// re-launches Mogoi.app (or double-clicks it after "You're connected"),
...
static MogoiTrayTarget* gTrayTarget = nil;

// mogoiDropPath builds the brand drop: a rounded box with three big-radius
...
// sharp corner.
static NSBezierPath* mogoiDropPath(NSRect r, CGFloat rBig, CGFloat rSharp) {
...
// mogoiMakeDropImage renders the status-item icon for the given state.
...
static NSImage* mogoiMakeDropImage(int stateCode, int muted) {
    CGFloat S = 18.0;
    ...
    NSBezierPath* drop = mogoiDropPath(dropRect, 6.0, 2.5);
    drop.lineWidth = 1.6;
    if (muted) {
...
// mogoiTraySetup creates the status item, target/delegate and an (empty) menu.
// Main thread only. The menu is populated by mogoiTrayRebuild.
static void mogoiTraySetup(void) {
    ...
    gStatusItem.button.image = mogoiMakeDropImage(0, 0);
    gStatusItem.button.toolTip = @"Mogoi";

    gTrayTarget = [[MogoiTrayTarget alloc] init];
    // Become the app delegate so panel webviews skip their own bootstrap run
    // loop (see the MogoiTrayTarget interface comment).
...
static void mogoiAddDisabled(NSMenu* menu, NSString* title) {
    NSMenuItem* it = [[NSMenuItem alloc] initWithTitle:title action:nil keyEquivalent:@""];
    [it setEnabled:NO];
    [menu addItem:it];
}

static void mogoiAddItem(NSMenu* menu, NSString* title, SEL sel) {
...
static void mogoiAddCheck(NSMenu* menu, NSString* title, SEL sel, int checked) {
    NSMenuItem* it = [[NSMenuItem alloc] initWithTitle:title action:sel keyEquivalent:@""];
    [it setTarget:gTrayTarget];
    [it setEnabled:YES];
...
// mogoiTrayRebuild replaces the menu contents + status icon from the live model.
...
static void mogoiTrayRebuild(const char* header, int waiting, int paused, int muted,
                              const char* recentJoined, const char* footer,
                              int online, int stateCode) {
    NSString* headerS = [NSString stringWithUTF8String:header ? header : "Mogoi"];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        gStatusItem.button.image = mogoiMakeDropImage(stateCode, muted);

        NSMenu* menu = [[NSMenu alloc] init];
        menu.autoenablesItems = NO;

        mogoiAddDisabled(menu, headerS);
        [menu addItem:[NSMenuItem separatorItem]];

        if (waiting > 0) {
            mogoiAddItem(menu, [NSString stringWithFormat:@"Waiting on you (%d)", waiting], @selector(onWaiting:));
            [menu addItem:[NSMenuItem separatorItem]];
        }

        mogoiAddCheck(menu, @"Pause Mogoi", @selector(onPause:), paused);
        mogoiAddCheck(menu, @"Mute microphone", @selector(onMute:), muted);
        [menu addItem:[NSMenuItem separatorItem]];

        if (recentS.length > 0) {
            mogoiAddDisabled(menu, @"Recent");
            NSArray<NSString*>* lines = [recentS componentsSeparatedByString:@"\n"];
            int n = 0;
            for (NSString* line in lines) {
                if (line.length == 0) continue;
                if (n >= 3) break;
                mogoiAddDisabled(menu, [@"   " stringByAppendingString:line]);
                n++;
            }
            [menu addItem:[NSMenuItem separatorItem]];
        }

        mogoiAddItem(menu, @"Open dashboard", @selector(onChat:));
        mogoiAddItem(menu, @"Account", @selector(onAccount:));
        mogoiAddItem(menu, @"Settings", @selector(onSettings:));
        mogoiAddItem(menu, @"Logs", @selector(onLogs:));
        [menu addItem:[NSMenuItem separatorItem]];

        mogoiAddItem(menu, @"Quit Mogoi", @selector(onClose:));

        if (footerS.length > 0) {
            [menu addItem:[NSMenuItem separatorItem]];
            mogoiAddDisabled(menu, footerS);
        }
...
// gTrayShouldQuit is set true only by mogoiTrayQuit. A webview window closing
...
// close no longer calls [NSApp stop] at all (see MOGOI_PATCH.md). This
...
// mogoiTrayRun runs the Cocoa main loop (blocks until mogoiTrayQuit).
static void mogoiTrayRun(void) {
    while (!gTrayShouldQuit) {
        [NSApp run];
    }
}

// mogoiTrayQuit removes the status item and stops the run loop. Safe to call
// from any goroutine — it marshals onto the main queue and posts a dummy event
// so -stop takes effect immediately.
static void mogoiTrayQuit(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (gStatusItem) {
...
var (
	...
	trayOnReopenDarwin     func()                                         // Mogoi.app re-launched / double-clicked (Dock-less reopen)
...
func runWithTray(ctx context.Context, cancel context.CancelFunc, client *SidecarClient) {
	...
	C.mogoiTrayRun() // blocks on [NSApp run]
}

...
// Temporary helper: dumps the local pages to MOGOI_PAGE_DUMP_DIR for visual
...
func withCustomChrome(t *testing.T, name, html string) string {
	...
	marked := strings.Replace(out, "<body>", "<body><script>window.__mogoiCustomChrome=true;</script>", 1)
	if marked == out {
...
func TestDumpBrandPages(t *testing.T) {
	dir := os.Getenv("MOGOI_PAGE_DUMP_DIR")
	if dir == "" {
		t.Skip("set MOGOI_PAGE_DUMP_DIR to dump the local pages")
	}
	if err := os.MkdirAll(dir, 0755); err != nil {
		t.Fatal(err)
	}
	demoURL := "https://app.motius.dev/connect?handshake=3KcshMk_demo_nonce_x8Qw2v0hV5uJ9pTz4rLb6nA1eYdC7fGm"
	pages := map[string]string{
		...
		"hosted-err.html":           hostedShellWithError("Could not reach motius. Check your connection and try again."),

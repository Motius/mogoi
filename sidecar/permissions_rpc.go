...
type permissionsReport struct {
	...
	// (a bare binary rather than Mogoi.app). Grants then bind to whatever
...
// (which is what makes Mogoi APPEAR in the pane's list - an app absent from
...
func handleSystemRequestPermission(params map[string]any) (*RPCResult, error) {
	...
	// as a permission granted to Mogoi would be wrong twice over, so the
	// report's `bundled: false` is a refusal here and a warning on screen,
	// not a row to click.
	if !setupProcessBundled() {
		return nil, fmt.Errorf("not running as an app bundle: a grant would attach to the launching app, not to Mogoi")
	}
	...
	if mode == grantPane {
		if err := setupOpenPane(name); err != nil {
			// Not fatal: the request above may still have registered Mogoi in

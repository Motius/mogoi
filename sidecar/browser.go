...
// executable, e.g. chrome.exe -> "mogoi-chrome-profile",
// msedge.exe -> "mogoi-msedge-profile".
...
func makeBrowserTypeHandler(cfg *SidecarConfig) RPCHandler {
	return func(params map[string]any) (*RPCResult, error) {
		...
		script := fmt.Sprintf(`(() => {
        const el = window.__mogoi_elements && window.__mogoi_elements[%d];

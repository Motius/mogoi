...
func TestConnectPageURLEscapesNonce(t *testing.T) {
	got := connectPageURL("https://app.motius.dev", "abc/+?=")
	if got != "https://app.motius.dev/connect?handshake=abc%2F%2B%3F%3D" {
...
func TestIsNoSuchHostErr(t *testing.T) {
	...
	dns := &net.DNSError{Err: "no such host", Name: "app.motius.dev", IsNotFound: true}
	wrapped := &url.Error{Op: "Post", URL: "https://app.motius.dev/api/handshake/register",
		Err: &net.OpError{Op: "dial", Net: "tcp", Err: dns}}
	if !isNoSuchHostErr(wrapped) {
		t.Fatal("wrapped DNS not-found error not detected")
	}
	if isNoSuchHostErr(&net.DNSError{Err: "server misbehaving", Name: "app.motius.dev"}) {

...
func TestParseEnrollDeepLink(t *testing.T) {
	t.Run("valid link round-trips nonce and token", func(t *testing.T) {
		nonce, token, err := parseEnrollDeepLink("mogoi://enroll?v=1&nonce=abc123&token=eyJ.x.y")
		if err != nil {
	...
	t.Run("token whitespace is trimmed", func(t *testing.T) {
		_, token, err := parseEnrollDeepLink("mogoi://enroll?nonce=n&token=%20%0aeyJ.x.y%0a")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if token != "eyJ.x.y" {
	...
	t.Run("rejects wrong scheme, wrong host, and missing params", func(t *testing.T) {
		for _, bad := range []string{
			...
			"mogoi://n?id=x&kind=approval&a=approve", // notification URI namespace
			"mogoi://enroll?nonce=n",
			"mogoi://enroll?token=t",
			"mogoi://enroll",
		} {
...
func TestEnrollDeepLinkSocketRoundTrip(t *testing.T) {
	...
	uri := "mogoi://enroll?v=1&nonce=abc&token=e.f.g"
...
func TestMaybeDropProtocolLaunch(t *testing.T) {
	...
	// Any unclaimed mogoi:// URI ends the process instead of booting a full
	// sidecar (the Linux scheme registration makes this web-page-reachable).
	os.Args = []string{"mogoi-sidecar", "mogoi://n?id=x&kind=approval&a=approve"}
	if !maybeDropProtocolLaunch() {
		t.Fatal("unrecognized mogoi:// URI must be dropped")
	}
	os.Args = []string{"mogoi-sidecar"}
	if maybeDropProtocolLaunch() {
		t.Fatal("normal launch must proceed")
	}
	os.Args = []string{"mogoi-sidecar", "--token", "abc"}
	if maybeDropProtocolLaunch() {

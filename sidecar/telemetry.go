...
// opt-out. It deliberately does NOT honor the brain's MOGOI_TELEMETRY /
// DO_NOT_TRACK env vars — the sidecar is opted out only via its own Settings >
// Privacy toggle (sidecar.yaml telemetry.enabled) or MOGOI_SIDECAR_TELEMETRY=0.
//
// Sends are fire-and-forget and total-failure-safe: nothing here may crash or
// block the sidecar. Set MOGOI_SIDECAR_TELEMETRY_DEBUG=1 to log each outcome.
...
const (
	sidecarAnonNamespace = "mogoi-sidecar-telemetry-v1"
...
// MOGOI_SIDECAR_TELEMETRY env (if set) > config flag > default on.
...
// sha256("mogoi-sidecar-telemetry-v1:"+hostname+":"+username), 128-bit hex.
...
func StartTelemetry(ctx context.Context, c *SidecarClient) {
	debug := !isFalsy(os.Getenv("MOGOI_SIDECAR_TELEMETRY_DEBUG"))

	enabled := func() bool {
		return resolveTelemetryEnabled(telemetryEnabledPtr(c), os.Getenv("MOGOI_SIDECAR_TELEMETRY"))
	}
	...
	if envVal := strings.TrimSpace(os.Getenv("MOGOI_SIDECAR_TELEMETRY")); envVal != "" && isFalsy(envVal) {
		log.Printf("[telemetry] sidecar telemetry disabled via MOGOI_SIDECAR_TELEMETRY")
		return
	}
	if enabled() {
		log.Printf("[telemetry] anonymous sidecar metrics on (disable in Settings > Privacy or set MOGOI_SIDECAR_TELEMETRY=0)")
	} else {

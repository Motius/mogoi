...
@class MogoiRegionView;

static NSWindow*         gRegionWin  = nil;
static MogoiRegionView* gRegionView = nil;
...
@interface MogoiRegionView : NSView
@end

@implementation MogoiRegionView
...
@interface MogoiRegionWindow : NSWindow
@end
@implementation MogoiRegionWindow
...
static void region_start_impl(void) {
    ...
    NSWindow* win = [[MogoiRegionWindow alloc] initWithContentRect:fr
    ...
    MogoiRegionView* view = [[MogoiRegionView alloc] initWithFrame:NSMakeRect(0, 0, fr.size.width, fr.size.height)];
...
static void mogoiRegionStart(void) {
    dispatch_async(dispatch_get_main_queue(), ^{ region_start_impl(); });
}

// mogoiRegionClose / mogoiRegionCrop are called from goRegionFinish/Cancel,
// which run on the main thread (invoked from the Cocoa event handlers).
static void mogoiRegionClose(void) {
...
static unsigned char* mogoiRegionCrop(int x, int y, int w, int h, int* outLen) {
    *outLen = 0;
...
static void mogoiRegionFree(unsigned char* p) { if (p) free(p); }
*/
import "C"
...
func (s *regionSelectionDarwin) finish(x0, y0, x1, y1 int) {
	...
	var outLen C.int
	ptr := C.mogoiRegionCrop(C.int(x), C.int(y), C.int(w), C.int(h), &outLen)
	if ptr == nil || outLen <= 0 {

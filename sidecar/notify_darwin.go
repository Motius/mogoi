...
// Approve/Deny, done → View/Dismiss, sidecar → Open Mogoi/Dismiss. The user's
// tap arrives on the delegate and is forwarded to the brain as notify.action.
// (No mogoi:// protocol hop like Windows — macOS delivers the response into the
...
@interface MogoiNotifyDelegate : NSObject <UNUserNotificationCenterDelegate>
@end
@implementation MogoiNotifyDelegate
...
static MogoiNotifyDelegate* gNotifyDelegate = nil;
...
static void mogoiNotifySetup(void) {
    if (!bundled()) {
    ...
    if (@available(macOS 10.14, *)) {
        ...
        gNotifyDelegate = [[MogoiNotifyDelegate alloc] init];
        ...
        UNNotificationAction* open      = [UNNotificationAction actionWithIdentifier:@"review"  title:@"Open Mogoi" options:UNNotificationActionOptionForeground];
        ...
        UNNotificationAction* openU     = [UNNotificationAction actionWithIdentifier:@"review" title:@"Open Mogoi" options:UNNotificationActionOptionForeground];
        ...
        UNNotificationAction* viewU     = [UNNotificationAction actionWithIdentifier:@"review"  title:@"Open Mogoi" options:UNNotificationActionOptionForeground];
...
static void mogoiNotifyShow(const char* cid, const char* ckind, const char* ctitle, const char* cbody) {
    if (!bundled()) { return; }
    if (@available(macOS 10.14, *)) {
...
func init() {
	...
	// responses in-process via the delegate, so there's no mogoi:// hop.
}
...
func darwinShowNotification(n Notification) {
	...
	C.mogoiNotifyShow(cid, ckind, ctitle, cbody)
}

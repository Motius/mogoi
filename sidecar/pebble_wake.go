...
// STT provider and word-searches the transcript for "mogoi". Catching
// the wake via STT (rather than openwakeword + ONNX runtime) means:
//   - No new native deps to bundle per platform
//   - Naturally handles "any phrase containing Mogoi" (the user's ask)
...
// daemon also gates wake_segment processing during TTS playback so MOGOI
...
// favor catching brief utterances ("Mogoi") over avoiding false segments,
...
// the wake match → listening transition feels instant; "Hey Mogoi" runs
...
type WakeListenerService struct {
	...
	// so MOGOI's own voice / chord-press audio can't trigger a wake. It is a
...
// single short "Mogoi". (It replaced a first-to-last speech-timestamp span,
...
// what actually ships. Well under a spoken "Mogoi" (350-500 ms voiced).
...
func (w *WakeListenerService) maybeEmitSegment(ctx context.Context) {
	...
	// span: a single clipped or quiet word ("Mogoi") whose energy only crosses
	...
	// daemon's STT + "mogoi" regex is the real noise filter, so we err toward

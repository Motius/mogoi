...
// Browser -> sidecar enrollment deep link (the motius free/self-host door).
//
// The connect page's "Self-hosted" row hands the pasted enrollment token to
// THIS machine via `mogoi://enroll?v=1&nonce=<nonce>&token=<jwt>` — the token
// never transits the motius server (hosting repo ONBOARDING.md pins the
...
// own live handshake — any web page can fire mogoi:// URIs, but only the
...
//
// The socket lives under ~/.mogoi (0600; the dir itself is created 0700 by
...
// `mogoi enroll` printing the token to a terminal; avoiding it entirely
...
const enrollDeepLinkPrefix = "mogoi://enroll"
...
func parseEnrollDeepLink(raw string) (nonce, token string, err error) {
	...
	if u.Scheme != "mogoi" || u.Host != "enroll" {
...
// mogoi://enroll URI: forward it to the running first-run window and exit —
...
// notification forwarder matches any mogoi:// URI and would swallow enroll
...
func maybeForwardEnrollLaunch() bool {
	...
	if err := forwardEnrollDeepLink(uri); err != nil {
		...
		fmt.Fprintf(os.Stderr, "mogoi: no setup window is waiting for a token (%v)\nOpen the Mogoi app, then click Connect on the web page again.\n", err)
	}
	return true
}

// maybeDropProtocolLaunch is the LAST protocol-launch check in main: any
// mogoi:// URI that neither the enroll forwarder nor the (Windows-only)
// notification forwarder claimed must still end this process. Registering the
// scheme on Linux made every mogoi:// URI a launch vector for ANY web page —
...
func maybeDropProtocolLaunch() bool {
	for _, a := range os.Args[1:] {
		if strings.HasPrefix(a, "mogoi://") {
			fmt.Fprintf(os.Stderr, "mogoi: ignoring unrecognized mogoi:// launch URI\n")
			return true
		}
	}
	return false

...
// Outbound OS notifications (design: motius-tray.html §01). The brain
// decides WHEN to interrupt — the five reasons Mogoi is allowed to: it needs
...
// Mogoi" and open the app — the click-only-in-app rule still holds. The brain
...
var (
	...
	// the AUMID + mogoi:// URI scheme). Called once at startup; no-op elsewhere.
	setupNotifications = func() {}
	// maybeForwardProtocolLaunch handles the case where this process was launched
	// by the OS to service a notification-button click (a mogoi:// URI in argv):

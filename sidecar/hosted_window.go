...
// First-run window, hosted-first (motius).
//
...
// red while contacting motius, white think-ring while sign-in runs in the
...
<div class="pagebody" tabindex="-1">
  <div class="bhead"><span class="word"><span class="u">use</span>mogoi</span></div>
  <div class="center">
    <div class="statusbox">
      <div class="dropwrap">
      ...
      <div id="status">Contacting motius&hellip;</div>
      <div id="err"></div>
...
<script>
  ...
  // no window). Tell the user WHERE Mogoi went so the disappearance doesn't
  ...
  window.__setConnected = function () {
    ...
    document.getElementById('status').textContent =
      'Mogoi now lives in your menu bar, at the top-right of your screen. Click its drop up there any time to open it.';
  ...
  window.__setSelfHostHint = function () {
    ...
    document.getElementById('status').textContent =
      'motius is not reachable from this machine. Use the "Paste your enrollment token" link below to connect this device.';
    document.getElementById('err').textContent = '';
    document.getElementById('retry').style.display = 'none';
    document.getElementById('reopen').style.display = 'none';
    document.getElementById('url').style.display = 'none';
  ...
  window.__browserOpened = function (ok, url) {
    ...
    document.getElementById('status').textContent = ok
      ? 'We opened motius in your browser — finish signing in there. This window will continue on its own.'
...
// connectedDwell is how long the Connect window holds the "Mogoi lives in
...
func runFirstRunWindow(cfg *SidecarConfig) (string, error) {
	...
	// the deep-link handler off-thread — a mogoi://enroll link only counts
	...
	// holds it briefly before closing, so the user learns where Mogoi went
	...
	startHandshake := func() {
		...
		go func() {
			...
			if err := registerHandshake(ctx, base, nonce, hostname); err != nil {
				...
				showShellError("Could not reach motius. Check your connection and try again.")
				return
			}
	...
	// Enrollment deep links (mogoi://enroll — the connect page's free door):
	...
	dl, dlErr := listenEnrollDeepLinks(func(uri string) {
		...
		go func() {
			...
			if _, derr := DecodeJWTPayload(tok); derr != nil {
				reportSelfHostResult(verifyCtx, base, nonce, fmt.Errorf("That doesn't look like a valid token. Copy the full token printed by 'mogoi enroll'."))
				return
			}
			verr := verifyBrainToken(verifyCtx, tok, cfg.Brain)

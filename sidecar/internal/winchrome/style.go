...
func initJS(dblClickMs uint32) string {
	...
	return `(function(){try{` +
		`window.__mogoiCustomChrome=true;` +
		`window.__mogoiDblClickMs=` + strconv.FormatUint(uint64(dblClickMs), 10) + `;` +
		...
		`if(!window.__mogoi_chrome_sync){if(tries++<20)setTimeout(chk,0);return;}` +
		`done=true;try{window.__mogoi_chrome_sync(!!document.getElementById('wchrome'));}catch(e){}};` +

...
// (it is what defines __mogoiCustomChrome and the bindings), so a page can
...
// picks up the user's real double-click speed from __mogoiDblClickMs.
const TitlebarJS = `
(function () {
  if (!window.__mogoiCustomChrome) return;
  try {
    ...
    var syncMax = function (isMax) {
      ...
      if (!window.__mogoi_chrome_is_maximized) return;
      window.__mogoi_chrome_is_maximized().then(function (m) {
        bar.classList.toggle('is-max', !!m);
      }).catch(function () {});
    };

    var toggleMax = function () {
      if (!window.__mogoi_chrome_toggle_maximize) return;
      window.__mogoi_chrome_toggle_maximize().then(syncMax).catch(function () {});
    };

    var dblMs = window.__mogoiDblClickMs || 500;
    ...
    window.addEventListener('pointermove', function (e) {
      ...
      armed = false;
      if (!window.__mogoi_chrome_drag) return;
      window.__mogoi_chrome_drag().then(function () {
    ...
    bar.addEventListener('contextmenu', function (e) {
      if (!window.__mogoi_chrome_sysmenu) return;
      e.preventDefault();
      window.__mogoi_chrome_sysmenu();
    });
    ...
    on('wchrome-min', function () { if (window.__mogoi_chrome_minimize) window.__mogoi_chrome_minimize(); });
    on('wchrome-max', toggleMax);
    on('wchrome-close', function () { if (window.__mogoi_chrome_close) window.__mogoi_chrome_close(); });

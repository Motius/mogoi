...
// The sidecar shares the ~/.mogoi data folder with the brain (they rarely run
// on the same host); its files are named distinctly so they can't collide with
// brain files (mogoi.pid, sidecar-keys/, the db, etc.). captures/ is shared.
var configDir = filepath.Join(homeDir(), ".mogoi")
...
func defaultCaptureDir() string {
	return filepath.Join(homeDir(), ".mogoi", "captures")
}
...
func LoadConfig() (*SidecarConfig, error) {
	...
	if len(cfg.Capabilities) == 0 {
		...
		// hand-edit ~/.mogoi-sidecar/config.yaml.

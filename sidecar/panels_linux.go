...
static void mogoi_panel_focus(void* gtkwin_ptr) {
    if (!gtkwin_ptr) return;
    GtkWindow* w = GTK_WINDOW(gtkwin_ptr);
    if (!GTK_IS_WINDOW(w)) return;
...
static void mogoi_panel_destroy(void* gtkwin_ptr) {
    if (!gtkwin_ptr) return;
    GtkWindow* w = GTK_WINDOW(gtkwin_ptr);
    if (!GTK_IS_WINDOW(w)) return;
...
static void mogoi_panel_set_window_state(void* gtkwin_ptr, int state) {
    if (!gtkwin_ptr) return;
    GtkWindow* w = GTK_WINDOW(gtkwin_ptr);
    if (!GTK_IS_WINDOW(w)) return;
    if (state == 1) {
...
static void mogoi_panel_set_visible(void* gtkwin_ptr, int visible) {
    if (!gtkwin_ptr) return;
...
static void mogoi_panel_set_click_through(void* gtkwin_ptr, int clickThrough) {
    if (!gtkwin_ptr) return;
    GtkWindow* w = GTK_WINDOW(gtkwin_ptr);
    if (!GTK_IS_WINDOW(w)) return;
    GdkWindow* gdkw = gtk_widget_get_window(GTK_WIDGET(w));
...
static void mogoi_panel_move_window(void* gtkwin_ptr, int x, int y) {
    if (!gtkwin_ptr) return;
    GtkWindow* w = GTK_WINDOW(gtkwin_ptr);
    if (!GTK_IS_WINDOW(w)) return;
...
static void mogoi_panel_set_regions(void* gtkwin_ptr, int* rects, int count) {
    if (!gtkwin_ptr) return;
    GtkWindow* w = GTK_WINDOW(gtkwin_ptr);
    if (!GTK_IS_WINDOW(w)) return;
    GdkWindow* gdkw = gtk_widget_get_window(GTK_WIDGET(w));
...
func platformSetInteractiveRegions(handle unsafe.Pointer, rects []PanelRect) error {
	...
	if len(rects) == 0 {
		...
		C.mogoi_panel_set_regions(handle, nil, 0)
		return nil
	}
	...
	C.mogoi_panel_set_regions(handle, &flat[0], C.int(len(rects)))
	return nil
}

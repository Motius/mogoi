...
// things (see third_party/webview_go/MOGOI_PATCH.md). Under the tray's single
...
// this names the reason so a re-vendor that drops mogoi_native.go fails here
...
func TestRunLoopBindingSurvivesReVendoring(t *testing.T) {
	src, err := os.ReadFile(filepath.Join("third_party", "webview_go", "mogoi_native.go"))
	if err != nil {
		t.Fatalf("read mogoi_native.go: %v", err)
	}
	if !strings.Contains(string(src), "func SetHostOwnsRunLoop(") {
		t.Fatal("mogoi_native.go no longer exports SetHostOwnsRunLoop. A re-vendor " +
			"drops anything mogoi.patch stops carrying; see MOGOI_PATCH.md.")
	}

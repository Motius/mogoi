...
static GtkWidget* mogoiPanelOnCreate(WebKitWebView* web_view,
...
static int mogoiInstallPanelExtNav(void* webkit_view) {
    if (!webkit_view) return 0;
    WebKitWebView* view = WEBKIT_WEB_VIEW(webkit_view);
    g_signal_connect(view, "create", G_CALLBACK(mogoiPanelOnCreate), NULL);
...
func installPanelExternalNav(wv webview.WebView) bool {
	...
	if C.mogoiInstallPanelExtNav(ctrl) == 0 {

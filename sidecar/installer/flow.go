...
// package ships the Mogoi.app bundle rather than a bare binary. Earlier
...
func performInstall(registryURL string, silent bool, progress progressFn) installOutcome {
	...
	workDir, err := os.MkdirTemp("", "mogoi-installer-*")
	if err != nil {
...
func runInstall(registryURL string, silent, noLaunch, autostartOn bool) int {
	...
	case out.NpmManaged:
		...
		logf("this machine's sidecar is managed by %s; update it with: %s update -g @motius/sidecar", pm, pm)
		return exitOK
	case out.UpToDate:
...
// @motius/sidecar and names its owner ("bun" or "npm"; "" when neither has
...
func hasSidecarPackage(nodeModules string) bool {
	fi, err := os.Stat(filepath.Join(nodeModules, "@motius", "sidecar", "package.json"))

...
// (AppUserModelId + mogoi:// protocol class, registered by the sidecar at
// every startup), the uninstall key, and finally the install directory via a
// detached self-delete. ~/.mogoi is prompted separately (default keep) and
...
const notifyAUMID = "Mogoi.Sidecar"
...
func runUninstall(silent bool) int {
	...
	if err != nil || inst.InstallDir == "" {
		...
		if !silent {
			notify("Uninstall Mogoi", "Mogoi does not appear to be installed for this user.", false)
		}
		return exitOK
	}
	...
	if !silent && !confirm("Uninstall Mogoi",
		"Remove Mogoi from this computer?\n\n"+inst.InstallDir) {
		return exitOK
	}
	...
	deleteKeyTree(registry.CURRENT_USER, `Software\Classes\mogoi\shell\open\command`)
	deleteKeyTree(registry.CURRENT_USER, `Software\Classes\mogoi\shell\open`)
	deleteKeyTree(registry.CURRENT_USER, `Software\Classes\mogoi\shell`)
	deleteKeyTree(registry.CURRENT_USER, `Software\Classes\mogoi`)
	...
	if err := validateRemovableInstallDir(inst.InstallDir); err != nil {
		...
		if !silent {
			notify("Uninstall Mogoi",
				"Mogoi was unregistered, but its files could not be removed:\n\n"+
					inst.InstallDir+"\n\n"+err.Error(), true)
		}
	...
	logf("Mogoi has been uninstalled.")
	if !silent {
		notify("Uninstall Mogoi", "Mogoi has been uninstalled.", false)
	}

...
import (
	...
	"github.com/mogoi/sidecar/internal/brand"
	"github.com/mogoi/sidecar/internal/webview2"
	"github.com/mogoi/sidecar/internal/webviewui"
	"github.com/mogoi/sidecar/internal/winchrome"
)
...
type wizardState struct {
	...
	// NoLaunch mirrors --no-launch. The buttons that would start Mogoi only
	// close the window then, so they must not say "Launch Mogoi".
...
// Mogoi. Mirrors the JS `homeSpot` phrasing in the done screen.
func launchHomeSpot() string {
	switch runtime.GOOS {
	case "darwin":
		return "the menu bar at the top-right of your screen (open Mogoi from your Applications folder)"
	case "windows":
		return "the system tray near the clock (open Mogoi from the Start menu)"
	default:
		return "the menu bar"
	}
}
...
func runWizard(registryURL string, noLaunch, autostartDefault bool) int {
	...
	opened := webviewui.RunWindow("Install Mogoi", 480, 560, webview.HintNone, winchrome.CustomTitleBar, func(w webview.WebView) {
		...
		// hand off to Mogoi.app --setup for the permission wizard) and closes.
		_ = w.Bind("launchAndClose", func() {
			...
			if !noLaunch && res.InstallDir != "" && res.Rel != nil {
				...
				if err := launchInstalled(res.InstallDir, res.Rel.Version, firstInstall); err != nil {
					...
					// just closes and Mogoi never appears — the same
					...
					notify("Mogoi could not start",
						"Mogoi is installed but could not be started automatically. Open it yourself — it runs in "+launchHomeSpot()+".",
						true)
					return
				}
			}
...
<div class="pagebody" tabindex="-1">
  <div class="hero">
    ...
    <h1><span class="word"><span class="u">use</span>mogoi</span> sidecar</h1>
  </div>
  ...
  <div class="row" id="autostartRow">
    <div><div class="lbl">Start at login</div><div class="note">Mogoi only helps while the sidecar is running.</div></div>
...
<script>
  ...
  function render(st) {
    ...
    // Where Mogoi lives after it starts. It has no Dock/taskbar presence and
    ...
    var npmLine = 'This machine\'s sidecar is managed by ' + pm + '. Update it with ' +
      pm + ' update -g @motius/sidecar, or remove it with ' +
      pm + ' remove -g @motius/sidecar to use this installer instead.';
    // Autostart applies on first install only; on updates the user's own
    // choice (Mogoi settings / setup wizard) stands.
    ...
    if (st.phase === 'done') {
      ...
      if (st.npm_managed) {
        ...
        el('subtitle').textContent =
          ...
          ' Start Mogoi from ' + startSpot + ' when you want it.';
        main.textContent = 'Close';
        main.onclick = function () { window.closeInstaller(true); };
      } else {
        el('subtitle').textContent = st.up_to_date ? 'Already up to date. Mogoi lives in ' + homeSpot + '.'
          : (st.platform === 'darwin' && st.first_install)
            ? 'Installed. Mogoi will ask for its permissions, then live in ' + homeSpot + '.'
            : st.first_install ? 'Installed. Mogoi lives in ' + homeSpot + '.'
              : 'Updated. Mogoi lives in ' + homeSpot + '.';
        main.textContent = 'Launch Mogoi';
    ...
    if (st.npm_managed) {
      ...
      el('subtitle').textContent = 'You already have the latest sidecar. Start Mogoi from ' + startSpot + ' when you want it.';
      main.textContent = 'Close';
      main.onclick = function () { window.closeInstaller(true); };
    } else if (st.up_to_date) {
      // Menu-bar-only app: a user who re-ran the installer to "get Mogoi
      ...
      el('subtitle').textContent = st.installed
        ? 'This updates the Mogoi sidecar on this machine.'
        : 'This installs the Mogoi sidecar on this machine.';

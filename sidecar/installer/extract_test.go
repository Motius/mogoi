...
func TestExtractHappyPath(t *testing.T) {
	tgz := writeTgz(t, []tgzEntry{
		...
		{name: "package/bin/mogoi.exe", body: []byte("MZ fake"), mode: 0755},
		{name: "package/bin/Mogoi.app/Contents/MacOS/mogoi", body: []byte("#!fake"), mode: 0755},
	})
	dest := t.TempDir()
	if err := extractPayload(tgz, dest); err != nil {
		t.Fatalf("extract: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dest, "bin", "mogoi.exe")); err != nil {
		t.Errorf("missing extracted exe: %v", err)
	}
	fi, err := os.Stat(filepath.Join(dest, "bin", "Mogoi.app", "Contents", "MacOS", "mogoi"))
	if err != nil {
...
func TestExtractRejectsSymlinks(t *testing.T) {
	tgz := writeTgz(t, []tgzEntry{
		{name: "package/bin/mogoi", body: []byte("x"), mode: 0755},

// The Mogoi sidecar installer/updater/uninstaller.
//
...
//
// Boundaries (see code-signing/ in the motius-docs repo):
//   - Enrollment is strictly the sidecar's concern: this program has no
//     --token flag and never reads or writes ~/.mogoi.
//   - It never requests OS permissions (macOS TCC grants bind to the
//     requesting bundle); it launches the installed Mogoi.app --setup.
package main
...
func main() {
	...
	noAutostart := flag.Bool("no-autostart", false, "Do not register the sidecar to start at login (Windows; macOS decides in Mogoi's own setup)")
	...
	// uninstall.exe / macOS "Uninstall Mogoi.app") gets no arguments when

...
func installDirDefault() (string, error) {
	return "", fmt.Errorf("the installer supports Windows and macOS; install with: bun install -g @motius/sidecar")
}
...
func detectInstalled() (installedSidecar, error) {
	...
	if inst.PackageManager == "" {
		return inst, fmt.Errorf("the installer supports Windows and macOS; install with: bun install -g @motius/sidecar")
	}
	return inst, nil
}

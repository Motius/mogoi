...
const (
	...
	uninstallKeyPath = `Software\Microsoft\Windows\CurrentVersion\Uninstall\Mogoi`

	trayWindowClass = "MogoiSidecarTray"
...
// installDirDefault is %LOCALAPPDATA%\Programs\Mogoi — the per-user install
...
func installDirDefault() (string, error) {
	...
	dir := filepath.Join(base, "Programs", "Mogoi")
...
// swapInstall replaces mogoi.exe in installDir (the only file the win32

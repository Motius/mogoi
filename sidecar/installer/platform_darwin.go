...
import (
	...
	"github.com/mogoi/sidecar/internal/autostart"
)

const appBundleName = "Mogoi.app"
...
func canWriteDir(dir string) bool {
	probe := filepath.Join(dir, ".mogoi-installer-probe")
...
// detectInstalled looks for Mogoi.app in /Applications then ~/Applications,
...
func detectInstalled() (installedSidecar, error) {
	...
	// No bundle install — a global bun/npm @motius/sidecar owns this
	// machine instead? (Positive identification only: a bare `mogoi` on PATH
...
func installedBinaryVersion(installDir string) (string, error) {
	bin := filepath.Join(installDir, appBundleName, "Contents", "MacOS", "mogoi")
...
// stopRunningSidecar SIGTERMs any mogoi process running from the installed
...
func stopRunningSidecar(inst installedSidecar, _ bool) error {
	...
	binPath := filepath.Join(inst.InstallDir, appBundleName, "Contents", "MacOS", "mogoi")
...
// checkPayloadLayout rejects a package that predates the Mogoi.app bundle.
...
// swapInstall atomically replaces <installDir>/Mogoi.app with the staged one,
...
// the --setup onboarding wizard (permissions must be requested by Mogoi.app
...
// runUninstall removes Mogoi.app, the LaunchAgent, and the retained
// uninstaller; ~/.mogoi is prompted (default keep). TCC entries are inert
...
func runUninstall(silent bool) int {
	...
	if err != nil || inst.InstallDir == "" {
		logf("no installed Mogoi.app found")
		if !silent {
			notify("Uninstall Mogoi", "Mogoi does not appear to be installed.", false)
		}
		return exitOK
	}
	// Double-clicked from Finder there is no console — confirm with a dialog.
	if !silent && !confirm("Uninstall Mogoi",
		"Remove Mogoi from this Mac?\n\n"+filepath.Join(inst.InstallDir, appBundleName)) {
		return exitOK
	}
	if err := stopRunningSidecar(inst, silent); err != nil {
		logf("could not stop the running sidecar: %v", err)
		if !silent {
			notify("Uninstall Mogoi", "Could not stop the running Mogoi sidecar:\n\n"+err.Error(), true)
		}
	...
	if err := os.RemoveAll(app); err != nil {
		...
		if !silent {
			notify("Uninstall Mogoi", "Could not remove:\n\n"+app+"\n\n"+err.Error(), true)
		}
		return exitFilesystem
	}

	maybeRemoveConfig(silent)
	logf("Mogoi has been uninstalled. (Permission entries can be cleared with: tccutil reset All com.mogoi.sidecar)")
	if !silent {
		notify("Uninstall Mogoi",
			"Mogoi has been uninstalled.\n\nmacOS keeps its (now inert) privacy entries; "+
				"clear them any time with:\ntccutil reset All com.mogoi.sidecar", false)
	}

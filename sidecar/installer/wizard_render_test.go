...
//
// OPT-IN: set MOGOI_BROWSER_TESTS=1, the same gate as the sidecar's
...
func TestWizardPanelSaysWhatIsOnTheMachine(t *testing.T) {
	if os.Getenv("MOGOI_BROWSER_TESTS") == "" {
		t.Skip("set MOGOI_BROWSER_TESTS=1 to render the wizard in a real browser")
	}
	chrome := findChromium()
	if chrome == "" {
	...
	cases := []struct {
	...
	}{
		{
			...
			subtitle:  "This installs the Mogoi sidecar on this machine.",
			button:    "Install",
			autostart: true,
			cancel:    "Cancel",
		},
		{
			...
			subtitle: "This updates the Mogoi sidecar on this machine.",
			button:   "Update",
			cancel:   "Cancel",
		},
		...
		// --no-launch: a "Launch Mogoi" button would only close the window.
		{
			...
			status:    "Up to date",
			subtitle:  "You already have the latest sidecar. Start Mogoi from the Start menu when you want it.",
			button:    "Close",
			closeOnly: true,
		},
		...
		// anyway.
		{
			...
			subtitle:  "This machine's sidecar is managed by bun. Update it with bun update -g @motius/sidecar, or remove it with bun remove -g @motius/sidecar to use this installer instead.",
			button:    "Close",
			closeOnly: true,
		},
		{
		...
		// reporting the plan's answer.
		{
			...
			status:    "Installed",
			subtitle:  "Installed. Mogoi lives in the system tray, near the clock.",
			button:    "Launch Mogoi",
			closeOnly: true,
		},
		...
		// exact-string surface, so pin the darwin homeSpot too.
		{
			...
			status:    "Installed",
			subtitle:  "Installed. Mogoi will ask for its permissions, then live in the menu bar, at the top-right of your screen.",
			button:    "Launch Mogoi",
			closeOnly: true,
		},
		{
			...
			status:    "Updated",
			subtitle:  "Updated. Mogoi lives in the system tray, near the clock.",
			button:    "Launch Mogoi",
			closeOnly: true,
		},
		{
			...
			status:    "Installed",
			subtitle:  "Installed. Start Mogoi from the Start menu when you want it.",
			button:    "Close",
			closeOnly: true,
		},
		{

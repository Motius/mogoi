...
// wizard to MOGOI_PAGE_DUMP_DIR for visual QA in a real browser. Not part of
...
func preview(t *testing.T, st wizardState, chromed bool) string {
	...
	if chromed {
		stub += `window.__mogoiCustomChrome=true;`
	}
	stub += `</script>`
...
func TestDumpWizardPage(t *testing.T) {
	...
	if dir == "" {
		t.Skip("set MOGOI_PAGE_DUMP_DIR to dump the wizard page")
	}

...
// First-launch onboarding wizard (`mogoi --setup`), launched by the installer
...
func runOnboarding(cfg *SidecarConfig) {
	...
	if !webviewui.RunWindow("Welcome to MOGOI", 520, 620, webview.HintNone, winchrome.CustomTitleBar, func(w webview.WebView) {
...
<div class="pagebody" tabindex="-1">
  <div class="hero">
    ...
    <h1>Welcome to <span class="word"><span class="u">use</span>mogoi</span></h1>
  </div>
  <p class="sub" id="intro">Mogoi needs a few permissions to see, hear, and notify on this machine. Grant each one below — rows turn green as you go. You can change any of this later in System Settings.</p>

  <div class="sec" id="permSec">Permissions</div>
  <div class="panel" id="permPanel">
    <div class="srow" id="row-notifications">
      <div class="sl7"><div class="a">Notifications</div><div class="b">Alerts from Mogoi when something needs you.</div></div>
    ...
    <div class="srow" id="row-screen">
      <div class="sl7"><div class="a">Screen Recording</div><div class="b">Ambient screen awareness. macOS opens a toggle in System Settings — flip it on for Mogoi.</div></div>
    ...
    <div class="srow" id="row-accessibility">
      <div class="sl7"><div class="a">Accessibility</div><div class="b">Global hotkeys (Ctrl+Space). Grant Mogoi in the Accessibility list.</div></div>
  ...
  <div class="panel">
    <div class="srow">
      <div class="sl7"><div class="a">Start at login</div><div class="b">Launch the sidecar automatically when you log in — Mogoi only helps while it's running.</div></div>
...
<script>
  ...
  function paint(st) {
    ...
    if (!anyVisible) {
      ...
      document.getElementById('intro').textContent =
        'One choice and you are done — Windows and Linux ask for permissions the first time Mogoi needs them.';
    }
  ...
  // OS (so Mogoi appears in the pane's list), then open that pane.

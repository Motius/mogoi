...
A Go program that runs on the user's machine, connects to the MOGOI brain over
...
```bash
make build          # -> ./mogoi   (injects the version from VERSION)
...
CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ \
  ...
  go build -ldflags "-H windowsgui" -o mogoi.exe .

# macOS — must be built on a Mac; both arches via clang -target.
CC="clang -target arm64-apple-macos11" GOOS=darwin GOARCH=arm64 go build -o mogoi .
```
...
```bash
./mogoi                  # start using the saved token
./mogoi --token <jwt>    # enroll: verified against the brain, then saved
./mogoi --setup          # first-launch onboarding (permissions, autostart)
./mogoi --version
./mogoi --help
./mogoi --test <cmd>     # built-in platform tests; needs -tags sidecartest
```

Config lives at `~/.mogoi/sidecar.yaml` (shared with the brain — the sidecar
owns only its own keys). A token can also arrive from the dashboard as a
`mogoi://` deep link, and unconfigured launches open a connect window.
...
| `config.go`, `types.go` | `~/.mogoi/sidecar.yaml`, shared message/config types |
...
| `npm/` | per-platform npm packages plus the `@motius/sidecar` wrapper |
...
```sh
MOGOI_PAGE_DUMP_DIR=/tmp/pages go test -run TestDumpBrandPages .
...
```sh
MOGOI_PAGE_DUMP_DIR=/tmp/wizard go test -run TestDumpWizardPage ./installer/
```
...
```sh
MOGOI_BROWSER_TESTS=1 go test -run TestWizardPanel ./installer/
```
...
```sh
MOGOI_BROWSER_TESTS=1 go test -run TestTitlebarGesture .
```
...
`MOGOI_BROWSER_TESTS` is never set in CI — these browser tests are a developer
tool, not a gate.
...
Published as platform npm packages (`@motius/sidecar-*`) behind the
`@motius/sidecar` wrapper, and installed for non-developers by the desktop
installer in `installer/`.

On macOS the sidecar ships as **`Mogoi.app`**, not a bare binary:

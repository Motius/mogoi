...
//
// OPT-IN: set MOGOI_BROWSER_TESTS=1 to run it, the same shape of gate as the
// page dump's MOGOI_PAGE_DUMP_DIR. It is not in the default suite because a
...
(async function () {
  ...
  var stub = function (name, fn) {
    window['__mogoi_chrome_' + name] = function () {
      calls.push(name);
...
func TestTitlebarGesture(t *testing.T) {
	if os.Getenv("MOGOI_BROWSER_TESTS") == "" {
		t.Skip("set MOGOI_BROWSER_TESTS=1 to drive the title bar in a real browser")
	}
	chrome := findChromium()
	if chrome == "" {

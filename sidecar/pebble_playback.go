...
const (
	...
	// tell the wake listener that MOGOI has stopped speaking. It has to
	// outlast both the device's own buffered audio and the gap
	// between two streamed sentences (synthesizing the next one takes
	// 300-800 ms), or the mic re-opens mid-answer and hears MOGOI.
...
// never fires and the wake listener stays suppressed: MOGOI goes deaf.
...
// gate captures during TTS so MOGOI's voice through the speakers doesn't

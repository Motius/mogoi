...
// mogoi_panel_on_main runs block on the main thread: inline when the caller is
...
// open. That last one is "launch Mogoi" while the dashboard sits minimized:
...
static void mogoi_panel_on_main(dispatch_block_t block) {
...
static void mogoi_panel_apply_flags(
    void* nswindow_ptr,
    int alwaysOnTop,
    int clickThrough,
    int transparent,
...
static void mogoi_panel_focus(void* nswindow_ptr) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
    mogoi_panel_on_main(^{
...
static void mogoi_panel_destroy(void* nswindow_ptr) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
    mogoi_panel_on_main(^{
        [w close];
...
static void mogoi_panel_set_window_state(void* nswindow_ptr, int state) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
    mogoi_panel_on_main(^{
        if (state == 1) {
...
static void mogoi_panel_set_visible(void* nswindow_ptr, int visible) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
    mogoi_panel_on_main(^{
...
static void mogoi_panel_set_click_through(void* nswindow_ptr, int clickThrough) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
    mogoi_panel_on_main(^{
...
static void mogoi_panel_cursor_pos(int* x, int* y) {
...
static void mogoi_panel_move_window(void* nswindow_ptr, int x, int y) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
    mogoi_panel_on_main(^{
...
static void mogoi_panel_watch_close(void* nswindow_ptr, unsigned long long token) {
    if (!nswindow_ptr) return;
    NSWindow* w = (__bridge NSWindow*)nswindow_ptr;
...
func registerPanelCloseWatch(handle unsafe.Pointer, impl *panelImpl) {
	...
	C.mogoi_panel_watch_close(handle, C.ulonglong(token))
}
...
func platformFocusWindow(handle unsafe.Pointer) error {
	...
	C.mogoi_panel_focus(handle)
	return nil
}

func platformGetCursorPos() (int, int, error) {
	var x, y C.int
	C.mogoi_panel_cursor_pos(&x, &y)
...
func platformMoveWindow(handle unsafe.Pointer, x, y int) error {
	...
	C.mogoi_panel_move_window(handle, C.int(x), C.int(y))
	return nil
}
...
func platformSetClickThrough(handle unsafe.Pointer, clickThrough bool) error {
	...
	C.mogoi_panel_set_click_through(handle, boolToCInt(clickThrough))
	return nil
}
...
func platformSetWindowState(handle unsafe.Pointer, state PanelWindowState) error {
	...
	C.mogoi_panel_set_window_state(handle, s)
	return nil
}
...
func platformSetWindowVisible(handle unsafe.Pointer, visible bool) error {
	...
	C.mogoi_panel_set_visible(handle, v)
	return nil
}

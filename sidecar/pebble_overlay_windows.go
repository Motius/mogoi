...
func (s *pebbleServiceWindows) createWindow() error {
	className, _ := syscall.UTF16PtrFromString("MogoiPebbleOverlay")
	windowName, _ := syscall.UTF16PtrFromString("MOGOI")
...
func (s *pebbleServiceWindows) present() error {
	...
	// W6-T4 — outward halo around disc when MOGOI is remotely controlling

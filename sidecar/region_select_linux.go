...
// mogoiRegionStart shows the overlay (called from an arbitrary goroutine).
static void mogoiRegionStart(void) {
    g_idle_add(region_start_idle, NULL);
}

// mogoiRegionClose tears the overlay down. Called from goRegionFinish/Cancel,
...
// mogoiRegionCrop crops the snapshot to (x,y,w,h) and PNG-encodes it. Returns a
// g_malloc'd buffer (caller frees via g_free) and sets *outLen. NULL on failure.
static guchar* mogoiRegionCrop(int x, int y, int w, int h, int* outLen) {
    *outLen = 0;
...
static void mogoiRegionFree(guchar* p) { if (p) g_free(p); }
*/
import "C"

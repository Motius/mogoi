...
//
//   make build-test                         # produces mogoi-sidecar-test
//   scp mogoi-sidecar-test mac:~/          # or windows:
//   ./mogoi-sidecar-test --test capture    # exercises full save + OCR loop
...
func printTestHelp() {
	fmt.Printf(`mogoi-sidecar platform test runner (built %s/%s)

Usage:
  mogoi-sidecar --test help                  This text
  mogoi-sidecar --test preflight             Print capability availability + reasons
  mogoi-sidecar --test ocr <image-path>      Run platformOCR against a PNG
  mogoi-sidecar --test capture               Live screen capture -> save -> OCR
  mogoi-sidecar --test all                   preflight + capture (no path needed)
...
func testCapture() int {
	...
	if captureDir == "" {
		captureDir = filepath.Join(os.TempDir(), "mogoi-sidecar-test-captures")
	}

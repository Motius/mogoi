...
Calculated message digest : AAAA
	Signer #0:
		Subject: /C=US/O=Mogoi Technologies Inc/CN=Mogoi Technologies Inc
...
KEYRING='projects/mogoi-signing/locations/global/keyRings/codesign'
ALIAS='mogoi-authenticode/cryptoKeyVersions/1'
...
out="$(KEYRING_OVERRIDE="${KEYRING}/cryptoKeys/mogoi-authenticode" run_sign "$WORK/app.exe" 2>&1)"
if [ $? -ne 0 ] && [[ "$out" == *GCP_KMS_KEYRING* ]]; then
...
make_osslsigncode() {
	...
	if [ "${6:-slash}" = "rfc2253" ]; then
		subject="C=US,ST=Delaware,L=Claymont,O=Mogoi Technologies Inc,CN=${2}"
	else
		subject="/C=US/O=Mogoi Technologies Inc/CN=${2}"
	fi
	cat >"$WORK/bin/osslsigncode" <<STUB
#!/usr/bin/env bash
printf '%s\0' "\$@" >> "\${VERIFY_LOG:-/dev/null}"
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "$TS_OK" "$CHAIN_OK" 0
out="$(SIGNING_PUBLISHER_CN="Mogoi Technologies Inc" run_sign "$WORK/app.exe" 2>&1)"
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "$TS_OK" "CMS_verify error" 1
out="$(run_sign "$WORK/app.exe" 2>&1)"
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "$TS_OK" "$CHAIN_OK" 0 rfc2253
out="$(SIGNING_PUBLISHER_CN="Mogoi Technologies Inc" run_sign "$WORK/app.exe" 2>&1)"
if [ $? -eq 0 ] && [[ "$out" == *"signed by 'Mogoi Technologies Inc'"* ]]; then
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "$TS_OK" "$CHAIN_OK" 0 rfc2253
sed -i 's/^\t\tSubject: C=US.*/\t\tSubject: C=US,CN=Attacker Ltd,OU=x,CN=Mogoi Technologies Inc/' "$WORK/bin/osslsigncode"
out="$(SIGNING_PUBLISHER_CN="Mogoi Technologies Inc" run_sign "$WORK/app.exe" 2>&1)"
...
make_osslsigncode "DEADBEEF     MISMATCH!!!" "Mogoi Technologies Inc" "$TS_OK" "$CHAIN_OK" 1
out="$(run_sign "$WORK/app.exe" 2>&1)"
...
make_osslsigncode "$GOOD_DIGEST" "Attacker Ltd/OU=x/CN=Mogoi Technologies Inc" "$TS_OK" "$CHAIN_OK" 0
out="$(SIGNING_PUBLISHER_CN="Mogoi Technologies Inc" run_sign "$WORK/app.exe" 2>&1)"
if [ $? -ne 0 ] && [[ "$out" == *"Attacker Ltd"* ]]; then
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "Timestamp is not available" "$CHAIN_OK" 0
out="$(run_sign "$WORK/app.exe" 2>&1)"
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "$TS_OK" "" 1
out="$(run_sign "$WORK/app.exe" 2>&1)"
...
make_osslsigncode "$GOOD_DIGEST" "Mogoi Technologies Inc" "$TS_OK" "$CHAIN_OK" 1

...
for v in GCP_KMS_KEYRING GCP_KMS_KEY_ALIAS GCP_ACCESS_TOKEN; do
	if [ -z "${!v:-}" ]; then
		...
		fail "$v is required (see code-signing/ci-pipeline.md in motius-docs)"
	fi
done
...
[ -f "$CODESIGN_CERT_FILE" ] ||
	fail "certificate chain not found at ${CODESIGN_CERT_FILE} — Cloud KMS holds only the private key, so the Sectigo chain PEM must be committed at sidecar/packaging/windows/codesign-chain.pem (or pointed at with CODESIGN_CERT_FILE). See code-signing/windows-setup.md in motius-docs."

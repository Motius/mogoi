...
| | |
|---|---|
| Leaf | `CN=MotiusAI Inc., O=MotiusAI Inc., ST=Delaware, C=US` |
| Issuer | Sectigo Public Code Signing CA R36 |
| Serial | `99D22DF966EC85DDB3B46CFD4DF01400` |
| Valid | 2026-08-11 → **2027-08-11** |
| Key | Cloud KMS `mogoi-codesign` version 1 (keyring `mogoi`, project `motius-prod`) — non-exportable, HSM |
...
`SIGNING_PUBLISHER_CN` must be exactly `MotiusAI Inc.` (trailing period
...
motius-docs; the key stays put, so renewal replaces only this file.
...
gcloud kms keys versions get-public-key 1 \
  --key mogoi-codesign --keyring mogoi --location global \
  --project motius-prod --output-file /tmp/kms.pub
...
code-signing/windows-setup.md in the motius-docs repo.

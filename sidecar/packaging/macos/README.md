...
| File | Purpose |
|---|---|
| `Info.plist` | Bundle manifest — `com.mogoi.sidecar`, `LSUIElement` (menu-bar agent, no Dock icon), mic + AppleEvents usage strings. `__VERSION__` is filled in by the Makefile. |
...
```bash
cd sidecar
make build                        # produces ./mogoi (native, cgo)
...
make app-macos                    # assembles dist/macos/Mogoi.app
```
...
```
Mogoi.app/Contents/
  MacOS/mogoi          the sidecar (must live here so it's bundle-associated)
...
```bash
make app-macos BIN=dist/darwin-arm64/mogoi APP_DIR=npm/darwin-arm64/bin/Mogoi.app
```
...
credential setup lives in code-signing/macos-setup.md in the motius-docs
repo. To reproduce it
manually, sign inside-out (nested Mach-Os first, never `--deep`) with the
hardened runtime:

```bash
APP=dist/macos/Mogoi.app
...
ditto -c -k --keepParent "$APP" /tmp/Mogoi.zip
xcrun notarytool submit /tmp/Mogoi.zip --key <key.p8> --key-id <KEY_ID> --issuer <ISSUER_ID> --wait
...
The npm `bin/mogoi` launcher (`npm/mogoi-sidecar/bin/mogoi`) prefers
`bin/Mogoi.app/Contents/MacOS/mogoi` on darwin when present, falling back to a
bare `bin/mogoi`. `make npm-pack` builds the `.app` into the darwin packages.

...
MOGOI collects **anonymous** usage metrics so the project can answer two
...
`anon_id` is `sha256("mogoi-telemetry-v1:" + hostname + ":" + username)`,
...
The hourly heartbeat exists because MOGOI is a long-running server daemon:
...
- **Config flag** - set in `~/.mogoi/config.yaml`:
  ```yaml
  telemetry:
    enabled: false
  ```
- **Env var** - `MOGOI_TELEMETRY=0` (also accepts `false`/`no`/`off`).
- **`DO_NOT_TRACK=1`** - the cross-tool community standard
  (<https://consoledonottrack.com>) is honored.

Precedence: `DO_NOT_TRACK` > `MOGOI_TELEMETRY` > config flag > default (on).
...
observable, set `MOGOI_TELEMETRY_DEBUG=1`:

```bash
MOGOI_TELEMETRY_DEBUG=1 mogoi start
```
...
The desktop sidecar (the Go agent that gives MOGOI eyes and hands on your
...
(`mogoi-sidecar-telemetry-v1` namespace), its own opt-out, and it does **not**
honor the brain's `MOGOI_TELEMETRY` / `DO_NOT_TRACK` env vars.
...
- Set `telemetry.enabled: false` in `~/.mogoi/sidecar.yaml`.
- Run with `MOGOI_SIDECAR_TELEMETRY=0`.

Set `MOGOI_SIDECAR_TELEMETRY_DEBUG=1` to log each ping's outcome.

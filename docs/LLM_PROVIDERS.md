# M.O.G.O.I. LLM Providers Documentation

Complete guide to the LLM provider abstraction layer for Project M.O.G.O.I.
...
The M.O.G.O.I. LLM provider system provides a unified abstraction layer for multiple Large Language Model providers, enabling seamless switching between Anthropic Claude, OpenAI GPT, and local Ollama models with automatic fallback support.
...
See [QUICKSTART.md](~/mogoi/QUICKSTART.md) for detailed setup instructions.
...
**Implementation**: `~/mogoi/src/llm/anthropic.ts`
...
**Implementation**: `~/mogoi/src/llm/openai.ts`
...
**Implementation**: `~/mogoi/src/llm/ollama.ts`
...
See [~/mogoi/src/config/README.md](~/mogoi/src/config/README.md) for complete config documentation.
...
- Add at least one API key to `~/.mogoi/config.yaml`
...
- **OpenAI**: caching is automatic server-side (~50% discount on cached prefixes over 1024 tokens); Mogoi just keeps stable content first in the prompt. Reported `cached_tokens` are surfaced in telemetry.
- **OpenRouter**: most routed upstreams (OpenAI, Gemini 2.5, DeepSeek, Groq, Grok, Moonshot) cache automatically. For `anthropic/*` models Mogoi sends OpenRouter's top-level `cache_control: {type: "ephemeral"}` and OpenRouter places the breakpoints itself; it also sticky-routes follow-up requests to the same upstream endpoint to maximize hit rates. (`qwen/*` also needs explicit caching but only supports per-content-block breakpoints - not emitted yet.) Cached reads and writes come back in `prompt_tokens_details` and are surfaced in telemetry (both chat and streaming).
...
- **MotiusAI AI (hosted)**: explicit Anthropic-style breakpoints in OpenAI wire format — `cache_control` riding on content parts, since LiteLLM drops the top-level field OpenRouter accepts. **OFF by default**: emission is gated on the SYSTEM `motius_ai.prompt_cache: true` opt-in (config.yaml, provisioner-written) *and* the user-level `llm.prompt_cache` switch. See "MotiusAI AI prompt caching" below for what the opt-in requires — as of 2026-08-19 it is HARD-BLOCKED on platform-side per-tenant upstream credentials (shared cache namespace). Streamed requests set `stream_options: {include_usage: true}` so cache telemetry is real on the conversational path.
...
#### MotiusAI AI prompt caching — verification record and enablement conditions
...
- [LLM Provider README](~/mogoi/src/llm/README.md)
- [Config System README](~/mogoi/src/config/README.md)
- [Quick Start Guide](~/mogoi/QUICKSTART.md)
- [Implementation Summary](~/mogoi/IMPLEMENTATION_SUMMARY.md)

### Examples

- [Integration Example](~/mogoi/examples/llm-integration.ts)
- [Test Suite](~/mogoi/src/llm/test.ts)
- [Setup Script](~/mogoi/scripts/setup-config.ts)
...
Built for Project M.O.G.O.I. with ❤️

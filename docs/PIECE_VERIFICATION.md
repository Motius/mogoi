...
How to verify a piece is wired correctly end-to-end -- from source on disk all the way to a workflow run inside the engine subprocess. Applies to Mogoi-authored pieces (under `src/workflows/activepieces/packages/pieces/mogoi/`) and to community pieces installed at runtime via the pieces library.
...
1. Write piece source under `packages/pieces/mogoi/<name>/src/`.
...
```ts
// packages/pieces/mogoi/my-piece/src/lib/actions/greet.ts
...
- [ ] `package.json` is present at the piece root with `name: "@mogoipieces/piece-mogoi-<short>"`, a `version`, and `"main": "./dist/src/index.js"`.
...
The daemon's bootstrap runs `buildAllMogoiPieces()` on every start with content-hash caching. New piece directories are auto-discovered. You do not need to run `bun run build:workflows` manually unless you want to verify the build offline before restarting.
...
# expected output:
#   built @mogoipieces/piece-mogoi-<short>@0.0.1
#     bundle: /home/<you>/.mogoi/cache/pieces/<hash>/<short>/dist/src/index.js
```

After build, the piece's `dist/` lives under `~/.mogoi/cache/pieces/<piece-hash>/<short>/`. The hash mixes the engine bundle hash, so framework changes invalidate every piece automatically.

Verify:

- [ ] `~/.mogoi/cache/pieces/<hash>/<short>/dist/src/index.js` exists.
...
If you see `catalog built with K piece(s), <M> extraction failure(s)`, one or more pieces failed to extract metadata. Failures persist in `~/.mogoi/cache/piece-metadata.json` as part of the cache write (since the partial-cache fix), and the daemon prints the failing package name and error.

Verify:

- [ ] The piece's package name appears in `~/.mogoi/cache/piece-metadata.json` under `entries[].name`.
...
If the daemon was already running with stale cache before your changes, the cache key may not have changed enough to invalidate. Bump `CATALOG_SCHEMA_VERSION` in `src/workflows/runtime/piece-catalog.ts`, or just delete `~/.mogoi/cache/piece-metadata.json` and restart.
...
```bash
curl -s http://localhost:1846/api/workflows/pieces \
  | jq '.[] | select(.name == "@mogoipieces/piece-mogoi-<short>")'
```
...
MOGOI_GATED_REAL_PIECE_TESTS=1 bun test src/workflows/runner/engine-runtime/extract-piece-metadata.test.ts
```

The two most common extraction failures:

- `PieceNotFoundError: Piece not found for package: @mogoipieces/piece-mogoi-<short>-<version>`. The engine couldn't find your piece in its dev-pieces list. This is now auto-discovered via `discoverMogoiDevPieces()` in `engine-runtime.ts` -- if you see this, confirm `packages/pieces/mogoi/<short>/package.json` exists and has a valid `name`.
...
3. **Engine-extract smoke (gated).** For pieces with auth, the engine-extract test exercises `EXTRACT_PIECE_METADATA` against a real installed piece. Gate it with `MOGOI_GATED_REAL_PIECE_TESTS=1` so CI doesn't require Gmail credentials.

## What fails at each stage

| Symptom | Stage | Where to look |
|---|---|---|
| Piece doesn't appear in `bun run build:workflows` output | 2 | `package.json` missing or malformed under `packages/pieces/mogoi/<short>/` |
| `PieceNotFoundError` in daemon log | 3 | `discoverMogoiDevPieces()` in `engine-runtime.ts` -- check the piece dir has a `package.json` with a `name` field |
...
Just drop a new directory under `packages/pieces/mogoi/<name>/`. Restart the daemon. Everything else is automatic:

- `buildAllMogoiPieces()` walks the directory and builds it (content-hash cached).
- `discoverMogoiDevPieces()` walks the same directory and adds it to the engine's dev-pieces list.
...
Community pieces ship pre-built from npm and live under `~/.mogoi/pieces/node_modules/@activepieces/`. Verification flow:
...
If the catalog refresh doesn't see the new piece, the bundle cache may be holding a stale entry. The install path invalidates the bundle cache automatically; if it didn't, delete `~/.mogoi/cache/piece-metadata.json` and restart.

...
- Not a marketplace. The set of installable pieces is the curated catalog under `src/workflows/pieces-library/` plus the Mogoi-authored pieces in the vendored tree. Users cannot side-load arbitrary npm packages.

## Architecture

```
+------------------------------------------------------------------------------+
|                       Mogoi daemon (Bun, single process)                    |
|                                                                              |
...
|      service-backends.ts: glues daemon services into /v1/mogoi/* fns        |
|      event-bus.ts + event-buffer.ts: in-process pub/sub for mogoi-trigger   |
...
|        - /v1/mogoi/{llm,tools,notify,context,agent,events,workflows}        |
|                                                                              |
|    runner/                                                                   |
...
|        flow-version-adapter.ts: Mogoi flow shape -> upstream shape          |
...
|                   pieces/common, pieces/mogoi/*)                            |
|    pieces-library/  curated community catalog + installer + reconciler       |
|    credentials/  MogoiConnectionSource adapters (mogoi:google, etc.)       |
|                                                                              |
+------------------------------------------------------------------------------+
                              |
...
|  -> imports vendored pieces dynamically (dev-pieces mode for Mogoi pieces,  |
...
src/workflows/
  activepieces/                 Vendored Activepieces source. See UPSTREAM.md.
    ...
    packages/
      ...
      pieces/
        ...
        mogoi/                 Our pieces (ask, tool, notify, context, agent,
  ...
  sandbox-api/                  Loopback HTTP+WS API the engine subprocess hits
    ...
    routes/                     One file per /v1 surface:
      ...
      mogoi-agent.ts           /v1/mogoi/agent  -> agent-delegator
      mogoi-context.ts         /v1/mogoi/context -> vault/awareness/commitments
      mogoi-events.ts          /v1/mogoi/events -> event buffer poll
      mogoi-llm.ts             /v1/mogoi/llm    -> LLMManager.chat
      mogoi-notify.ts          /v1/mogoi/notify -> ChannelService + desktop
      mogoi-tools.ts           /v1/mogoi/tools  -> ToolRegistry.invoke
      mogoi-workflows.ts       /v1/mogoi/workflows -> workflow runner
  ...
  runner/                       Things that run a flow
    ...
    engine-runtime/
      build.ts                  Builds the engine into a CJS bundle. The bundle
                                ...
                                `~/.mogoi/cache/engine/<hash>/`
      build-pieces.ts           Walks packages/pieces/mogoi/* and esbuilds each
                                into `~/.mogoi/cache/pieces/<hash>/<short>/`.
  ...
  pieces-library/               Curated community-pieces catalog + installer
    ...
    installer.ts                Writes ~/.mogoi/pieces/installed.json, runs
                                bun install, extracts metadata
    reconciler.ts               Idempotent reconcile (install/uninstall delta)

  credentials/                  MogoiConnectionSource adapters that bridge
                                Mogoi's existing OAuth/state into pieces
    adapter.ts                  Registry of sources by `mogoi:*` external id
    google-source.ts            mogoi:google -> existing Google OAuth tokens
    telegram-source.ts          mogoi:telegram -> daemon's bot token

  mogoi-pieces/                Daemon-side service shims invoked by /v1/mogoi
    agent-delegator.ts          Backs mogoi-agent.delegate (M7 sub-agent loop)
    context-provider.ts         Backs mogoi-context (vault/awareness reads)
    llm-client.ts               Backs mogoi-ask via LLMManager.chat
    notifier.ts                 Backs mogoi-notify -- per-channel routing
    tool-registry.ts            Backs mogoi-tool -- invoke a Mogoi tool
    workflow-runner.ts          Backs mogoi-trigger.run_workflow
...
scripts/
  build-engine.ts               Builds the engine bundle into a content-hashed
                                dir in ~/.mogoi/cache/engine/<hash>/
  build-pieces.ts               Builds every Mogoi-authored piece (content-hash
                                cached)
...
1. `buildEngineBundle()` checks `~/.mogoi/cache/engine/<hash>/main.js`. If the hash matches current sources, returns immediately. Otherwise rebuilds (~700ms cold) and caches.
2. `buildAllMogoiPieces()` walks `packages/pieces/mogoi/*` and esbuilds each piece into its dist dir. Unchanged pieces skip on hash hit (~2ms each).
...
5. `PieceCatalog.build()` runs `EXTRACT_PIECE_METADATA` for every known piece (Mogoi + installed community). Failures don't block successful entries: partial cache writes persist what extracted. The cache key includes `CATALOG_SCHEMA_VERSION` so daemon-side projection changes invalidate it.
...
If bootstrap fails (e.g. esbuild error in a piece), the daemon logs a warning and falls back to "no workflows" mode -- the rest of Mogoi comes up clean. The Workflows room shows an empty-catalog notice.
...
5. The engine subprocess imports the populated flow's pieces (Mogoi pieces via dev-pieces resolution, community pieces via `node_modules`), runs the trigger payload through each step, and streams `WorkerNotify.updateStepProgress` events back over the WS for every step boundary.
...
- **Mogoi-authored** pieces live in `src/workflows/activepieces/packages/pieces/mogoi/`. They use the same `createPiece` / `createAction` API as upstream Activepieces. They're auto-discovered: drop a directory with a valid `package.json` and the next daemon restart finds it, builds it, and surfaces it in the library.
- **Community** pieces ship as npm packages. They install at runtime via the pieces library UI -- the installer writes `~/.mogoi/pieces/installed.json`, runs `bun install`, and asks the engine to extract metadata. They are picked from a curated catalog (Verified or Community tier).
...
All workflow tables live in `~/.mogoi/mogoi.db` (the same SQLite file as the rest of Mogoi). Schema: `src/workflows/db/schema.ts`. Repos: `src/workflows/db/repos/`.

Connection secrets are encrypted at rest with AES-256-GCM. Wrapping format: `enc1:<iv>:<tag>:<ciphertext>`. The key comes from `MOGOI_WORKFLOW_ENCRYPTION_KEY` (env) or `~/.mogoi/cache/workflow-encryption.key` (auto-generated, `chmod 0600`). Legacy plaintext rows are accepted transparently for backwards compat.
...
- **Engine bundle hash** mixes the synthesized package.json (esbuild deps), the `UPSTREAM_PIN_SHA`, and every file in `PATCHED_VENDOR_SOURCES` (see `src/workflows/runner/engine-runtime/build.ts`). Editing a patched vendor file flips the hash; a fresh bundle goes to `~/.mogoi/cache/engine/<new-hash>/`.
...
- **Catalog cache key** mixes the bundle hashes of every piece + `CATALOG_SCHEMA_VERSION`. Bump the constant in `piece-catalog.ts` if you change the projection format. Cache lives at `~/.mogoi/cache/piece-metadata.json`.
...
2. **Engine-extract tests against real pieces** -- gated. Set `MOGOI_GATED_REAL_PIECE_TESTS=1` to opt in. They actually install a piece (e.g. Gmail) and run `EXTRACT_PIECE_METADATA` against it; useful in CI but pricey locally.
3. **End-to-end engine tests** -- gated by `MOGOI_TEST_ENGINE_BUILD=1`. Build the engine bundle, spawn it, run real flows from BEGIN to terminal status. Includes the RESUME-from-paused suite (`end-to-end-resume.test.ts`) and the Phase L plumbing smoke (`end-to-end-l.test.ts`).
...
|---|---|---|
| Add a new Mogoi piece | Drop a directory under `packages/pieces/mogoi/<name>/` | [`PIECE_VERIFICATION.md`](./PIECE_VERIFICATION.md) |
...
| Add a new `/v1/mogoi/*` service | Add a route file under `sandbox-api/routes/`, wire it into `server.ts`, wire the backend into `service-backends.ts` | (this file -- "Source tree map") |
...
| Run the engine-extract test against a real piece | `MOGOI_GATED_REAL_PIECE_TESTS=1 bun test src/workflows/runner/engine-runtime/extract-piece-metadata.test.ts` | (this file -- "Testing") |
| Add a new connection source for `mogoi:*` external ids | Implement a `MogoiConnectionSource`, register in `src/workflows/credentials/adapter.ts` | (this file -- "Source tree map") |
| Debug a stuck or weird run | Inspect `flow_run.status` + `waitpoint` rows, then `~/.mogoi/cache/run-logs/<runId>.zst` for the engine's last execution state | (this file -- "Persistence and encryption") |

## Glossary

- **Piece** -- an npm package that ships actions and/or triggers. Examples: `@mogoipieces/piece-mogoi-ask`, `@activepieces/piece-gmail`.

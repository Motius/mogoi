...
#   4. Links the `mogoi` command globally
#
# ─────────────────────────────────────────────────────────────────────

REPO_URL="https://github.com/Motius/mogoi.git"
INSTALL_DIR="$HOME/.mogoi/daemon"
...
print_banner() {
  ...
  echo "╚═╝     ╚═╝ ╚═════╝  ╚═════╝  ╚═════╝ ╚═╝"
  echo -e "${RESET}"
  echo -e "${DIM}  Motius-ke labs ,your Co-Founder AI with no emotions${RESET}"
  echo ""
}
...
ensure_libc_headers() {
  ...
  if command -v apt-get &> /dev/null; then
    ...
    warn "If 'mogoi' fails with \"sys/file.h not found\", install your distro's libc dev headers."
    return 0
  fi
...
main() {
  ...
  echo -e "${CYAN}[2/3]${RESET} ${BOLD}Downloading M.O.G.O.I...${RESET}"
  ...
  # Stamp install-method marker so `mogoi update` / `mogoi uninstall` know
  ...
  rm -f "$HOME/.bun/bin/mogoi" 2>/dev/null || true
  local bun_bin="$HOME/.bun/bin"
  mkdir -p "$bun_bin"
  printf '#!/usr/bin/env bash\nexec bun "%s/bin/mogoi.ts" "$@"\n' "$INSTALL_DIR" > "$bun_bin/mogoi"
  chmod +x "$bun_bin/mogoi"

  ensure_bun_path
  add_path_to_shell

  if command -v mogoi &> /dev/null; then
    ok "mogoi command is available"
  else
    warn "mogoi installed but not in PATH yet. Restart your terminal or run:"
  ...
  echo ""
  echo -e "${GREEN}${BOLD}✓ M.O.G.O.I. installed successfully!${RESET}"
  echo ""
  ...
  echo -e "    ${CYAN}mogoi start${RESET}"
  echo -e "    ${DIM}→ http://localhost:1846 (dashboard guides you through LLM, voice, and profile)${RESET}"
}

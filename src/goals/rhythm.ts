...
export class DailyRhythm {
  ...
  private buildMorningPrompt(): string {
    ...
    return `You are MOGOI, an AI assistant running a morning planning session.${tone}
...
Respond with ONLY valid JSON:
{
  ...
  private buildEveningPrompt(): string {
    const tone = this.getToneInstructions();
    return `You are MOGOI, an AI assistant running an evening review session.${tone}

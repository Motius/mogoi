import type { MogoiConfig, RealtimeReasoningEffort, RealtimeVoiceConfig } from './types.ts';
import type { RealtimeEnablement } from '../daemon/motius-ai.ts';
 ...
 *
 * The twin of `readMotiusAIAiBlock` (daemon/motius-ai.ts) and required to
 ...
 */
export function realtimeServedByPlan(config: MogoiConfig): boolean {
  ...
  // readMotiusAIAiBlock guards against and warns about. Reaching straight for
  ...
  // and as absent to hasMotiusAIAi, and the two must never disagree about
 ...
 */
function findOpenAIProviderKey(config: MogoiConfig): string {
 ...
 *
 *  - HOSTED (a complete `motius_ai` block): the plan serves it through the
...
export function resolveRealtimeVoice(
   ...
   * (daemon/motius-ai.ts realtimeEnablement).
   *
  ...
  if (servedByPlan) {
    ...
    // (`llm.motius.host` → `llm.motius.host/v1/realtime`, no ws://),
    ...
    try {
      ...
      if (url.protocol !== 'http:' && url.protocol !== 'https:') {
        return { ok: false, reason: `motius_ai.base_url has unsupported scheme: ${url.protocol}` };
      }
      const path = url.pathname.replace(/\/+$/, '');
      const origin = `${url.protocol}//${url.host}${path}`;
      httpBase = /\/v1$/i.test(origin) ? origin : `${origin}/v1`;
    } catch {
      return { ok: false, reason: `motius_ai.base_url is not a valid URL: ${raw}` };
    }
    return {
      ok: true,
      resolved: {

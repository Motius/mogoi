...
import type { MogoiConfig } from './types.ts';
 ...
 */
function normalizeLogFilePath(config: MogoiConfig): void {
 ...
 * the DB, so MOGOI_* stays a working self-host/dev convenience).
 */
export function applyEnvOverrides(config: MogoiConfig): void {
  const env = process.env;

  if (env.MOGOI_PORT) {
    const port = parseInt(env.MOGOI_PORT, 10);
    if (!isNaN(port)) config.daemon.port = port;
  }

  if (env.MOGOI_HOME) {
    const home = env.MOGOI_HOME;
    config.daemon.data_dir = home;
    config.daemon.db_path = join(home, 'mogoi.db');
  }
  ...
  if (env.MOGOI_BRAIN_DOMAIN) {
    config.daemon.brain_domain = env.MOGOI_BRAIN_DOMAIN;
  }

  if (env.MOGOI_PUBLIC_URL) {
    config.daemon.public_url = env.MOGOI_PUBLIC_URL;
  }

  if (env.MOGOI_WAKE_ENGINE) {
    const engine = env.MOGOI_WAKE_ENGINE;
    if (engine === 'openwakeword' || engine === 'webspeech' || engine === 'auto') {
      ...
      console.warn(`[Config] Invalid MOGOI_WAKE_ENGINE="${engine}" — must be openwakeword|webspeech|auto; ignoring.`);
    }
  }
  ...
  if (env.MOGOI_REALTIME_VOICE !== undefined) {
    if (!config.voice) config.voice = { wake_engine: 'openwakeword' };
    if (!config.voice.realtime) config.voice.realtime = { enabled: false };
    const v = env.MOGOI_REALTIME_VOICE.trim().toLowerCase();
    config.voice.realtime.enabled = v !== '' && v !== '0' && v !== 'false' && v !== 'no';
  }
}

export async function loadConfig(configPath?: string): Promise<MogoiConfig> {
  const path = configPath || expandTilde('~/.mogoi/config.yaml');
  ...
  const parsed = (doc.toJS() ?? {}) as Partial<MogoiConfig>;

  // Deep merge with defaults to ensure all required fields exist
  const config = deepMerge(structuredClone(DEFAULT_CONFIG), parsed) as MogoiConfig;
  ...
  applyEnvOverrides(config); // env wins over the discard (e.g. MOGOI_WAKE_ENGINE)
...
/**
 * Re-read ONLY the SYSTEM-owned `motius_ai` block from config.yaml into
 ...
 */
export async function reloadMotiusAIAiBlock(
  config: MogoiConfig,
  configPath?: string,
): Promise<void> {
  await reloadSystemBlock(config, 'motius_ai', configPath);
}

/**
 * Re-read ONLY the SYSTEM-owned `motius_billing` block, with exactly the
 * semantics of `reloadMotiusAIAiBlock`: a converge that rotates the billing
 * secret or moves the endpoint lands on SIGHUP without a restart.
 */
export async function reloadMotiusAIBillingBlock(
  config: MogoiConfig,
  configPath?: string,
): Promise<void> {
  await reloadSystemBlock(config, 'motius_billing', configPath);
}

async function reloadSystemBlock(
  config: MogoiConfig,
  key: 'motius_ai' | 'motius_billing',
  configPath?: string,
): Promise<void> {
  try {

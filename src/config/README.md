# M.O.G.O.I. Configuration System

Type-safe YAML configuration management for Project M.O.G.O.I.
...
Default location: `~/.mogoi/config.yaml`
...
// Load from default location (~/.mogoi/config.yaml)
...
SYSTEM config (daemon.*, auth, google, motius_ai) that the brain never
writes; in hosted mode it is root-owned and managed by the hosting server.

### The `motius_ai` block (hosted installs only)
...
```yaml
motius_ai:
  base_url: "https://llm.motius.host/v1"   # quote both values: unquoted
...
- **File-authoritative.** The block is re-read on SIGHUP and
  `POST /api/config/reload` (`reloadMotiusAIAiBlock`), so a key rotation
...
- **Never persisted.** The injected `motius_ai` provider entry is excluded
...
### The `motius_billing` block (hosted installs only)
...
```yaml
motius_billing:
  url: https://app.motius.dev/api/billing/instance   # signed POST, https
  instance_id: <uuid>
  secret: <64 hex>                                      # per-instance, derived
  page_url: https://app.motius.dev/billing           # opened by a person
```
...
- **File-authoritative** like `motius_ai`: re-read on SIGHUP and
  `POST /api/config/reload` (`reloadMotiusAIBillingBlock`).
...
daemon: {
  ...
  data_dir: string;      // Data directory path (default: ~/.mogoi)
  db_path: string;       // SQLite database path (default: ~/.mogoi/mogoi.db)
...
Mogoi only ever wrote a log file under `mogoi start -d` (the CLI redirects the
...
- The file is a ring capped at `log_file_max_bytes` (default 1 MiB, clamped to
  ...
  rewrite is atomic (temp file + `rename`), which is why `mogoi logs -f` uses
...
- If the launcher already has the daemon's stdout/stderr open on that same file
  (`mogoi start -d`, launchd's `StandardOutPath`, a `StandardOutput=append:`),
...
Core personality traits that guide M.O.G.O.I. behavior.
...
1. **Copy Example Config**:
   ```bash
   mkdir -p ~/.mogoi
   cp config.example.yaml ~/.mogoi/config.yaml
   ```

2. **Edit Configuration**:
   ```bash
   nano ~/.mogoi/config.yaml
   ```
...
1. **Never commit API keys**: Add `~/.mogoi/config.yaml` to `.gitignore`
...
```typescript
import type { MogoiConfig } from './config/index.ts';

function validateConfig(config: MogoiConfig): boolean {

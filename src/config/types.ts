...
export type GoogleConfig = {
  ...
  /**
   * HOSTED ONLY (motius, GOOGLE.md "Push bridging"). Present when the control
 ...
 * there is no separate realtime key. When disabled (default) MOGOI uses the
 * standard STT -> text LLM -> TTS pipeline.
 *
 * See docs/GPT_REALTIME_2_INTEGRATION.md.
 */
export type RealtimeVoiceConfig = {
  /** Master opt-in. Default false. Env: MOGOI_REALTIME_VOICE. */
  enabled: boolean;
...
export type STTConfig = {
  /**
   * `motius` is a pure string choice: the hosted "MotiusAI AI" STT rides
   * the system-owned `motius_ai` credentials, which are threaded to
   ...
   */
  provider: 'openai' | 'groq' | 'local' | 'sarvam' | 'motius';
  /**
   * ISO-639-1 hint sent to the Whisper-shaped providers (openai / groq /
   * local / motius). Unset = auto-detect (the `language` param is omitted
...
export type TTSConfig = {
  enabled: boolean;
  /**
   * Default: 'edge'. `motius` is a pure string choice like the STT one:
   * the hosted credentials ride the factory's separate `hosted` argument
   * (never this persisted section).
   */
  provider?: 'edge' | 'elevenlabs' | 'sarvam' | 'motius';
...
export type WorkflowConfig = {
   ...
   * MOGOI_ENGINE_IDLE_TTL_MS env var overrides this setting (workflows is
   ...
   * `${version}` placeholder expanded from the `MOGOI_VERSION` env var
   * (useful when one machine keeps artifacts per installed version). Any
   * unset/missing path just means mogoi does the work itself, as usual.
   * The MOGOI_ENGINE_CACHE_ROOT / MOGOI_SHARED_PIECES_DIR /
   * MOGOI_PIECE_METADATA_CACHE env vars are honored as fallbacks; config
   * wins when both are set.
   */
  /** Dir holding prebuilt engine bundles as `<hash>/main.js` — consulted
   * before building into `~/.mogoi/cache/engine`. */
  engine_dir?: string;
   ...
   * Library into `~/.mogoi/pieces` shadows the copy found here. */
  pieces_dir?: string;
...
export type AuthConfig = {
   ...
   * The brain is JWT-only by default: enroll a device (`mogoi enroll`) and
 ...
 * `enabled: false`, the `MOGOI_TELEMETRY=0` env var, or the community
...
export type LLMProviderKind =
  ...
  | 'litellm'
  | 'omniroute'
  // Hosted "MotiusAI AI": the platform's LLM proxy, configured exclusively
  // by the root-owned config.yaml `motius_ai` block (never the dashboard).
  | 'motius_ai';
...
export type LLMProviderEntry = {
  ...
  auth_header?: string;
  /**
   * motius_ai entries only: the system-block prompt-cache opt-in,
   * carried through by applyMotiusAIAi so the binding layer can see it.
   * Unlike the user-level `llm.prompt_cache` toggle (default ON), this
   * defaults OFF — see the `motius_ai.prompt_cache` block comment.
   */
...
export type MogoiConfig = {
   ...
   * applyMotiusAIAi (daemon/motius-ai.ts). Absent on self-hosted installs.
   */
  motius_ai?: {
    base_url?: string;
    api_key?: string;
   ...
   *
   * Its own block rather than inside `motius_ai`: that block is only
  ...
  daemon: {
     ...
     * only way in: `listen: unix:/run/mogoi/u_<id>.sock`. Omit for the
     ...
     * (`brain.example.com`, `10.0.0.5:1846`). Bare local hosts default to
     * ws/http; everything else defaults to wss/https.
     *
     * Legacy alias for `public_url`. Precedence is `MOGOI_PUBLIC_URL` /
     * `public_url`, then `MOGOI_BRAIN_DOMAIN` / this field, then the internal
     ...
     *
     * Under systemd - how the hosted fleet runs it, and `mogoi@.service`
     ...
     * `/home/u_<id>/.mogoi/logs/mogoi.log` rendered by the control plane; it
     * has to sit under the instance's home because the unit is
     * `ProtectSystem=strict` with `ReadWritePaths=/home/%i /run/mogoi`.
     *
...
export const DEFAULT_CONFIG: MogoiConfig = {
  user: {
    name: '',
  },
  telemetry: {
...
export const USER_OWNED_SECTIONS = [
  ...
  'goals',
  'workflows',
] as const satisfies readonly (keyof MogoiConfig)[];

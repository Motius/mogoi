...
function makeConfig(overrides: Partial<MogoiConfig> = {}): MogoiConfig {
  return { ...structuredClone(DEFAULT_CONFIG), ...overrides };
}

function withOpenAIProvider(config: MogoiConfig, key: string): MogoiConfig {
...
describe('resolveRealtimeVoice', () => {
  ...
  test('does not fall back to env vars - key must come from a configured provider', () => {
    ...
    const config = makeConfig();
    config.voice!.realtime = { enabled: true };
    const prevMogoi = process.env.MOGOI_OPENAI_KEY;
    const prevOpenAI = process.env.OPENAI_API_KEY;
    process.env.MOGOI_OPENAI_KEY = 'env-key';
    ...
    try {
      ...
      if (prevMogoi === undefined) delete process.env.MOGOI_OPENAI_KEY; else process.env.MOGOI_OPENAI_KEY = prevMogoi;
  ...
  test('an uppercase scheme in the provisioned block still yields a ws(s) URL', () => {
    const config = makeConfig();
    config.motius_ai = { base_url: 'HTTPS://LLM.MotiusAI.Host', api_key: 'sk-uj-abc' };
  ...
  test('a scheme-less base_url reads as https and derives wss://', () => {
    const config = makeConfig();
    config.motius_ai = { base_url: 'llm.motius.host', api_key: 'sk-uj-abc' };
    config.voice!.realtime = { enabled: true };
    const res = resolveRealtimeVoice(config);
    expect(res.ok).toBe(true);
    if (res.ok) {
      expect(res.resolved.url).toBe('wss://llm.motius.host/v1/realtime');
      expect(res.resolved.modelsUrl).toBe('https://llm.motius.host/v1/models');
    }
  });

  test('an uppercase /V1 suffix is recognized, not doubled', () => {
    const config = makeConfig();
    config.motius_ai = { base_url: 'https://llm.motius.host/V1', api_key: 'sk-uj-abc' };
    config.voice!.realtime = { enabled: true };
    const res = resolveRealtimeVoice(config);
    expect(res.ok).toBe(true);
    if (res.ok) {
  ...
  test('trailing slashes are stripped before the /v1 suffix check', () => {
    ...
    const res = resolveRealtimeVoice(config);
    expect(res.ok).toBe(true);
    if (res.ok) expect(res.resolved.url).toBe('wss://llm.motius.host/v1/realtime');
  });

  test('an unparseable or non-http base_url refuses instead of dialing garbage', () => {
    const bad = makeConfig();
    bad.motius_ai = { base_url: 'ftp://llm.motius.host', api_key: 'sk-uj-abc' };
  ...
  test('hosted fallback: no BYO key + motius_ai block resolves the proxy session', () => {
    const config = makeConfig();
    config.motius_ai = { base_url: 'https://llm.motius.host', api_key: 'sk-uj-abc' };
    config.voice!.realtime = { enabled: true, model: 'gpt-realtime-2', monthly_budget_usd: 25 };
    const res = resolveRealtimeVoice(config);
    expect(res.ok).toBe(true);
    if (res.ok) {
      expect(res.resolved.provider).toBe('motius_ai');
      expect(res.resolved.url).toBe('wss://llm.motius.host/v1/realtime');
      expect(res.resolved.modelsUrl).toBe('https://llm.motius.host/v1/models');
  ...
  test('hosted block with realtime disabled stays off; partial block falls to no-key', () => {
    ...
    disabled.motius_ai = { base_url: 'https://llm.motius.host', api_key: 'sk-uj-abc' };
  ...
  test('http dev proxy derives ws:// (not wss://)', () => {
    const config = makeConfig();
    config.motius_ai = { base_url: 'http://dev-llm.motius.dev:4000', api_key: 'sk-uj-abc' };
    config.voice!.realtime = { enabled: true };
    const res = resolveRealtimeVoice(config);
    expect(res.ok).toBe(true);
    if (res.ok) {
      expect(res.resolved.url).toBe('ws://dev-llm.motius.dev:4000/v1/realtime');
      expect(res.resolved.modelsUrl).toBe('http://dev-llm.motius.dev:4000/v1/models');
    }
  });

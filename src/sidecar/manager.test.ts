...
describe('buildEnrollmentUrls', () => {
  ...
  test('parses http URL into ws/http pair', () => {
    expect(buildEnrollmentUrls('http://10.0.0.5:1846')).toEqual({
      brainWs: 'ws://10.0.0.5:1846/sidecar/connect',
      jwksUrl: 'http://10.0.0.5:1846/api/sidecars/.well-known/jwks.json',
    });
  });

  test('bare localhost host gets ws/http (insecure)', () => {
    expect(buildEnrollmentUrls('localhost:1846')).toEqual({
      brainWs: 'ws://localhost:1846/sidecar/connect',
      jwksUrl: 'http://localhost:1846/api/sidecars/.well-known/jwks.json',
    });
  });

  test('bare 127.0.0.1 host gets ws/http (insecure)', () => {
    expect(buildEnrollmentUrls('127.0.0.1:1846')).toEqual({
      brainWs: 'ws://127.0.0.1:1846/sidecar/connect',
      jwksUrl: 'http://127.0.0.1:1846/api/sidecars/.well-known/jwks.json',
    });
  });
...
describe('isLocalhostBrainUrl', () => {
  test.each([
    ...
    ['[::1]', true],
    ['[::1]:1846', true],
    ['http://localhost:1846', true],
    ['ws://127.0.0.1:1846', true],
...
describe('SidecarManager key storage', () => {
  test('stores the enrollment private key with owner-only permissions', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'mogoi-sidecar-manager-'));
    try {
...
describe('SidecarManager panel sessions', () => {
  ...
  test('a revoked device is refused on the next request, not at the next sweep', async () => {
    await withManager(async (manager) => {
      ...
      // What `mogoi revoke` does from ANOTHER process: the row goes, and this
  ...
  test('a CLI revoke takes the rows with it, in that process, via the cascade', async () => {
    await withManager(async (manager) => {
      ...
      // What `mogoi revoke` does, from a process that knows nothing about

...
describe('revocation severs live sessions (review finding)', () => {
  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'mogoi-revoke-'));
    initDatabase(':memory:');
  ...
  test('sweep leaves enrolled sessions alone', async () => {
    const { sidecar } = await enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-A', {
      onExisting: 'upsert',
    });
  ...
  test('CLI-style revocation (row deleted by another process) is severed by the sweep', async () => {
    ...
    // What `mogoi revoke` does from its own process: delete the row.

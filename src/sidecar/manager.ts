...
export class SidecarManager implements Service {
   ...
   * Example: "shiny-panda.domain.com" or "localhost:1846"
   *
  ...
  setBrainUrl(url: string, source?: 'env' | 'config' | 'public_url' | 'default'): void {
    ...
    if (source) {
      ...
      const sourceLabel = source === 'env' ? 'MOGOI_PUBLIC_URL / MOGOI_BRAIN_DOMAIN env var'
        : source === 'public_url' ? 'daemon.public_url / MOGOI_PUBLIC_URL'
        : source === 'config' ? 'config.yaml daemon.brain_domain'
        : `default fallback — set daemon.public_url in config.yaml or MOGOI_PUBLIC_URL env to override`;
   ...
   * `mogoi enroll` CLI); the dashboard API keeps reject-on-duplicate.
   */
   ...
   * revocations the daemon cannot observe directly: `mogoi revoke` runs in
  ...
  registerConnection(sidecar: ConnectedSidecar): void {
    ...
    // sinks: `mogoi sidecars list --json` (read by the hosting server) and,

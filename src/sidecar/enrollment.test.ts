...
describe('standalone enrollment', () => {
  beforeEach(async () => {
    dataDir = await mkdtemp(join(tmpdir(), 'mogoi-enroll-'));
    initDatabase(':memory:');
  });
  ...
  test('mints a verifiable ES256 JWT with self-describing claims and stores the record', async () => {
    const result = await enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-NA23', {
      onExisting: 'upsert',
    });

    expect(result.created).toBe(true);
    const claims = decodeJwt(result.token);
    expect(claims.brain).toBe('wss://u1.vps1.motius.host/sidecar/connect');
    expect(claims.jwks).toBe('https://u1.vps1.motius.host/api/sidecars/.well-known/jwks.json');
  ...
  test('upsert by name: same device record, fresh jti, both tokens stay valid', async () => {
    const first = await enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-NA23', {
      onExisting: 'upsert',
    });
    const second = await enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-NA23', {
      onExisting: 'upsert',
    });
  ...
  test('re-mint after a brain_domain change points the token at the new host (migration)', async () => {
    await enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-NA23', { onExisting: 'upsert' });
    const moved = await enrollDevice(dataDir, 'u1.vps2.motius.host', 'desktop-NA23', {
      onExisting: 'upsert',
    });
    expect(decodeJwt(moved.token).brain).toBe('wss://u1.vps2.motius.host/sidecar/connect');
  });

  test('reject mode preserves the dashboard duplicate error', async () => {
    await enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-NA23', { onExisting: 'upsert' });
    await expect(
      enrollDevice(dataDir, 'u1.vps1.motius.host', 'desktop-NA23', { onExisting: 'reject' }),
...
test('rotate: fresh sid, old tokens die with the old row', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'mogoi-rotate-'));
  initDatabase(':memory:');
  try {
    const first = await enrollDevice(dir, 'u1.vps1.motius.host', 'desktop-A', { onExisting: 'upsert' });
    const rotated = await enrollDevice(dir, 'u1.vps1.motius.host', 'desktop-A', {
      onExisting: 'upsert',
      rotate: true,
    });
...
test('rotate: carries the old row\'s connection history onto the new sid', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'mogoi-rotate-seen-'));
  initDatabase(':memory:');
  try {
    const first = await enrollDevice(dir, 'u1.vps1.motius.host', 'desktop-A', { onExisting: 'upsert' });
...
test('rotate: a device that never connected stays never-connected', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'mogoi-rotate-unseen-'));
  initDatabase(':memory:');
  try {
    await enrollDevice(dir, 'u1.vps1.motius.host', 'desktop-A', { onExisting: 'upsert' });
    const rotated = await enrollDevice(dir, 'u1.vps1.motius.host', 'desktop-A', {
      onExisting: 'upsert',
      rotate: true,
    });

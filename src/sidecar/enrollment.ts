 ...
 * shared by the SidecarManager (daemon) and the `mogoi enroll` /
 * `mogoi revoke` / `mogoi sidecars` CLIs.
 *
...
// host[:port] (`brain.example.com`, `10.0.0.5:1846`). Pure function: no
...
export async function enrollDevice(
  ...
  if (!brainUrl) {
    throw new Error('Brain URL not configured - set daemon.public_url (or MOGOI_PUBLIC_URL)');
  }

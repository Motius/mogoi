...
export function buildToolGuide({ hasSidecars, piecesManaged, machines }: ToolGuideOptions): string {
  ...
  lines.push('## Browser');
  lines.push('');
  lines.push('Control a Chrome browser for web research and interaction. Chrome auto-launches on first use. A persistent profile at ~/.mogoi/browser/profile retains login sessions.');
  lines.push('');
  lines.push('Workflow:');
  ...
  lines.push('Create, run, and manage Mogoi workflows (automations).');

 ...
 * authority gating, and Mogoi-specific feature knowledge - just on the
 ...
 * Mogoi context (couldn't manage workflows, no tools, no role). Routing
...
const TEMPLATE_PROMPTS: Record<TaskTemplate, string> = {
  ...
  plan: `[TASK TEMPLATE: PLAN] Decompose the intent into concrete steps with clear ownership and rough effort. If the plan involves Mogoi features (workflows, commitments, goals, browser, sidecar) call the corresponding tools to inspect what already exists before drafting. Output a structured plan.\n\n${TOOL_USE_INSTRUCTION}`,
 ...
 * tier - which gives the task tier full access to the role's tools, Mogoi

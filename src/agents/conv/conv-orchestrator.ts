...
export class ConvOrchestrator {
  ...
  private buildStaticSystemPrompt(): string {
    ...
    parts.push(
      ...
      '`delegate` tool to send the work to a task tier that has access to all of Mogoi\'s ' +
      'real tools (workflows, browser, file ops, vault, calendar, commitments, goals, etc.).',
    );
    parts.push('');

    parts.push('# CRITICAL: You have NO direct knowledge of Mogoi state');
    parts.push(
    ...
    parts.push('- The user asks about real Mogoi state (existing workflows, files, vault, calendar) - tier=medium, template=general');
    ...
    parts.push('- The user asks who you are / what you can do / how Mogoi works (general capabilities, not specifics)');
    parts.push('');
    ...
    parts.push(
      ...
      'in the summary. If the summary mentions Mogoi features (workflows, ' +
      'commitments, goals), say so - do not suggest generic alternatives like ' +
      '"you could write a Python script" when the task already used a Mogoi ' +

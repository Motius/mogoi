...
const SYSTEM_PROMPT = `You generate 3 to 5 short voice-suggestion phrases for a MOGOI dashboard's "Try saying" panel.
...
export async function generateVoiceSuggestions(
  ...
  const contextLines = trimmedTurns
    .map((t) => `${t.role === 'user' ? 'USER' : 'MOGOI'}: ${t.text.replace(/\s+/g, ' ').slice(0, 200)}`)
    .join('\n');

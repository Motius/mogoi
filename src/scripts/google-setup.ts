 ...
 *
 * Interactive CLI to authenticate MOGOI with Google APIs.
 * Opens browser to Google consent screen, handles callback,
 * and saves tokens to ~/.mogoi/google-tokens.json.
 *
...
async function main() {
  console.log('');
  console.log('=== Google OAuth2 Setup for MOGOI ===');
  console.log('');
  ...
  if (!clientId || !clientSecret) {
    ...
    console.log('');
    console.log('Add the following to your ~/.mogoi/config.yaml:');
    console.log('');
    console.log('google:');
  ...
  if (auth.isAuthenticated()) {
    console.log('Already authenticated! Tokens exist at ~/.mogoi/google-tokens.json');
  ...
  try {
      ...
      async fetch(req) {
        ...
        if (url.pathname === '/api/auth/google/callback') {
          ...
          try {
            ...
            console.log(`Tokens saved to ~/.mogoi/google-tokens.json`);
            ...
            return new Response(
              '<html><body><h1>MOGOI Google Authorization Complete!</h1><p>Tokens saved. You can close this tab.</p></body></html>',
              { headers: { 'Content-Type': 'text/html' } }
            );
          } catch (err) {
    ...
    console.error('The Mogoi daemon is probably running and already owns that port.');

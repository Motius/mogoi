 ...
 * Tokens stored at ~/.mogoi/google-tokens.json
 */
...
export function googleTokensPath(): string {
  return path.join(os.homedir(), '.mogoi', 'google-tokens.json');
}
...
export class GoogleAuth {
  ...
  constructor(
    ...
    this.tokensPath = opts?.tokensPath ?? path.join(os.homedir(), '.mogoi', 'google-tokens.json');
    this.redirectUri = opts?.redirectUri ?? 'http://localhost:1846/api/auth/google/callback';
    this.loadTokens();
  }
   ...
   * `MogoiGoogleConnectionSource` to surface the full OAuth2-shaped value

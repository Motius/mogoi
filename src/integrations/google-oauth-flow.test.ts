...
describe('GoogleOAuthFlowStore', () => {
  it('binds a one-time state to the exact callback and PKCE verifier', () => {
    ...
    const started = store.start('https://mogoi.example.com/api/auth/google/callback', 1_000);
    ...
    expect(store.consume(started.state, 2_000)).toEqual({
      redirectUri: 'https://mogoi.example.com/api/auth/google/callback',
  ...
  it('rejects expired attempts', () => {
    ...
    const started = store.start('https://mogoi.example.com/callback', 1_000);

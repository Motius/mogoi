 ...
 *
 * Used by the daemon to flag TTS sentences that contain "Mogoi" so the
 ...
 * the SpeechRecognition wake matcher hears Mogoi say his own name and
 * interrupts the in-flight reply.
 *
 * Word-boundary aware so we don't false-positive on substrings like
 * "Mogoison". Case-insensitive. Also matches the loose "hey mogoi"
 * variant the recognizer accepts on the UI side.
 */
export function containsWakePhrase(text: string): boolean {
  if (!text) return false;
  return /\bmogoi\b/i.test(text);
}

/**
 * True when outgoing TTS text contains a spoken stop command ("Mogoi
 * stop" / "Mogoi (be) quiet"). Stop phrases deliberately bypass the
 * containsWake echo suppression on the UI side so the user can interrupt
 * a reply that says "Mogoi" — but if the reply itself *speaks* a stop
 * phrase (e.g. explaining "say 'Mogoi, stop' to interrupt me"), that
 ...
 * how TTS text usually renders them ("Mogoi, stop").
 */
export function containsStopPhrase(text: string): boolean {
  if (!text) return false;
  return /\bmogoi\b\W+(?:stop|(?:be\s+)?quiet)\b/i.test(text);
}
 ...
 * every punctuation mark -- "Mogoi, $100 budget" and "Mogoi, #general"
 ...
 * "mogoi" in the transcript, so "I'm at home, Mogoi play music" =>
 ...
 * STT renders a bare summon as "Hey Mogoi." or "Hey Mogoi!" more or

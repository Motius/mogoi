...
import { MotiusAIAIProvider } from './motius.ts';
...
describe('MotiusAIAIProvider', () => {
  it('normalizes the provisioner-written origin to a /v1 base', () => {
    const bare = new MotiusAIAIProvider('https://llm.motius.host', 'sk-uj-a');
    expect((bare as any).apiUrl).toBe('https://llm.motius.host/v1/chat/completions');
    // Already-suffixed and trailing-slash forms stay stable.
    const suffixed = new MotiusAIAIProvider('https://llm.motius.host/v1/', 'sk-uj-a');
    expect((suffixed as any).apiUrl).toBe('https://llm.motius.host/v1/chat/completions');
  });

  it('is created by the canonical config binding only when fully configured', () => {
    const provider = instantiateProvider('motius_ai', {
      kind: 'motius_ai',
      base_url: 'https://llm.motius.host',
      api_key: 'sk-uj-abc',
    });
    expect(provider).toBeInstanceOf(MotiusAIAIProvider);
    expect(provider?.name).toBe('motius_ai');
    // System-owned config always carries both values; a partial entry is a
    // broken render, not something to guess around.
    expect(instantiateProvider('motius_ai', { kind: 'motius_ai', base_url: 'https://x' })).toBeNull();
  });
  ...
  it('listModels returns the key-scoped alias catalog, deduped + sorted', async () => {
    ...
    const provider = new MotiusAIAIProvider('https://llm.motius.host', 'sk-uj-abc');
    expect(await provider.listModels()).toEqual(['uj-chat', 'uj-low']);
    expect(url).toBe('https://llm.motius.host/v1/models');
  ...
  it('budget copy quotes the /key/info reset time and memoizes the lookup', async () => {
    ...
    expect(infoUrl).toBe('https://llm.motius.host/key/info');
  ...
  it('budget copy degrades to time-less when /key/info fails, leaking nothing', async () => {
    globalThis.fetch = (async (input: any) => {
      if (String(input).endsWith('/key/info')) {
        throw new Error('connect ETIMEDOUT llm.motius.host:443');
      }
    ...
    expect(message).not.toContain('llm.motius.host');
  });
  ...
  it('rewrites stream error EVENTS, leaking neither the key nor the body', async () => {
    ...
    globalThis.fetch = (async () => new Response(
      `<html><title>401</title><body>Upstream rejected credential Bearer ${key} `
        + 'for host llm.motius.host. ' + 'x'.repeat(4000) + '</body></html>',
      { status: 401, headers: { 'Content-Type': 'text/html' } },
    )) as unknown as typeof fetch;
    const provider = new MotiusAIAIProvider('https://llm.motius.host', key);
    ...
    expect(text).not.toContain('llm.motius.host');
...
describe('MotiusAIAIProvider prompt-cache markers', () => {
  const capture = async (
    ...
    const provider = new MotiusAIAIProvider('https://llm.motius.host', 'sk-uj-abc', opts);
  ...
  it('sends no cache_control unless the system block opts in', async () => {
    const sent = await capture([
      { role: 'system', content: 'You are Mogoi. ' + 'x'.repeat(200), cache: true },
  ...
  it('config binding enables markers only with the block opt-in AND the user switch', () => {
    const entry = {
      kind: 'motius_ai' as const,
      base_url: 'https://llm.motius.host',
      api_key: 'sk-uj-abc',
    };
    const flag = (p: unknown): boolean => (p as any).promptCache;
    expect(flag(instantiateProvider('motius_ai', entry))).toBe(false);
    expect(flag(instantiateProvider('motius_ai', { ...entry, prompt_cache: true }))).toBe(true);
    expect(flag(instantiateProvider('motius_ai', { ...entry, prompt_cache: true }, { promptCache: false }))).toBe(false);
  });
  ...
  it('marks the cache:true system block and the newest user turn', async () => {
    ...
    expect(sent.messages[0].content).toEqual([
      { type: 'text', text: expect.stringContaining('You are Mogoi.'), cache_control: { type: 'ephemeral' } },
    ]);
  ...
  it('ends the cached prefix on stable bytes when a dynamic system block follows', async () => {
    const persona = 'You are Mogoi. ' + 'x'.repeat(2000);
  ...
  it('skips an empty cache:true system block and marks the previous one', async () => {
    const persona = 'You are Mogoi. ' + 'x'.repeat(500);
    const sent = await capture([
      { role: 'system', content: persona, cache: true },

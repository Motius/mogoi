...
import { GroqProvider, isGroqMogoiModel, relaxOptionalFieldsToNullable } from './groq.ts';
...
describe('Groq request shaping', () => {
  ...
  test('live catalog filtering excludes non-chat routes', () => {
    expect(isGroqMogoiModel('openai/gpt-oss-20b')).toBe(true);
    expect(isGroqMogoiModel('qwen/qwen3.6-27b')).toBe(true);
    expect(isGroqMogoiModel('minimaxai/minimax-m2.7')).toBe(true);
    expect(isGroqMogoiModel('whisper-large-v3')).toBe(false);
    expect(isGroqMogoiModel('canopylabs/orpheus-v1-english')).toBe(false);
    expect(isGroqMogoiModel('groq/compound')).toBe(false);
    expect(isGroqMogoiModel('groq/compound-mini')).toBe(false);
    expect(isGroqMogoiModel('meta-llama/llama-guard-4-12b')).toBe(false);
    expect(isGroqMogoiModel('openai/gpt-oss-safeguard-20b')).toBe(false);
    expect(isGroqMogoiModel('playai-tts')).toBe(false);
  });
  ...
  test('live catalog keeps deprecated models the account can still reach', () => {
    expect(isGroqMogoiModel('llama-3.3-70b-versatile')).toBe(true);
    expect(isGroqMogoiModel('llama-3.1-8b-instant')).toBe(true);
    expect(isGroqMogoiModel('deepseek-r1-distill-llama-70b')).toBe(true);
  });

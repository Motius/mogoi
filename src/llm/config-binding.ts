...
export function instantiateProvider(
  ...
  switch (kind) {
    ...
    case 'motius_ai':
      ...
      // config.yaml block (daemon/motius-ai.ts) - nothing to guess.
      if (!entry.base_url?.trim() || !entry.api_key?.trim()) return null;
      provider = new MotiusAIAIProvider(entry.base_url.trim(), entry.api_key.trim(), {
        ...
        // motius_ai.prompt_cache in config/types.ts), AND still subject

...
/**
 * Hosted "MotiusAI AI" provider: the platform's OpenAI-compatible LLM proxy.
 * Configured EXCLUSIVELY by the system-owned `motius_ai` config.yaml block
 * (daemon/motius-ai.ts injects it over every DB merge) — never by the
 ...
 */
export class MotiusAIAIProvider extends OpenAIProvider {
  override name = 'motius_ai';
  ...
  constructor(baseUrl: string, apiKey: string, opts?: { promptCache?: boolean }) {
    ...
    // see the `motius_ai.prompt_cache` block comment in config/types.ts
   ...
   * (see "MotiusAI AI prompt caching" in docs/LLM_PROVIDERS.md for the
  ...
  async listModelsDetailed(): Promise<{ models: string[]; degraded: boolean }> {
    const fallback = { models: MotiusAIAIProvider.FALLBACK_MODELS.slice(), degraded: true };
    try {
      ...
      if (!response.ok) {
        console.warn(`[motius] model catalog unavailable (${response.status}); serving fallback aliases`);
        return fallback;
      }
      const payload = await response.json() as { data?: Array<{ id?: unknown }> };
      if (!Array.isArray(payload.data)) {
        console.warn(`[motius] model catalog malformed; serving fallback aliases`);
        return fallback;
      }
      ...
      console.warn('[motius] model catalog fetch failed; serving fallback aliases:', err instanceof Error ? err.message : err);
      return fallback;
    }
  }
  ...
  private static bumpForThinkingBudget(options: LLMOptions | undefined): LLMOptions | null {
    ...
    if (current >= MotiusAIAIProvider.THINKING_RETRY_MAX_TOKENS) return null;
    const model = options?.model ?? '(default)';
    if (!MotiusAIAIProvider.warnedThinkingTiers.has(model)) {
      MotiusAIAIProvider.warnedThinkingTiers.add(model);
      console.warn(
        `[motius] ${model} rejected max_tokens=${current || '(unset)'} below its thinking budget; ` +
          `retried with ${MotiusAIAIProvider.THINKING_RETRY_MAX_TOKENS}. If this tier runs short ` +
          `structured calls, drop reasoning_effort from its model.`,
      );
    }
    return { ...options, max_tokens: MotiusAIAIProvider.THINKING_RETRY_MAX_TOKENS };
  }
  ...
  override async chat(
    ...
    try {
      ...
      if (MotiusAIAIProvider.isThinkingBudgetError(error)) {
        const [messages, options] = args;
        const bumped = MotiusAIAIProvider.bumpForThinkingBudget(options);
        if (bumped) {
          try {
    ...
    restart: for (;;) {
      ...
      for await (const event of super.stream(messages, options)) {
        if (event.type === 'error' && typeof event.error === 'string') {
          if (!retried && !yieldedContent && MotiusAIAIProvider.isThinkingBudgetError(event.error)) {
            const bumped = MotiusAIAIProvider.bumpForThinkingBudget(options);
            if (bumped) {
              retried = true;
              options = bumped;
          ...
          yield {
            ...
            ...(MotiusAIAIProvider.isRoutingMiss(event.error) ? MotiusAIAIProvider.ROUTING_MISS_RETRY : {}),
          };
        } else {
          yieldedContent = true;
          yield event;
  ...
  private async budgetResetAt(): Promise<Date | null> {
    const now = Date.now();
    if (this.resetCache && now - this.resetCache.at < MotiusAIAIProvider.RESET_CACHE_MS) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!value) console.warn('[motius] /key/info carried no parseable budget_reset_at; budget copy stays time-less');
      } else {
        console.warn(`[motius] /key/info unavailable (${response.status}); budget copy stays time-less`);
      }
    } catch (err) {
      console.warn(
        '[motius] /key/info lookup failed; budget copy stays time-less:',
   ...
   * form keeps the exact `MotiusAI AI API error (NNN)` prefix that
  ...
  private static isRoutingMiss(text: string): boolean {
    const match = text.match(MotiusAIAIProvider.BASE_ERROR_RE);
  ...
  private async rewrite(error: unknown): Promise<unknown> {
    ...
    const match = error.message.match(MotiusAIAIProvider.BASE_ERROR_RE);
    if (!match) return error;
    const friendly = await this.friendly(Number(match[1] ?? match[2]), match[3] ?? '');
    if (!MotiusAIAIProvider.isRoutingMiss(error.message)) return friendly;
    ...
    const { code, retry_after_ms } = MotiusAIAIProvider.ROUTING_MISS_RETRY;

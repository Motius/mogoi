 ...
 *
 *   mogoi export [--out <path>|-] [--full]   consistent archive of user data
 *   mogoi restore <archive|->                validate + replace data from an archive
 *
 ...
 * data dir on a MOGOI_HOME install the daemon has migrated, ~/.mogoi
 ...
 * secret entries are still ~/.mogoi-only; the legacy-root warning further
 * down covers them.
 *
 * Restore is stop-before-restore: it acquires the daemon's own flock (which
 * is MOGOI_HOME-aware, same resolution the daemon uses) plus the data dir's,
...
export async function cmdExport(args: string[], io: CliIo = defaultIo): Promise<number> {
  ...
  if (outIdx !== -1) {
    ...
    if (!out || (out.startsWith('--') && out !== '-')) {
      io.err('usage: mogoi export [--out <path>|-] [--full]');
      return 2;
    }
  }
  ...
  const outPath = toStdout
    ? undefined
    : out ?? `mogoi-export-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.tar`;
  ...
  try {
    ...
    try {
      ...
      snapshotDb(dbPath, join(staging, 'mogoi.db'));

      // The keychain is RESOLVED, not assumed: it sits in the data dir once
      // the daemon has migrated it there, and in ~/.mogoi before that. Taking
      ...
      const manifest: ExportManifest = {
        ...
        full,
        files: ['manifest.json', 'mogoi.db', ...entries],
      };
...
export async function cmdRestore(
  ...
  if (!archive) {
    io.err('usage: mogoi restore <archive|->');
    return 2;
  }
  ...
  try {
    ...
    // Stop-before-restore: ACQUIRE the daemon's own lock (MOGOI_HOME-aware —
    ...
    for (const lockPath of lockPaths) {
      ...
      if (!lock) {
        ...
        io.err('error: the daemon is running — stop it first (mogoi stop)');
        return 1;
      }
      heldLocks.push(lock);
    }
    ...
    try {
      ...
      try {
        ...
        const snapPath = join(staging, 'mogoi.db');
        if (!existsSync(snapPath)) throw new Error('archive has no mogoi.db');
        ...
        assertPlainEntry(snapPath, 'mogoi.db');
        ...
        const restored: string[] = ['mogoi.db'];
        const failPoint = (name: string) => {
          if (_hooks.failAfterSwap === name) throw new Error(`injected failure after swapping '${name}'`);
        };
        try {
          swaps.push(retireEntry(`${dbPath}-wal`, join(asideDir, 'mogoi.db-wal')));
          swaps.push(retireEntry(`${dbPath}-shm`, join(asideDir, 'mogoi.db-shm')));
          swaps.push(swapIn(snapPath, dbPath, join(asideDir, 'mogoi.db')));
          failPoint('mogoi.db');
        ...
        // dir when MOGOI_HOME is set, ~/.mogoi otherwise), so a pair restored
        ...
        io.err('Restore complete. Start the daemon with: mogoi start');
        return 0;
      } finally {
        rmSync(staging, { recursive: true, force: true });
      }

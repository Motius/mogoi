/**
 * `mogoi update` — install-method-aware updater.
 *
 * Dispatches based on how MOGOI was installed:
 *   docker      refuse, point user at host-side `docker pull`
 *   bun-global  `bun update -g @motius/brain`
...
/**
 * Spawn the daemon in a detached process, same pattern as `mogoi start -d`.
...
function restartDaemonDetached(packageRoot: string): void {
  ...
  const child = spawn('bun', [join(packageRoot, 'bin/mogoi.ts'), 'start', '--no-open'], {
    detached: true,
...
function updateDocker(info: InstallMethodInfo): UpdateResult {
  console.log(c.yellow('MOGOI is running inside a Docker container.'));
  console.log('');
  console.log('  `mogoi update` does not apply to container installs — the container is the unit of update.');
  console.log('  From your host, run:');
  console.log(c.dim('    docker pull <your-mogoi-image>'));
  console.log(c.dim('    docker rm -f mogoi && docker run -d ... <your-mogoi-image>'));
  console.log('');
...
function updateDev(info: InstallMethodInfo, packageRoot: string): UpdateResult {
  console.log(c.yellow('MOGOI is running from a developer checkout.'));
  console.log('');
  console.log('  `mogoi update` does not manage dev checkouts — updating your working tree');
...
function updateUnknown(info: InstallMethodInfo): UpdateResult {
  console.log(c.red('Could not determine how MOGOI was installed.'));
  console.log('');
  console.log('  `mogoi update` needs to know whether to run `bun update -g`, `git pull`,');
  console.log('  or redirect you to `docker pull`. Run `mogoi doctor` to see what was detected.');
  console.log('');
  console.log('  Update manually with one of:');
  console.log(c.dim('    bun update -g @motius/brain              # if installed via bun'));
...
function updateBunGlobal(
  ...
  console.log(c.dim('Running `bun update -g @motius/brain`...\n'));

  const result = spawn(['bun', 'update', '-g', '@motius/brain']);
...
export async function runUpdate(deps: UpdateDeps): Promise<UpdateResult> {
  ...
  if (runningPid && result.outcome !== 'failed' && restart) {
    if (daemonStopped) {
      ...
      console.log(c.dim(`  Stop PID ${holder} and run \`mogoi start\` to pick up the update.`));
    }
  }

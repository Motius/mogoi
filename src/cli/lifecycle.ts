...
export const DEFAULT_DAEMON_PORT = 1846;
 ...
 */
export function readConfiguredPort(configPath = join(homedir(), '.mogoi', 'config.yaml')): number | null {
  try {
...
export type StopPortResolution =
   ...
   * `mogoi stop` must be pid-only - SIGTERM/SIGKILLing whatever same-user
   * process happens to listen on daemon.port/1846 would murder an unrelated
 ...
 */
export function readConfiguredUnixListen(configPath = join(homedir(), '.mogoi', 'config.yaml')): string | null {
  try {
    if (!existsSync(configPath)) return null;
    const text = readFileSync(configPath, 'utf-8');
    const doc = YAML.parseDocument(text, { merge: true });
...
/**
 * Resolve which port `mogoi stop` should verify.
 *
 * Precedence (highest first):
 *   1. The port the running daemon recorded in its lockfile (authoritative).
 *   2. `MOGOI_PORT` env var (matches `applyEnvOverrides` in the config loader).
 *   3. An explicit `--port N` passed on the stop command line.
 *   4. `daemon.port` from `~/.mogoi/config.yaml`.
 *   5. The hardcoded default (1846).
 *
 ...
 * env var beats `--port` so a user with a persistent `MOGOI_PORT` in their
...
export function resolveStopPort(options?: {
  ...
  const fromEnv = validPort(env.MOGOI_PORT);

 ...
 * MOGOI_HOME — each test gets a throwaway data dir through that seam, exactly
...
function seedDataDir(dataDir: string): void {
  ...
  const db = new Database(join(dataDir, 'mogoi.db'), { create: true });
  ...
  Bun.write(join(dataDir, 'logs', 'mogoi.log'), 'log line\n');
}
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'mogoi-backup-test-'));
  dataDir = join(root, 'data');
  seedDataDir(dataDir);
  prevHome = process.env.MOGOI_HOME;
  process.env.MOGOI_HOME = dataDir;
  // Pin the keychain to the throwaway dir as well: export resolves it rather
  // than assuming data_dir, and must never reach the real ~/.mogoi store.
  prevSecretsDir = process.env.MOGOI_SECRETS_DIR;
  process.env.MOGOI_SECRETS_DIR = dataDir;
});

afterEach(() => {
  if (prevHome === undefined) delete process.env.MOGOI_HOME;
  else process.env.MOGOI_HOME = prevHome;
  if (prevSecretsDir === undefined) delete process.env.MOGOI_SECRETS_DIR;
  else process.env.MOGOI_SECRETS_DIR = prevSecretsDir;
  rmSync(root, { recursive: true, force: true });
});

describe('mogoi export', () => {
  test('default export curates user data and EXCLUDES secrets + config + logs', async () => {
    ...
    expect(entries).toContain('mogoi.db');
    ...
    expect(entries).not.toContain('mogoi.db-wal');
  ...
  test('--full picks the keychain up from where it actually lives, not from data_dir', async () => {
    ...
    process.env.MOGOI_SECRETS_DIR = elsewhere;
    ...
    process.env.MOGOI_SECRETS_DIR = readAt;
  ...
  test('snapshot is consistent: uncommitted writes are not captured', async () => {
    ...
    const writer = new Database(join(dataDir, 'mogoi.db'));
    writer.exec('BEGIN');
    ...
    const proc = Bun.spawn(['tar', '-xf', archive, '-C', extractTo]);
    expect(await proc.exited).toBe(0);
    const snap = new Database(join(extractTo, 'mogoi.db'), { readonly: true });
...
describe('mogoi restore', () => {
  test('round-trip: export --full, wipe, restore recovers db + files + modes', async () => {
    ...
    const db = new Database(join(dataDir, 'mogoi.db'), { readonly: true });
  ...
  test('refuses while the daemon holds its lock', async () => {
    // With MOGOI_HOME set, `dataDir/mogoi.pid` IS the daemon's own lock path
    ...
    expect(await cmdExport(['--out', archive], capture().io)).toBe(0);
    const lock = acquireLockAt(join(dataDir, 'mogoi.pid'), process.pid);
  ...
  test('rejects an archive with a corrupt database', async () => {
    // Build a tar whose mogoi.db is garbage but whose manifest is valid.
    const forge = join(root, 'forge');
    mkdirSync(forge, { recursive: true });
    await Bun.write(
      join(forge, 'manifest.json'),
      JSON.stringify({ format: EXPORT_FORMAT, brainVersion: '0.0.0', createdAt: '', full: false, files: ['manifest.json', 'mogoi.db'] }),
    );
    await Bun.write(join(forge, 'mogoi.db'), 'garbage bytes, not sqlite');
    const bad = join(root, 'bad.tar');
    const proc = Bun.spawn(['tar', '-cf', bad, '-C', forge, 'manifest.json', 'mogoi.db']);
    expect(await proc.exited).toBe(0);
  ...
  test('rejects an unsupported manifest format', async () => {
    ...
    const db = new Database(join(forge, 'mogoi.db'), { create: true });
  ...
  test('an archive whose entry is a SYMLINK is refused, data untouched', async () => {
    ...
    const proc = Bun.spawn(['tar', '-cf', evil, '-C', forge, 'manifest.json', 'mogoi.db', 'content']);
    expect(await proc.exited).toBe(0);
  ...
  test('a failed swap rolls back: no half-restored data dir, WAL preserved', async () => {
    ...
    const dbBefore = new Database(join(dataDir, 'mogoi.db'));
    dbBefore.exec("INSERT INTO facts (body) VALUES ('written-after-the-export')");
    dbBefore.close();
    rmSync(join(dataDir, 'mogoi.db-wal'), { force: true });
    await Bun.write(join(dataDir, 'mogoi.db-wal'), 'SENTINEL-WAL-FRAMES');
    ...
    expect(await Bun.file(join(dataDir, 'mogoi.db-wal')).text()).toBe('SENTINEL-WAL-FRAMES');
    ...
    rmSync(join(dataDir, 'mogoi.db-wal'));
    rmSync(join(dataDir, 'mogoi.db-shm'), { force: true });
    ...
    // `mogoi export` (which does open readonly) returns 0.
    const db = new Database(join(dataDir, 'mogoi.db'));
  ...
  test('warns when the archive comes from a newer brain', async () => {
    ...
    await Bun.write(
      join(forge, 'manifest.json'),
      JSON.stringify({ format: EXPORT_FORMAT, brainVersion: '999.0.0', createdAt: '', full: false, files: ['manifest.json', 'mogoi.db'] }),
    );
    const db = new Database(join(forge, 'mogoi.db'), { create: true });
    db.exec('CREATE TABLE t (x)');
    db.close();
    const newer = join(root, 'newer.tar');
    const proc = Bun.spawn(['tar', '-cf', newer, '-C', forge, 'manifest.json', 'mogoi.db']);
    expect(await proc.exited).toBe(0);
...
describe('bin dispatch (subprocess)', () => {
  test('mogoi export --out - streams a valid tar to stdout', async () => {
    const repoRoot = join(import.meta.dir, '..', '..');
    const proc = Bun.spawn(['bun', join(repoRoot, 'bin', 'mogoi.ts'), 'export', '--out', '-', '--full'], {
      env: { ...process.env, MOGOI_HOME: dataDir, MOGOI_SECRETS_DIR: dataDir },
      stdout: 'pipe',
      stderr: 'pipe',
    });

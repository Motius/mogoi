...
/**
 * Check for core CLI tools MOGOI relies on for common workflows.
 */
...
export function checkGoogleAuth(): DepStatus {
  const tokensPath = join(homedir(), '.mogoi', 'google-tokens.json');
...
export async function setupGoogleOAuth(config: any): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    ...
    try {
        ...
        async fetch(req) {
          ...
          if (url.pathname === '/api/auth/google/callback') {
            ...
            try {
              ...
              return new Response(
                '<html><body><h1>MOGOI Google Authorization Complete!</h1><p>You can close this tab.</p></body></html>',
                { headers: { 'Content-Type': 'text/html' } }
              );
            } catch (err) {
              clearTimeout(timeout);
      ...
      printInfo('The MOGOI daemon is probably running — connect Google from the dashboard (Settings → Integrations),');

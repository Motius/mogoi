...
function getMogoiPath(): string {
  // When installed globally, import.meta.dir points to the package
  return join(import.meta.dir, '../../bin/mogoi.ts');
}
...
const SYSTEMD_SERVICE = join(SYSTEMD_DIR, 'mogoi.service');

export function generateSystemdUnit(): string {
  const bunPath = getBunPath();
  const mogoiPath = getMogoiPath();

  return `[Unit]
Description=M.O.G.O.I. Daemon
After=network.target

[Service]
Type=simple
ExecStart=${bunPath} ${mogoiPath} start --foreground
Restart=on-failure
RestartSec=5
Environment=HOME=${homedir()}
${systemdMogoiHomeLine()}
[Install]
...
/**
 * Propagate MOGOI_HOME into the systemd unit when the installing shell has one
 * set.
 *
 * Without this the service runs against ~/.mogoi while the CLI that installed
 * it (and `mogoi logs`, and the restart in `mogoi update`) resolves
 * $MOGOI_HOME — so the daemon locks and logs under one root while every tool
 ...
 */
function systemdMogoiHomeLine(): string {
  const home = process.env.MOGOI_HOME;
  if (!home) return '';
  const escaped = home
    .replace(/\\/g, '\\\\')
    .replace(/"/g, '\\"')
    .replace(/%/g, '%%');
  return `Environment="MOGOI_HOME=${escaped}"\n`;
}
...
async function installSystemd(): Promise<boolean> {
  try {
    ...
    const enable = Bun.spawnSync(['systemctl', '--user', 'enable', 'mogoi.service']);
    if (enable.exitCode !== 0) {
      printErr('Failed to enable service. You may need to run: systemctl --user enable mogoi.service');
      return false;
    }
...
async function startSystemdService(): Promise<boolean> {
  try {
    const start = Bun.spawnSync(['systemctl', '--user', 'start', 'mogoi.service']);
    if (start.exitCode !== 0) {
      printErr('Failed to start systemd service. You may need to run: systemctl --user start mogoi.service');
      return false;
    }

    printOk('MOGOI keepalive service is running.');
    return true;
  } catch (err) {
...
export function scheduleSystemdRestart(spawnSync: SpawnSyncFn = defaultSpawnSync): boolean {
  ...
  try {
    const res = spawnSync(['systemctl', '--user', '--no-block', 'restart', 'mogoi.service']);
...
async function uninstallSystemd(): Promise<boolean> {
  try {
    Bun.spawnSync(['systemctl', '--user', 'stop', 'mogoi.service']);
    Bun.spawnSync(['systemctl', '--user', 'disable', 'mogoi.service']);
...
const LAUNCHD_PLIST = join(LAUNCHD_DIR, 'ai.mogoi.daemon.plist');

export function generateLaunchdPlist(): string {
  const bunPath = getBunPath();
  const mogoiPath = getMogoiPath();
  // getLogDir() so the plist's StandardOutPath matches where the daemon itself
  // resolves its log file (both honor MOGOI_HOME) — and the plist exports the
...
<dict>
  <key>Label</key>
  <string>ai.mogoi.daemon</string>
  <key>ProgramArguments</key>
  <array>
    <string>${xmlEscape(bunPath)}</string>
    <string>${xmlEscape(mogoiPath)}</string>
    <string>start</string>
  ...
  <string>${xmlEscape(logDir)}/mogoi.log</string>
  <key>StandardErrorPath</key>
  <string>${xmlEscape(logDir)}/mogoi-error.log</string>
  <key>EnvironmentVariables</key>
  <dict>
    <key>HOME</key>
    <string>${xmlEscape(homedir())}</string>
${process.env.MOGOI_HOME ? `    <key>MOGOI_HOME</key>\n    <string>${xmlEscape(process.env.MOGOI_HOME)}</string>\n` : ''}    <key>PATH</key>
  ...
  try {
    ...
    if (typeof uid === 'number') {
      ...
      if (bootstrap.exitCode === 0 || isLaunchdAlreadyLoaded(bootstrap)) {
        printOk('MOGOI launch agent is running.');
        return true;
      }
...
function scheduleLaunchdRestart(): boolean {
  ...
  const command = uid != null
    ? `sleep 1; launchctl kickstart -k gui/${uid}/ai.mogoi.daemon >/dev/null 2>&1`
    : `sleep 1; launchctl kickstart -k gui/$(id -u)/ai.mogoi.daemon >/dev/null 2>&1`;

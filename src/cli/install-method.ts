 ...
 *
 * MOGOI ships through three install paths — a published bun/npm package,
 * a git-clone installer script, and a Docker image — plus a fourth "developer
 * checkout" case. `mogoi update` and `mogoi uninstall` need different
...
/**
 * Detect how MOGOI was installed. Pure function — all environmental inputs
...
export function detectInstallMethod(
  ...
  if (env.MOGOI_INSTALL_METHOD === 'docker') {
    return { method: 'docker', reason: 'MOGOI_INSTALL_METHOD=docker' };
  }
  ...
  const scriptInstallRoot = join(home, '.mogoi', 'daemon');
...
/**
 * Human-readable one-liner for the given method. Used by `mogoi doctor`.
 */
export function describeInstallMethod(info: InstallMethodInfo): string {
  const labels: Record<InstallMethod, string> = {
    docker: 'Docker container',
    'bun-global': 'Bun global install (@motius/brain)',
    script: 'install.sh (git clone under ~/.mogoi/daemon)',
 ...
 * `mogoi update` / `mogoi uninstall` delegate to these internally — the
...
export function getMethodCommands(method: InstallMethod): MethodCommands {
  switch (method) {
    case 'docker':
      return {
        update: 'docker pull <image> && docker rm -f mogoi && docker run -d ... <image>',
        uninstall: 'docker rm -f mogoi   # and `docker volume rm mogoi-data` to delete data',
      };
    case 'bun-global':
      return {
        update: 'mogoi update   # runs `bun update -g @motius/brain`',
        uninstall: 'mogoi uninstall   # runs `bun uninstall -g @motius/brain` and cleans up',
      };
    case 'script':
      return {
        update: 'mogoi update   # checks out the latest release tag + `bun install`',
        uninstall: 'mogoi uninstall   # removes ~/.mogoi and the CLI wrapper',
      };
    case 'dev':
      return {
        update: 'git pull && bun install   # manage your dev checkout yourself',
        uninstall: 'rm -rf <your checkout>   # `mogoi uninstall` only cleans side effects',
      };
    case 'unknown':
      return {
        update: '(install method unknown — see `mogoi doctor`)',
        uninstall: '(install method unknown — see `mogoi doctor`)',
      };
  }
}

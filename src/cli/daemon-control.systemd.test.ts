 ...
 * never touch the real `mogoi.service`.
 */
...
const UNIT_PREFIX = `mogoi-itest-${process.pid}`;
...
describe.skipIf(!ENABLED)('daemon stop under a real systemd supervisor', () => {
  beforeAll(() => {
    ...
    DATA_DIR = mkdtempSync(join(tmpdir(), 'mogoi-systemd-test-'));
...
`, 'utf-8');
    ...
    startUnit(name, `[Unit]
Description=mogoi lock relaunch probe
[Service]
Type=simple
ExecStart=${Bun.which('bun')} ${holder}
Restart=always
RestartSec=0
Environment="MOGOI_HOME=${DATA_DIR}"
`);
    ...
    expect(existsSync(join(DATA_DIR, 'mogoi.pid'))).toBe(true); // and left its lock alone
  }, 60_000);
  ...
  test('the generated Environment= line round-trips through systemd', () => {
    ...
    process.env.MOGOI_HOME = hostile;
    let envLine: string;
    try {
      envLine = generateSystemdUnit()
        .split('\n')
        .find((l) => l.startsWith('Environment="MOGOI_HOME='))!;
    } finally {
      process.env.MOGOI_HOME = prev;
    }
    ...
    writeFileSync(dump,
      ...
      + `writeFileSync(${JSON.stringify(outFile)}, process.env.MOGOI_HOME ?? '<unset>');\n`,
      'utf-8');

    const name = `${UNIT_PREFIX}-env`;
    startUnit(name, `[Unit]
Description=mogoi env round-trip probe
[Service]
Type=oneshot

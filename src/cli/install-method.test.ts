...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'mogoi-install-method-'));
...
describe('detectInstallMethod — docker signals', () => {
  test('MOGOI_INSTALL_METHOD=docker wins even when marker says otherwise', () => {
    ...
    expect(info.reason).toContain('MOGOI_INSTALL_METHOD');
  });
...
describe('detectInstallMethod — path inference', () => {
  test('package root under ~/.bun/install/global is bun-global', () => {
    const root = join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@motius', 'brain');
    mkdirSync(root, { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
      dockerEnvPath: noDockerEnv,
  ...
  test('~/.mogoi/daemon with .git is script install', () => {
    const root = join(fakeHome, '.mogoi', 'daemon');
  ...
  test('~/.mogoi/daemon without .git is unknown (half-installed)', () => {
    const root = join(fakeHome, '.mogoi', 'daemon');
    mkdirSync(root, { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
  ...
  test('arbitrary git checkout is dev', () => {
    const root = join(workDir, 'work', 'mogoi');
    mkdirSync(join(root, '.git'), { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
      dockerEnvPath: noDockerEnv,

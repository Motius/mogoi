...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'mogoi-uninstall-test-'));
...
describe('createCleanupPlan — method-specific paths', () => {
  test('script install schedules dataDir for removal', () => {
    const plan = createCleanupPlan(join(fakeHome, '.mogoi', 'daemon'), {
      env: {},
      homeDir: fakeHome,
      detect: fakeDetect('script'),
    });
    expect(plan.method).toBe('script');
    expect(plan.removablePaths).toContain(join(fakeHome, '.mogoi'));
    expect(plan.runBunUninstall).toBe(false);
  });

  test('bun-global install schedules dataDir only; package handled by bun', () => {
    const packageRoot = join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@motius', 'brain');
  ...
  test('dev checkout is never added to removablePaths', () => {
    const devPath = join(workDir, 'work', 'mogoi');
...
describe('createCleanupPlan — env overrides', () => {
  test('honors MOGOI_HOME for data dir', () => {
    const customDataDir = join(workDir, 'custom-mogoi');
    const plan = createCleanupPlan(join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@motius', 'brain'), {
      env: { MOGOI_HOME: customDataDir },
      homeDir: fakeHome,
      detect: fakeDetect('bun-global'),
    });
    expect(plan.dataDir).toBe(customDataDir);
    expect(plan.removablePaths).toContain(customDataDir);
    // Must not include the default ~/.mogoi when MOGOI_HOME is set.
    expect(plan.removablePaths).not.toContain(join(fakeHome, '.mogoi'));
  });

  test('honors BUN_INSTALL for wrapper candidates', () => {
    const customBun = join(workDir, 'custom-bun');
    const wrapperPath = join(customBun, 'bin', 'mogoi');
  ...
  test('default wrapper candidates live under ~/.bun/bin', () => {
    const defaultWrapper = join(fakeHome, '.bun', 'bin', 'mogoi');
...
describe('buildCleanupScript — executes correctly against a fixture tree', () => {
  test('unlinks wrappers, removes dataDir, skips package dir when flag off', async () => {
    const dataDir = join(workDir, 'fake-home', '.mogoi');
    const wrapperPath = join(workDir, 'fake-bun', 'bin', 'mogoi');
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'mogoi-uninstall-exec-'));
  ...
  test('missing wrapper is a soft no-op, not a failure', async () => {
    const dataDir = join(workDir, 'fake-home-2', '.mogoi');
    const missingWrapper = join(workDir, 'does-not-exist', 'mogoi');
    mkdirSync(dataDir, { recursive: true });

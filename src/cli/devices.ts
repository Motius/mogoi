 ...
 *
 *   mogoi enroll <name> [--json]    mint + store an enrollment JWT (upsert by name)
 *   mogoi sidecars list [--json]    list enrolled devices
 *   mogoi revoke <sid> [--json]     revoke a device
 *
  ...
  if (resolved.source === 'fallback' && resolved.warnings.length === 0) {
    io.err(
      'warning: daemon.public_url is not set; tokens will point at ' +
        ...
        'to the brain URL that device can reach, restart Mogoi, and run this command again. ' +
...
export async function cmdEnroll(args: string[], io: CliIo = defaultIo): Promise<number> {
  ...
  if (!name) {
    io.err('usage: mogoi enroll <device-name> [--json] [--rotate]');
    return 2;
  }
...
export async function cmdSidecars(args: string[], io: CliIo = defaultIo): Promise<number> {
  ...
  const json = args.includes('--json');
  if (sub !== 'list') {
    io.err('usage: mogoi sidecars list [--json]');
    return 2;
  }
...
export async function cmdRevoke(args: string[], io: CliIo = defaultIo): Promise<number> {
  ...
  if (!sid) {
    io.err('usage: mogoi revoke <sid> [--json]');
    return 2;
  }

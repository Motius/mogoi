 ...
 * booted out afterwards; it never touches the real `ai.mogoi.daemon`.
 */
...
const LABEL = `ai.mogoi.itest.${process.pid}`;
...
describe.skipIf(!ENABLED)('daemon stop under a real launchd supervisor', () => {
  beforeAll(() => {
    ...
    DATA_DIR = mkdtempSync(join(tmpdir(), 'mogoi-launchd-test-'));

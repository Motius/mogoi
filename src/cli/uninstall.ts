/**
 * `mogoi uninstall` — install-method-aware uninstaller.
 *
...
function resolveDataDir(env: NodeJS.ProcessEnv, home: string): string {
  return env.MOGOI_HOME ? resolve(env.MOGOI_HOME) : join(home, '.mogoi');
}
...
function resolveWrapperCandidates(env: NodeJS.ProcessEnv, home: string): string[] {
  ...
  return ['mogoi', 'mogoi.cmd', 'mogoi.ps1'].map((name) => join(bin, name));
}
...
export function createCleanupPlan(
  ...
  if (method === 'script') {
    // dataDir already covers resolvedPackageRoot at ~/.mogoi/daemon, so only
  ...
  return {
    ...
    // ~/.mogoi-uninstall.log sits next to, not inside, the data dir so it
    // survives the removal and is there for the user to inspect on failure.
    logPath: join(home, '.mogoi-uninstall.log'),
  };
}
...
// must be self-contained — no imports from the MOGOI codebase, since the
...
// Give the parent mogoi CLI 1.5s to fully exit and release the wrapper /
...
if (payload.runBunUninstall) {
  try {
    const r = spawnSync(payload.bunPath, ['uninstall', '-g', '@motius/brain'], {
      stdio: 'pipe',
      env: { ...process.env },
    });
...
// mogoi-uninstall-* directories across repeated uninstall attempts.
try {
...
async function runSideEffectCleanup(plan: CleanupPlan): Promise<boolean> {
  ...
  // A daemon we failed to kill is still writing mogoi.db, the keychain, and
  ...
  if (!stop.stopped) {
    ...
    console.log(c.dim('  Stop it as its owner (or with root), then re-run `mogoi uninstall`.'));
    return false;
  }
...
async function schedulePackageRemoval(plan: CleanupPlan): Promise<void> {
  const tempDir = mkdtempSync(join(tmpdir(), 'mogoi-uninstall-'));
...
function printPlan(plan: CleanupPlan): void {
  ...
  if (plan.runBunUninstall) {
    console.log(c.dim(`  • Run: bun uninstall -g @motius/brain`));
  }
  if (plan.method === 'dev') {
...
function refuseDocker(plan: CleanupPlan): void {
  console.log(c.yellow('\nMOGOI is running inside a Docker container.\n'));
  console.log('  `mogoi uninstall` does not apply to container installs — the');
  console.log('  container is the unit of uninstall.');
  console.log('');
  console.log('  From your host, run:');
  console.log(c.dim('    docker rm -f mogoi'));
  console.log(c.dim('    docker volume rm mogoi-data   # if you want to delete data too'));
  console.log('');

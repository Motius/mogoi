...
function makeTempDir(name: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mogoi-version-test-${name}-`));
...
async function writePackageJson(dir: string, version: string): Promise<void> {
  await Bun.write(join(dir, 'package.json'), JSON.stringify({ name: '@motius/brain', version }, null, 2));
}
...
async function withFakeGit<T>(
  ...
  const previousGitBin = process.env.MOGOI_GIT_BIN;
  process.env.MOGOI_GIT_BIN = join(dir, 'bin', 'git');
  try {
    return await run(dir);
  } finally {
    if (previousGitBin === undefined) {
      delete process.env.MOGOI_GIT_BIN;
    } else {
      process.env.MOGOI_GIT_BIN = previousGitBin;
    }
  }
}
...
describe('CLI version resolver', () => {
  ...
  test('ignores an exact tag that is not a version and uses package.json', async () => {
    // The state that made `mogoi --version` print `vinstaller-v0.1.1`: HEAD
...
if [ "$3" = "describe" ] && [ "$4" = "--tags" ] && [ "$5" = "--always" ]; then
  ...
  test('falls back to package.json when not a git checkout', async () => {
    ...
    // Point MOGOI_GIT_BIN at a non-existent file so rev-parse fails →
    // isGitCheckout returns false and we never call describe.
    const previousGitBin = process.env.MOGOI_GIT_BIN;
    process.env.MOGOI_GIT_BIN = join(dir, 'no-such-git-binary');
    try {
      expect(getInstalledVersion(dir)).toBe('3.2.1');
    } finally {
      if (previousGitBin === undefined) delete process.env.MOGOI_GIT_BIN;
      else process.env.MOGOI_GIT_BIN = previousGitBin;
    }
  });
...
// Run the real `bin/mogoi.ts --version` / `version` paths so the banner
...
describe('CLI --version output', () => {
  ...
  const MOGOI_BIN = join(REPO_ROOT, 'bin', 'mogoi.ts');

  async function runCli(args: string[]): Promise<{ exitCode: number; stdout: string }> {
    const proc = Bun.spawn(['bun', MOGOI_BIN, ...args], {
      stdout: 'pipe',
      stderr: 'pipe',
      cwd: REPO_ROOT,
    });
  ...
  test('`mogoi --version` prints v-prefixed version with no doubling', async () => {
  ...
  test('`mogoi version` matches `--version` output', async () => {
  ...
  test('`mogoi -v` matches `--version` output', async () => {
  ...
  test('`mogoi help` banner contains exactly one v-prefixed version (no vv)', async () => {

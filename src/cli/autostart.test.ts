...
describe('scheduleSystemdRestart', () => {
  test('uses --no-block so caller returns before systemd cycles the unit', () => {
    ...
    expect(calls[0]).toEqual(['systemctl', '--user', '--no-block', 'restart', 'mogoi.service']);
  });
...
//
// The daemon resolves its lock AND its logs through MOGOI_HOME. A service
// definition that doesn't export the var launches a daemon under ~/.mogoi
// while every CLI tool in the same shell looks under $MOGOI_HOME.
describe('service definitions propagate MOGOI_HOME', () => {
  function withMogoiHome<T>(value: string | undefined, fn: () => T): T {
    const prev = process.env.MOGOI_HOME;
    if (value === undefined) delete process.env.MOGOI_HOME;
    else process.env.MOGOI_HOME = value;
    try { return fn(); } finally {
      if (prev === undefined) delete process.env.MOGOI_HOME;
      else process.env.MOGOI_HOME = prev;
    }
  }

  test('systemd unit omits MOGOI_HOME when unset', () => {
    const unit = withMogoiHome(undefined, generateSystemdUnit);
    expect(unit).not.toContain('MOGOI_HOME');
    expect(unit).toContain('[Install]');
    expect(unit).toContain('Environment=HOME=');
  });

  test('systemd unit exports MOGOI_HOME when set', () => {
    const unit = withMogoiHome('/srv/tenant7', generateSystemdUnit);
    expect(unit).toContain('Environment="MOGOI_HOME=/srv/tenant7"');
    // The section header must not get swallowed by the injected line.
    expect(unit).toContain('[Install]');
    expect(unit).toContain('WantedBy=default.target');
  });

  test('launchd plist logs under MOGOI_HOME and exports it', () => {
    const plist = withMogoiHome('/srv/tenant7', generateLaunchdPlist);
    expect(plist).toContain('<string>/srv/tenant7/logs/mogoi.log</string>');
    expect(plist).toContain('<key>MOGOI_HOME</key>');
    expect(plist).toContain('<string>/srv/tenant7</string>');
  });

  test('launchd plist falls back to ~/.mogoi/logs with no MOGOI_HOME', () => {
    const plist = withMogoiHome(undefined, generateLaunchdPlist);
    expect(plist).toContain('/.mogoi/logs/mogoi.log');
    expect(plist).not.toContain('MOGOI_HOME');
  });
  ...
  test('systemd quotes a data root containing spaces and escapes %', () => {
    const unit = withMogoiHome('/srv/my data/100%', generateSystemdUnit);
    expect(unit).toContain('Environment="MOGOI_HOME=/srv/my data/100%%"');
  });

  // An unescaped & or < yields a plist launchctl refuses to load, so autostart
  // silently does nothing.
  test('launchd plist XML-escapes the data root', () => {
    const plist = withMogoiHome('/srv/a&b/<c>', generateLaunchdPlist);
  ...
  test.skipIf(!SYSTEMD_ANALYZE)('systemd-analyze accepts the generated unit', () => {
    const unit = withMogoiHome('/srv/my data/100%', generateSystemdUnit);
    const path = join(mkdtempSync(join(tmpdir(), 'mogoi-unit-')), 'mogoi-verify.service');
  ...
  const PLIST_LINT = IS_MAC ? Bun.which('plutil') : Bun.which('xmllint');
  test.skipIf(!PLIST_LINT)('the generated plist parses with a hostile data root', () => {
    const plist = withMogoiHome('/srv/a&b/<c>/"d"', generateLaunchdPlist);
    const path = join(mkdtempSync(join(tmpdir(), 'mogoi-plist-')), 'mogoi.plist');

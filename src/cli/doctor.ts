/**
 * M.O.G.O.I. Doctor — Environment Diagnostics
 *
...
const MOGOI_DIR = join(homedir(), '.mogoi');
const CONFIG_PATH = join(MOGOI_DIR, 'config.yaml');
...
export async function runDoctor(): Promise<void> {
  ...
  if (existsSync(MOGOI_DIR)) {
    results.push({ name: 'Data Directory', status: 'ok', message: MOGOI_DIR });
  } else {
    results.push({ name: 'Data Directory', status: 'warn', message: `${MOGOI_DIR} not found. Run: mogoi start (then finish setup at http://localhost:1846)` });
  }
  ...
  if (existsSync(CONFIG_PATH)) {
    ...
    results.push({ name: 'Config File', status: 'fail', message: 'Not found. Run: mogoi start (then finish setup at http://localhost:1846)' });
  }
  ...
  if (llmConfig) {
    if (llmProviderNames.length === 0) {
      results.push({ name: 'LLM Provider', status: 'fail', message: 'No providers configured. Add one in Settings > LLM (http://localhost:1846).' });
    } else {
      ...
      const { effectiveLlmForBinding } = await import('../daemon/motius-ai.ts');
  ...
  const port = config?.daemon?.port ?? 1846;
  try {
  ...
  if (config?.stt?.provider) {
    ...
    const { hasMotiusAIAi } = await import('../daemon/motius-ai.ts');
    const hasKey = sttProv === 'ollama' || sttProv === 'local'
      ...
      // Hosted STT rides the system-owned motius_ai credentials, not a
      // cfg.stt key — "API key missing" here was a false alarm on every
      // working hosted install.
      || (sttProv === 'motius' && hasMotiusAIAi(config));
    results.push({
      name: 'STT',
  ...
  if (failCount > 0) {
    console.log(c.red('\nSome checks failed. Run "mogoi start" and finish setup at http://localhost:1846 to fix configuration.\n'));
  } else if (warnCount > 0) {
    console.log(c.yellow('\nAll critical checks passed, but some optional features need setup.\n'));
  } else {
    console.log(c.green('\nAll checks passed! MOGOI is ready.\n'));
  }

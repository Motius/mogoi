...
import { DEFAULT_CONFIG, type MogoiConfig, type VoiceConfig } from '../config/types.ts';
...
import { effectiveSttForBinding, effectiveTtsForBinding, realtimeEnablement, resetRealtimeVaultWarningForTest } from './motius-ai.ts';

/**
 * Route-level regressions for the voice-config persistence discipline.
 *
 * The hosted "MotiusAI AI" STT/TTS defaults key off SILENCE: a stored
 ...
 *
 * Deliberately NOT the real `llm.motius.host`: reading the realtime block
...
function hostedConfig(): MogoiConfig {
  ...
  config.motius_ai = { base_url: 'https://llm.invalid/v1', api_key: 'sk-uj-abc123' };
  return config;
}

function makeCtx(config: MogoiConfig): ApiContext {
  return {
...
describe('voice config routes: persistence stays silence-preserving', () => {
  ...
  beforeEach(() => {
    ...
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-api-voice-'));
  ...
  test('POST /api/config/tts {enabled:true} leaves the stored row provider-free (hosted default survives)', async () => {
    ...
    expect(effectiveTtsForBinding(config)?.provider).toBe('motius');
  });
  ...
  test('POST /api/config/tts rejects provider motius on a self-hosted install', async () => {
    const config = structuredClone(DEFAULT_CONFIG); // no motius_ai block
    const routes = createApiRoutes(makeCtx(config));
    const res = await post(getHandler(routes, '/api/config/tts', 'POST'), '/api/config/tts', { provider: 'motius' }) as Response;
  ...
  // The Settings reset must restore SILENCE, never write 'motius' — a
  // recorded choice pins the account and the plan default stops applying.
  test('POST /api/config/tts {provider:null} deletes the choice rather than recording motius', async () => {
    const config = hostedConfig();
    const routes = createApiRoutes(makeCtx(config));
    ...
    // The explicit "cleared" sentinel — never 'motius' (a recorded choice
  ...
  test('POST /api/config/stt {provider:null} likewise restores the plan default', async () => {
    ...
    expect(effectiveSttForBinding(config)?.provider).toBe('motius');
  ...
  test('GET /api/config/tts reports the binding-view provider and availability, no key material', async () => {
    ...
    expect(body.provider).toBe('motius');
    expect(body.motius_available).toBe(true);
...
describe('voice config routes: a save must not silently decline realtime', () => {
  ...
  beforeEach(() => {
    prevSecretsDir = process.env.MOGOI_SECRETS_DIR;
    prevEnv = process.env.MOGOI_REALTIME_VOICE;
    delete process.env.MOGOI_REALTIME_VOICE;
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-api-rt-'));
    process.env.MOGOI_SECRETS_DIR = secretsDir;
    closeDb();
    initDatabase(':memory:');
    clearRealtimeGateCache();
  ...
  afterEach(() => {
    ...
    if (prevEnv === undefined) delete process.env.MOGOI_REALTIME_VOICE;
    else process.env.MOGOI_REALTIME_VOICE = prevEnv;
    rmSync(secretsDir, { recursive: true, force: true });
  });
...
describe('a corrupt voice row is not an answer', () => {
  ...
  beforeEach(() => {
    ...
    prevEnv = process.env.MOGOI_REALTIME_VOICE;
    delete process.env.MOGOI_REALTIME_VOICE;
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-rt-corrupt-'));
    process.env.MOGOI_SECRETS_DIR = secretsDir;
    closeDb();
    initDatabase(':memory:');
...
describe('GET /api/config/voice tells the truth about who pays', () => {
  ...
  beforeEach(() => {
    ...
    prevEnv = process.env.MOGOI_REALTIME_VOICE;
    delete process.env.MOGOI_REALTIME_VOICE;
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-rt-served-'));
    process.env.MOGOI_SECRETS_DIR = secretsDir;
    closeDb();
    initDatabase(':memory:');
    clearRealtimeGateCache();
  ...
  const read = async (config: MogoiConfig) => {
    const routes = createApiRoutes(makeCtx(config));
  ...
  test('a hosted tenant with a BYO OpenAI key is NOT told they pay for it', async () => {
    ...
    const config = hostedConfig();
    config.llm = { providers: { openai: { api_key: 'sk-their-own' } } } as MogoiConfig['llm'];
...
describe('replacing an unreadable settings row says so', () => {
  ...
  beforeEach(() => {
    prevSecretsDir = process.env.MOGOI_SECRETS_DIR;
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-rt-warn-'));
    process.env.MOGOI_SECRETS_DIR = secretsDir;
    closeDb();
    initDatabase(':memory:');
    warnings = [];

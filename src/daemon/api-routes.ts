...
import { hasMotiusAIAi, isHostedInstall, effectiveSttForBinding, effectiveTtsForBinding, realtimeEnablement, motiusVoiceCredentials } from './motius-ai.ts';
...
export type ApiContext = {
   ...
   * completion, the dashboard renders a "Restart Mogoi" banner when
   ...
   * show the "Restart Mogoi" fallback banner.
   */
...
let CORS: Record<string, string> = {
  'Access-Control-Allow-Origin': 'http://localhost:1846',
...
export function createApiRoutes(ctx: ApiContext): Record<string, unknown> {
  ...
  return {
    ...
    // Pebble long-answer panel — when a MOGOI response overflows the
    ...
    // after a Mogoi update or when swapping LLM providers.
    ...
    '/api/onboarding/reset': {
      POST: async (req: Request) => {
        try {
          ...
          const clientCacheKeys = ['mogoi:notif-read', 'mogoi:palette-recent'];
          if (scope === 'all') {
            clientCacheKeys.push('mogoi:v2:workspaces-ui');
            clientCacheKeys.push('mogoi:room-layout');
          }
    ...
    '/api/onboarding/skip': {
      POST: async () => {
        try {
          ...
          // Best-effort service start so the "Restart Mogoi" banner
    ...
    '/api/onboarding/setup': {
      POST: async (req: Request) => {
        try {
          ...
          // Edge" and never get the included MotiusAI AI default.
          ...
          //    "Welcome to Mogoi" reply is spoken immediately.
          if (body.tts) {
            try {
              if (ctx.config.tts && ctx.wsService) {
                ...
                const provider = createTTSProvider(ttsBinding, motiusVoiceCredentials(ctx.config));
          ...
          return json({
            ...
            dropped,
            message: 'Setup complete. Mogoi is ready.',
          });
        } catch (err) {
          return errorFromException(err);
        }
        ...
        return json({
          ...
          llm: {
            ...
            providers: Object.keys(config.llm.providers ?? {}).filter(
              (name) => name !== 'motius_ai' || !hasMotiusAIAi(config),
            ),
    ...
    '/api/system/autostart/restart': {
      POST: () => {
        ...
        if (!isAutostartInstalled()) {
          return error('MOGOI keepalive mode is not installed yet.', 400);
        }
        ...
        return json({
          ok: true,
          message: `Restarting the MOGOI 24/7 ${getAutostartName()} service.`,
        });
      },
    },
    ...
    // Live MotiusAI AI catalog: the uj-* aliases THIS account's key may
    ...
    '/api/billing': {
      GET: async (req: Request) => {
        ...
        if (!cfg) {
          const { isHostedInstall } = await import('./motius-ai.ts');
    ...
    '/api/config/llm/motius/models': {
      GET: async () => {
        const { hasMotiusAIAi } = await import('./motius-ai.ts');
        if (!hasMotiusAIAi(ctx.config)) {
          return error('MotiusAI AI is only available on hosted installs.', 503);
        }
        try {
          const { MotiusAIAIProvider } = await import('../llm/motius.ts');
          const { noteHostedCatalog } = await import('./motius-ai.ts');
          const block = ctx.config.motius_ai!;
          const provider = new MotiusAIAIProvider(block.base_url!.trim(), block.api_key!.trim());
          ...
          console.warn(
            '[LLM] MotiusAI AI catalog fetch failed:',
            err instanceof Error ? err.message : err,
          );
          return json({ ok: false, error: 'MotiusAI AI catalog unavailable', models: [] });
        }
      },
    },
    ...
    '/api/content/:id/attachments': {
      ...
      POST: async (req: Request & { params: { id: string } }) => {
        try {
          ...
          // Save file to ~/.mogoi/content/<id>/
          const baseDir = path.join(os.homedir(), '.mogoi', 'content', contentId);
    ...
    '/api/content/files/:contentId/:filename': {
      GET: async (req: Request & { params: { contentId: string; filename: string } }) => {
        ...
        const baseDir = path.join(os.homedir(), '.mogoi', 'content');
    ...
    '/api/auth/google/callback': {
      GET: async (req: Request) => {
        ...
        if (!pendingFlow) {
          ...
          return new Response(
            `<html><body><h1>Authorization Failed</h1><p>${reason}</p><p>Start Google authorization again from the Mogoi dashboard.</p></body></html>`,
        ...
        try {
          ...
          return new Response(
            `<html><body style="font-family:system-ui;text-align:center;padding:60px">
              <h1>MOGOI Google Authorization Complete!</h1>
      ...
      POST: async (req: Request) => {
        try {
          ...
          if (classifyGoogle(ctx.config).mode === 'managed' || ctx.config.google?.refresh_url) {
            return error(
              'This instance is managed by motius — its Google credentials are held by the control plane and cannot be set here.',
              409,
            );
          }
    ...
    '/api/auth/google/init': {
      POST: async () => {
        ...
        if (googleConfig?.connect_url) {
          return error(
            `This instance is managed by motius — connect Google from ${googleConfig.connect_url}`,
            409,
          );
        }
        if (!googleConfig?.client_id || !googleConfig?.client_secret) {
    ...
    '/api/auth/google/disconnect': {
      POST: async () => {
        try {
          const tokensPath = path.join(os.homedir(), '.mogoi', 'google-tokens.json');
    ...
    '/api/channels/status': {
      GET: () => {
        ...
        // provider while transcription runs happily on 'motius' — the raw
    ...
    '/api/config/channels': {
      ...
      POST: async (req: Request) => {
        try {
          ...
          const merged: NonNullable<MogoiConfig['channels']> = { ...ctx.config.channels };
    ...
    '/api/config/stt': {
      GET: () => {
        ...
        // user never chose, it reads 'motius' (what actually transcribes)
        ...
        return json({
          ...
          motius_available: hasMotiusAIAi(ctx.config),
      ...
      POST: async (req: Request) => {
        try {
          ...
          // Writing 'motius' instead would record a choice and pin them.
          ...
          // 'motius' on a self-hosted install, where createSTTProvider can
          ...
          // line to show for it.
          const STT_PROVIDERS = ['openai', 'groq', 'local', 'sarvam', 'motius'];
          if (body.provider !== undefined) {
            ...
            if (body.provider === 'motius' && !hasMotiusAIAi(ctx.config)) {
              return json({ ok: false, message: 'MotiusAI AI transcription is only available on hosted installs.' }, 400);
            }
          }
          // The hosted credentials never live in cfg.stt — a 'motius'
    ...
    '/api/config/tts': {
      GET: () => {
        ...
        // (hosted installs with no recorded choice read 'motius'), the
      ...
      POST: async (req: Request) => {
        try {
          ...
          // (or 'motius' on a self-hosted install, where no hosted
          ...
          // an ok:true toast.
          if (body.provider !== undefined) {
            const VALID_TTS_PROVIDERS = ['edge', 'elevenlabs', 'sarvam', 'motius'];
            ...
            if (body.provider === 'motius' && !hasMotiusAIAi(ctx.config)) {
              return json({ ok: false, error: 'The MotiusAI AI voice is only available on hosted installs; pick edge, elevenlabs or sarvam.' }, 400);
            }
          }
      ...
      GET: () => {
        ...
        return json({
          ...
          realtime: {
            ...
            hosted: hasMotiusAIAi(ctx.config),
    ...
    '/api/tts/preview': {
      POST: async (req: Request) => {
        try {
          ...
          const text = (typeof body.text === 'string' && body.text.trim() ? body.text.trim() : "Hi, I'm Mogoi. This is how I'll sound.").slice(0, 280);
          const cfg: Record<string, unknown> = {
            enabled: true,
            provider: body.provider === 'elevenlabs' || body.provider === 'motius' ? body.provider : 'edge',
          };
          if (body.provider === 'elevenlabs') {
          ...
          } else if (body.provider === 'motius') {
            // Hosted preview: no key in the body — the factory gets the
            // system-owned proxy credentials as its separate argument below.
            if (!hasMotiusAIAi(ctx.config)) return error('MotiusAI AI is not available on this install.', 400);
            if (typeof body.voice === 'string' && body.voice) {
              ...
              if (/Neural$/i.test(body.voice)) {
                return error(`"${body.voice}" is an Edge TTS voice — MotiusAI AI uses OpenAI-style voices (e.g. alloy).`, 400);
              }
          ...
          const { createTTSProvider } = await import('../comms/voice.ts');
          const provider = createTTSProvider(cfg as never, motiusVoiceCredentials(ctx.config));
     ...
     * "grant Mogoi email access" and have it persist.
     *
    ...
    '/api/awareness/captures/:id/image': {
      GET: async (req: Request & { params: { id: string } }) => {
        ...
        if (!capture.sidecar_id) {
          const mogoiDir = path.join(os.homedir(), '.mogoi');
          if (!isWithin(path.resolve(capture.image_path), path.resolve(mogoiDir))) {
     ...
     * shows it once and tells the user to run `mogoi --token <jwt>` on the
     * target machine. That is one self-hosted way to add a device -- the other,
     * and the one docs/SELF_HOSTING.md calls primary, is `mogoi enroll <name>`
     * on the brain host. Refusing here therefore never strands anyone.
     *
     * On a hosted install it is neither needed nor wanted:
     *
     *  - It is not the flow. A hosted user adds a device by installing Mogoi
     *    there and signing in; the control plane runs `mogoi enroll` over SSH
     ...
     *
     * Gated on `isHostedInstall`, NOT on `hasMotiusAIAi` alone. The feature
          ...
          if (isHostedInstall(ctx.config)) {
            ...
            return error(
              'Adding a device from the dashboard is disabled on hosted installs. '
                + 'Install Mogoi on the new device and sign in there with your MotiusAI '
                + 'account -- it enrols itself. On a self-hosted brain, run: mogoi enroll <name>',
              403,
            );
          }
        ...
        try {
          ...
          if (project?.githubUrl) {
            const meta = require('node:fs').readFileSync(
              require('node:path').join(projectPath, '.mogoi-project.json'), 'utf-8'
            );
            const parsed = JSON.parse(meta);
            if (parsed.github) {
              parsed.github.lastPushedAt = Date.now();
              await Bun.write(require('node:path').join(projectPath, '.mogoi-project.json'), JSON.stringify(parsed, null, 2));
            }
          }

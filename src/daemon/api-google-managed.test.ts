 ...
 */
const CONNECT_URL = 'https://app.motius.dev/account';
 ...
 */
const MANAGED = {
  refresh_url: 'https://app.motius.dev/api/integrations/google/refresh',
  instance_id: 'inst-1',
...
function routes(google: Record<string, string> | undefined, googleTokensPath?: string) {
  return createApiRoutes({
    ...
    googleTokensPath,
    config: {
      daemon: { port: 1846, data_dir: '/tmp/mogoi', db_path: '/tmp/mogoi/mogoi.db' },
      ...(google ? { google } : {}),
    } as MogoiConfig,
...
describe('managed (hosted) Google mode', () => {
  it('status reports managed + where to connect', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'mogoi-google-status-'));
  ...
  it('a self-hosted instance is NOT managed', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'mogoi-google-self-'));

...
import { DEFAULT_CONFIG, type MogoiConfig } from '../config/types.ts';
...
import {
  ...
  hasMotiusAIAi,
  motiusVoiceCredentials,
  USEMOGOI_PROVIDER_NAME,
} from './motius-ai.ts';
import { mergeLLMSettingsIntoConfig, saveLLMSettings } from './llm-settings.ts';

const base = (): MogoiConfig => structuredClone(DEFAULT_CONFIG);

const hosted = (): MogoiConfig => {
  const config = base();
  config.motius_ai = { base_url: 'https://llm.motius.host', api_key: 'sk-uj-abc123' };
  return config;
};

describe('applyMotiusAIAi (provider injection only)', () => {
  test('no-op on self-hosted installs (block absent or incomplete)', () => {
    const config = base();
    applyMotiusAIAi(config);
    expect(config.llm.providers?.[USEMOGOI_PROVIDER_NAME]).toBeUndefined();
    expect(hasMotiusAIAi(config)).toBe(false);

    const partial = base();
    partial.motius_ai = { base_url: 'https://llm.motius.host' }; // no key
    applyMotiusAIAi(partial);
    expect(partial.llm.providers?.[USEMOGOI_PROVIDER_NAME]).toBeUndefined();
  });

  test('injects the provider and NEVER touches config.llm.tiers', () => {
    const config = hosted();
    applyMotiusAIAi(config);
    expect(config.llm.providers![USEMOGOI_PROVIDER_NAME]).toEqual({
      kind: 'motius_ai',
  ...
  test('a malformed block (unquoted YAML scalars) is ignored, never throws', () => {
    const config = base();
    config.motius_ai = { base_url: 'https://llm.motius.host', api_key: 1234567890 } as never;
    expect(hasMotiusAIAi(config)).toBe(false);
    expect(() => applyMotiusAIAi(config)).not.toThrow();
    expect(config.llm.providers?.[USEMOGOI_PROVIDER_NAME]).toBeUndefined();
  });

  test('base_url is normalized: trailing slashes never reach the provider entry', () => {
    const config = base();
    config.motius_ai = { base_url: 'https://llm.motius.host/v1/', api_key: 'sk-uj-abc123' };
    applyMotiusAIAi(config);
    expect(config.llm.providers![USEMOGOI_PROVIDER_NAME]!.base_url).toBe(
      'https://llm.motius.host/v1',
    );
  });
  ...
  test('a DB row squatting on the reserved name is overwritten, not merged', () => {
    ...
    config.llm.providers = {
      [USEMOGOI_PROVIDER_NAME]: { kind: 'openai', base_url: 'https://evil.example' },
    };
    applyMotiusAIAi(config);
    expect(config.llm.providers[USEMOGOI_PROVIDER_NAME]).toEqual({
      kind: 'motius_ai',
  ...
  test('the provisioner prompt_cache opt-in carries through; junk reads as OFF', () => {
    ...
    optedIn.motius_ai!.prompt_cache = true;
    applyMotiusAIAi(optedIn);
    expect(optedIn.llm.providers![USEMOGOI_PROVIDER_NAME]!.prompt_cache).toBe(true);

    const junk = hosted();
    (junk.motius_ai as Record<string, unknown>).prompt_cache = 'yes'; // YAML-typed junk
    applyMotiusAIAi(junk);
    expect(junk.llm.providers![USEMOGOI_PROVIDER_NAME]!.prompt_cache).toBe(false);
  });

  test('idempotent: applying twice equals applying once', () => {
    const once = hosted();
    applyMotiusAIAi(once);
    const twice = hosted();
    applyMotiusAIAi(twice);
    applyMotiusAIAi(twice);
  ...
  test('testLLMProvider refuses the reserved name (key-exfiltration guard)', async () => {
    ...
    const result = await testLLMProvider(
      { name: USEMOGOI_PROVIDER_NAME, kind: 'openai_compatible', base_url: 'https://evil.example' },
      config,
    );
...
describe('effectiveLlmForBinding (per-slot: explicit ref → llm.default → plan alias)', () => {
  ...
  test('hosted, all silent: every slot resolves to its uj-* alias', () => {
    ...
    expect(effectiveLlmForBinding(config).tiers).toEqual({
      conversation: 'motius_ai:uj-chat',
      low: 'motius_ai:uj-low',
      medium: 'motius_ai:uj-medium',
      high: 'motius_ai:uj-high',
    });
  ...
  test("a user's explicit tier choice wins for THAT slot; the rest default", () => {
    ...
    expect(effective.tiers!.low).toBe('motius_ai:uj-low');
  ...
  test('llm.default narrows the fallback for unset slots instead of disabling the others', () => {
    ...
    config.llm.tiers = { high: 'motius_ai:uj-high' };
    const effective = effectiveLlmForBinding(config);
    expect(effective.tiers!.high).toBe('motius_ai:uj-high'); // explicit wins
...
describe('DB round-trips (restart survival + fill never persists)', () => {
  ...
  // never read from — or write to — the machine's real ~/.mogoi keychain.
  let secretsDir: string;
  let prevSecretsDir: string | undefined;

  beforeEach(() => {
    prevSecretsDir = process.env.MOGOI_SECRETS_DIR;
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-motius-ai-'));
  ...
  test('persisted motius_ai:* refs survive a boot merge (never pruned as orphans)', () => {
    initDatabase(':memory:');
    setSetting('llm.providers', '{}');
    setSetting('llm.default', 'motius_ai:uj-high');
    setSetting('llm.tiers.high', 'motius_ai:uj-high');

    const config = hosted();
    mergeLLMSettingsIntoConfig(config);

    // In memory AND in the settings table: the refs must survive the restart.
    expect(config.llm.default).toBe('motius_ai:uj-high');
    expect(config.llm.tiers!.high).toBe('motius_ai:uj-high');
    expect(getSetting('llm.default')).toBe('motius_ai:uj-high');
    expect(getSetting('llm.tiers.high')).toBe('motius_ai:uj-high');
  });

  test('un-hosting still prunes the now-dangling motius_ai refs', () => {
    initDatabase(':memory:');
    setSetting('llm.providers', '{}');
    setSetting('llm.tiers.high', 'motius_ai:uj-high');

    const config = base(); // no motius_ai block
    mergeLLMSettingsIntoConfig(config);
  ...
  test('clearing a tier persists the clear (no same-request refill)', () => {
    ...
    saveLLMSettings(config, { tiers: { high: 'motius_ai:uj-high' } });
    expect(getSetting('llm.tiers.high')).toBe('motius_ai:uj-high');
    ...
    expect(effectiveLlmForBinding(config).tiers!.high).toBe('motius_ai:uj-high');
  });
  ...
  test('editing/deleting the managed provider is refused atomically on hosted installs', () => {
    ...
    // old silent skip destroyed legacy configs named motius_ai).
    const selfHosted = base();
    selfHosted.llm.providers = {};
    expect(() =>
      saveLLMSettings(selfHosted, { providers: { [USEMOGOI_PROVIDER_NAME]: { kind: 'openai' } } }),
    ).not.toThrow();
    expect(selfHosted.llm.providers[USEMOGOI_PROVIDER_NAME]).toEqual({ kind: 'openai' });
  });
});

describe('hasMotiusAIAi: malformed blocks read as NOT hosted, never fatal', () => {
  const withBlock = (block: unknown): MogoiConfig => {
    const config = structuredClone(DEFAULT_CONFIG);
    (config as Record<string, unknown>).motius_ai = block;
    return config;
  };
  ...
  test('a numeric api_key or base_url does not throw', () => {
    expect(() => hasMotiusAIAi(withBlock({ base_url: 'https://x', api_key: 1234567890 }))).not.toThrow();
    expect(hasMotiusAIAi(withBlock({ base_url: 'https://x', api_key: 1234567890 }))).toBe(false);
    expect(hasMotiusAIAi(withBlock({ base_url: 42, api_key: 'sk-uj-abc' }))).toBe(false);
  });
  ...
  test('other malformed shapes are also non-fatal and non-hosted', () => {
    for (const block of [null, undefined, 'a string', [], { base_url: 'https://x' }, { api_key: 'sk-uj-a' },
      ...
      expect(() => hasMotiusAIAi(withBlock(block))).not.toThrow();
      expect(hasMotiusAIAi(withBlock(block))).toBe(false);
    }
  });

  test('a well-formed block still reads as hosted', () => {
    expect(hasMotiusAIAi(withBlock({ base_url: 'https://llm.motius.host', api_key: 'sk-uj-abc' }))).toBe(true);
  });
});

describe('motiusVoiceCredentials (the SEPARATE credential channel)', () => {
  test('null on self-hosted installs', () => {
    expect(motiusVoiceCredentials(base())).toBeNull();
    const partial = base();
    partial.motius_ai = { base_url: 'https://llm.motius.host' }; // no key
    expect(motiusVoiceCredentials(partial)).toBeNull();
  });

  test('returns trimmed proxy origin + key when hosted', () => {
    const config = hosted();
    config.motius_ai = { base_url: ' https://llm.motius.host ', api_key: ' sk-uj-abc123 ' };
    expect(motiusVoiceCredentials(config)).toEqual({
      baseUrl: 'https://llm.motius.host',
      apiKey: 'sk-uj-abc123',
    });
  });
});
...
describe('effectiveSttForBinding (hosted STT default, persistence-pure)', () => {
  ...
  test('hosted + no stored stt row: binding view says motius, config is NOT mutated', () => {
    const config = hosted();
    const view = effectiveSttForBinding(config, noRow);
    expect(view?.provider).toBe('motius');
  ...
  test('hosted + stored row with only unknown fields: still silent, defaults to motius', () => {
    const config = hosted();
  ...
  test('the view never carries credentials: only the provider string differs', () => {
    ...
    expect(view).toEqual({ ...config.stt!, provider: 'motius' });
  });
});
...
describe('effectiveTtsForBinding (hosted TTS default, explicit Edge respected)', () => {
  ...
  test('hosted + no stored tts row: binding view says motius, enabled and config untouched', () => {
    const config = hosted();
  ...
  test('the view never carries credentials', () => {
    ...
    const view = effectiveTtsForBinding(config, noRow)!;
    expect(JSON.stringify(view)).not.toContain('sk-uj-abc123');
    expect(view).toEqual({ ...config.tts!, provider: 'motius' });
  });
});

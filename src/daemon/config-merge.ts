...
/**
 * Warm-engine idle TTL: MOGOI_ENGINE_IDLE_TTL_MS env var wins over the
...
export function resolveEngineIdleTtlMs(
  ...
  const envRaw = env['MOGOI_ENGINE_IDLE_TTL_MS'];
  const raw = envRaw !== undefined ? Number(envRaw) : configured;
  if (raw === undefined) return undefined;
  if (!Number.isFinite(raw) || raw <= 0) {
    console.warn(
      `[Daemon] Ignoring engine idle TTL ${JSON.stringify(envRaw ?? configured)} (${envRaw !== undefined ? 'MOGOI_ENGINE_IDLE_TTL_MS' : 'workflows.engineIdleTtlMs'}): must be a positive number of ms; using default`,
    );
    return undefined;
  }
  return raw;

  ...
  private async handleChat(msg: WSMessage, ws?: ServerWebSocket<unknown>): Promise<WSMessage | void> {
    ...
    if (this.isSetupMode()) {
      return {
        ...
        payload: {
          code: 'setup_required',
          message: 'Finish first-run setup before chatting with Mogoi.',
        },
        id: msg.id,
        timestamp: Date.now(),
      };
...
1. **Explicit existing-project name.** If the user named an existing project (e.g. "in mogoi-landing", "edit the SP500 calculator"), use that one.
2. **Name keyword match.** If the user did NOT ask for a new project AND did NOT name one explicitly, but their request shares a meaningful keyword with an existing project's name (request mentions "landing page" + "mogoi-landing" exists → use mogoi-landing; request mentions "calculator" + "SP500 ROI Calculator" exists → use that), pick that project.
...
- Do NOT fabricate file paths, line counts, or "your file is now at X" descriptions without a successful site_write_file call backing them.`;
    ...
    try {
      ...
      const speakNextSentence = async () => {
        ...
        // sentence contains "Mogoi" so it can suppress the wake
        ...
        // A spoken stop phrase ("say 'Mogoi, stop'…") would echo through the
        ...
        if (!ttsStartSent) {
          ...
          // Subsequent sentence in the same turn that contains "Mogoi" —
          // notify so the UI can flip suppression on mid-turn. (We never
          // un-suppress mid-turn because earlier audio with "Mogoi" may
  ...
  private async tryStartRealtimeVoice(ws: ServerWebSocket<unknown>, mode?: 'pcm' | 'wav'): Promise<boolean> {
    ...
    this.realtimeSessions.set(ws, {
      ...
      hosted: resolved.provider === 'motius_ai',
  ...
  private getRealtimeBudget(): RealtimeBudgetTracker {
    if (!this.realtimeBudget) {
      const dataDir = this.agentService.getConfig().daemon?.data_dir || join(homedir(), '.mogoi');
  ...
  private async processVoiceTranscript(
    ...
    // before transcript echo/classification so "Mogoi stop" never produces a
...
function ackForRoomAction(ra: { room: string; action: string; args?: Record<string, unknown> }): string {
  ...
  switch (ra.action) {
    ...
    case 'restart_daemon':
      return `Restarting Mogoi. The dashboard will reconnect in a few seconds.`;

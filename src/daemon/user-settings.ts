 ...
 * installs default silent users to the included MotiusAI AI voice stack).
 *
...
export function persistUserPatch(section: 'stt' | 'tts' | 'voice', patch: Record<string, unknown>): void {
  ...
  if (section === 'stt') {
    ...
    // realtime ON for a silent user (motius-ai.ts realtimeEnablement).
...
export function persistPlainUserPatch<K extends PlainPatchSection>(
  section: K,
  patch: Partial<NonNullable<MogoiConfig[K]>>,
): void {
  const stored = loadUserSection(section);
  const usable = typeof stored === 'object' && stored !== null && !Array.isArray(stored);
  const base = usable ? (stored as Record<string, unknown>) : {};
  saveUserSection(section, { ...base, ...patch } as MogoiConfig[K]);
}
 ...
 * stack). Deliberately a delete rather than a write of 'motius': recording
 ...
 */
export function mergeUserSettingsIntoConfig(config: MogoiConfig): void {
...
export function saveGoogleSettings(value: MogoiConfig['google']): void {
...
function googleIsFileProvided(google: MogoiConfig['google']): boolean {
...
function mergeGoogleSettingsIntoConfig(config: MogoiConfig): void {

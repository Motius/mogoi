...
describe('settings-reload', () => {
  ...
  beforeEach(() => {
    ...
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-settings-reload-'));
  ...
  test('reloadAll notices the Google TOKENS FILE, not just the config section', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'mogoi-google-home-'));
    const tokens = join(home, '.mogoi', 'google-tokens.json');
    mkdirSync(join(home, '.mogoi'), { recursive: true });
    try {
      const config = freshConfig();

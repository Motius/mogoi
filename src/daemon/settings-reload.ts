 ...
 * into the /v1/mogoi/events/poll buffer.
 *
 * Rules for appliers:
...
import { USER_OWNED_SECTIONS, type MogoiConfig, type UserOwnedSection } from '../config/types.ts';
...
export type SectionApplier = (config: MogoiConfig) => void | Promise<void>;
...
export class SettingsReloadCoordinator {
  private readonly config: MogoiConfig;
   ...
   * sections (e.g. the motius_ai block) before the DB merge. Injected by
   * the daemon so tests stay hermetic (no config.yaml reads). */
  private readonly reloadSystemSections: ((config: MogoiConfig) => Promise<void>) | null;
  ...
  constructor(
    ...
    opts?: {
      ...
      reloadSystemSections?: (config: MogoiConfig) => Promise<void>;
    },
  ) {
    this.config = config;
   ...
   * SYSTEM-owned motius_ai block that no section applier watches) fire on
  ...
  reloadAll(): Promise<ReloadResult> {
    return this.enqueue(async () => {
      ...
      // provisioner key rotation (or removal of the motius_ai block)
      ...
      // sections, then env overrides so a MOGOI_* pin still wins.
      mergeLLMSettingsIntoConfig(this.config);
      mergeUserSettingsIntoConfig(this.config);
      applyEnvOverrides(this.config);

      // A reload is the moment a re-provisioned motius_ai block lands, so

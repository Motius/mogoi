...
describe('runInterviewTurn — happy path', () => {
  ...
  test('subsequent turns append the user message and return the assistant reply', async () => {
    ...
    const secondLLM = fakeLLM([textResponse('Got it — and what brings you to Mogoi?')]);
    const result = await runInterviewTurn(session, secondLLM, "I'm a software engineer");

    expect(result.assistantText).toBe('Got it — and what brings you to Mogoi?');
  ...
  test('wrap_interview tool call ends the interview with the agent\'s farewell', async () => {
      ...
      toolResponse('', [
        {
          id: 'call_wrap',
          name: 'wrap_interview',
          arguments: { farewell: 'All set — welcome to Mogoi. Talk soon!' },
        },
      ]),
    ]);
    ...
    expect(result.farewell).toBe('All set — welcome to Mogoi. Talk soon!');

...
const INTERVIEWER_SYSTEM_PROMPT = `You are Mogoi interviewing a new user during first-run onboarding. Mogoi is an AI cofounder that helps turn repetitive work into inspectable, programmatic workflows and uses AI where judgment is needed. Your job here is to learn enough about the user's work to make that collaboration useful, then save the context in their profile.
...
- At the end, briefly reflect a possible starting point if the user supplied one and explain that they can ask Mogoi to draft a workflow after setup. Never claim a workflow is ready, an integration is connected, or an approval policy is active because it was discussed.
...
async function advanceInterview(
  ...
  if (session.turnCount > MAX_INTERVIEW_TURNS) {
    ...
    session.done = true;
    session.farewell =
      "Let's wrap up here. You can add more context as you work with Mogoi.";
...
function executeInterviewerTool(
  ...
  if (call.name === 'wrap_interview') {
    ...
    const farewell = typeof args.farewell === 'string' && args.farewell.trim()
      ? args.farewell.trim()
      : 'Thanks. You can add more context as you work with Mogoi.';

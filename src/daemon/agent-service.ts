...
import { effectiveLlmForBinding } from './motius-ai.ts';
...
export class AgentService implements Service, IAgentService {
  ...
  constructor(config: MogoiConfig) {
    this.config = config;
  ...
  private registerProviders(): void {
    ...
    if (this.llmManager.hasConversationTier()) {
      ...
      // gating, and Mogoi-specific feature knowledge. Uses processTaskCall
  ...
  private buildPersona(): string {
    ...
    const name = p?.assistant_name ?? 'MOGOI';
    return [
   ...
   * paths (workflow's `mogoi-ask` piece, etc.) can give the LLM the same
   * Mogoi identity + role + personality + vault context that chat gets.
   *
   ...
   * live-voice framing (~100 tokens). Tools stay available, so MOGOI can still
  ...
  buildRealtimeVoiceInstructions(): string {
    const name = this.config.personality?.assistant_name?.trim() || this.role?.name || 'MOGOI';

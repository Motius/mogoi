 ...
 * M.O.G.O.I. Daemon
 *
 * Main entry point for the MOGOI daemon process.
...
import { realtimeEnablement } from "./motius-ai.ts";
...
// Constants
const DEFAULT_PORT = 1846;  // MOGOI port
const DEFAULT_DATA_DIR = path.join(os.homedir(), '.mogoi');
...
Options:
  ...
  --db-path <path>         Database file path (default: ~/.mogoi/mogoi.db)
  --data-dir <path>        Data directory (default: ~/.mogoi)
...
Example:
  bun run src/daemon/index.ts --port 1846 --data-dir ~/.mogoi
        `);
        process.exit(0);
    }
  }
...
export async function startDaemon(userConfig?: Partial<DaemonConfig>): Promise<void> {
  ...
  const logFilePath = mogoiConfig.daemon.log_file_path?.trim();
  if (logFilePath) {
    // Skip the sink when the launcher ALREADY has our stdout/stderr open on
    // that exact file - `mogoi start -d`, `mogoi update`'s restart, the
    ...
    // bind mounts and $MOGOI_HOME spellings of the same file.
    if (logFileIsProcessStdio(logFilePath)) {
      ...
      installLogFileSink({ path: logFilePath, maxBytes: mogoiConfig.daemon.log_file_max_bytes });
  ...
  const configuredDrain = mogoiConfig.daemon.drain_deadline_ms;
  ...
  const dataDir = userConfig?.dataDir ?? mogoiConfig.daemon.data_dir ?? DEFAULT_DATA_DIR;

  // If user specified a custom data dir but no db path, use mogoi.db in that dir
  const dbPath = userConfig?.dbPath ?? mogoiConfig.daemon.db_path ?? path.join(dataDir, 'mogoi.db');

  // Merge configuration
  const port = userConfig?.port ?? mogoiConfig.daemon.port ?? DEFAULT_PORT;
  ...
  // stale TCP lockfile from a mid-boot crash would mislead `mogoi stop`).
  const { resolveListen } = await import('../config/loader.ts');
  let listen: import('../config/loader.ts').ListenSpec;
  try {
    listen = resolveListen({ port, listen: mogoiConfig.daemon.listen });
  } catch (err) {
    console.error(`\n[Daemon] ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  }

  // Record the actual bound port in the lockfile so `mogoi stop` knows which
  // port to verify even when the daemon was started with --port, MOGOI_PORT,
  ...
  // and `mogoi stop` is pid-only (resolveStopPort returns port null).
  if (listen.kind === 'tcp') {
  ...
  try {
    ...
    // workflow_job / trigger_event) to the shared Mogoi DB. Idempotent.
    ...
    // 2a-bis. Relocate a pre-MOGOI_HOME keychain into the data dir before
    // anything reads a credential, so hydration below sees the moved store and
    // a data-dir backup finally includes it. No-op on a normal ~/.mogoi
    ...
    mergeLLMSettingsIntoConfig(mogoiConfig);
    ...
    // once, then env overrides are re-applied so MOGOI_* still wins.
    ...
    mergeUserSettingsIntoConfig(mogoiConfig);
    applyEnvOverrides(mogoiConfig);
    ...
    if (mogoiConfig.timezone) {
      try {
        const { setCronTimezone } = await import('../lib/cron-scheduler.ts');
        setCronTimezone(mogoiConfig.timezone);
        logWithTimestamp(`Cron timezone: ${mogoiConfig.timezone}`);
      } catch {
        console.warn(`[Daemon] Invalid timezone "${mogoiConfig.timezone}" in config - using machine local time`);
      }
    }
    ...
    const { reloadMotiusAIAiBlock, reloadMotiusAIBillingBlock } = await import('../config/loader.ts');
    settingsReload = new SettingsReloadCoordinator(mogoiConfig, {
      // File-authoritative SYSTEM sections are re-read on every reloadAll so
      // a provisioner rotation of the motius_ai key (or un-hosting) takes
      // effect on SIGHUP / POST /api/config/reload without a restart. Same
      // for the billing read's block.
      reloadSystemSections: async (cfg) => {
        await reloadMotiusAIAiBlock(cfg);
        await reloadMotiusAIBillingBlock(cfg);
      },
    });
    ...
    const heartbeatConfig = mogoiConfig.heartbeat;
    ...
    // start.
    googleAuth = makeGoogleAuth(mogoiConfig);
    ...
    const googleShape = classifyGoogle(mogoiConfig);
    ...
    const agentService = new AgentService(mogoiConfig);
    ...
    observerService?.setPushTargets({
      pubsubTopic: mogoiConfig.google?.pubsub_topic,
      pushCallback: mogoiConfig.google?.push_callback,
      channelToken: mogoiConfig.google?.channel_token,
    });
    // Hosted mode: daemon.listen = unix:/run/mogoi/u_<id>.sock binds a unix
    ...
    const channelService = new ChannelService(mogoiConfig, agentService);
    ...
    const sidecarManager = new SidecarManager(mogoiConfig.daemon.data_dir.replace('~', os.homedir()));
    ...
    const externalOrigin = resolveExternalOrigin(mogoiConfig);
    ...
    const brainSource: 'env' | 'config' | 'public_url' | 'default' = externalOrigin.source === 'public_url'
      ? (process.env.MOGOI_PUBLIC_URL ? 'env' : 'public_url')
      : externalOrigin.source === 'brain_domain'
        ? (process.env.MOGOI_BRAIN_DOMAIN ? 'env' : 'config')
        : 'default';
    ...
    // `MOGOI_AMBIENT_UI=0` to fall back to dashboard-only mode (useful
    // for headless servers, CI, or users who only want the web UI).
    if (process.env.MOGOI_AMBIENT_UI !== '0') {
      ...
      // Fallback timers for the bare-"Mogoi" listening state. If the sidecar's
      ...
      // the same mogoiConfig.* the dashboard uses so API keys / provider
      ...
      try {
        ...
        const { effectiveSttForBinding, motiusVoiceCredentials } = await import('./motius-ai.ts');
        const sttBinding = effectiveSttForBinding(mogoiConfig);
        pebbleSTT = sttBinding
          ? createSTTProvider(sttBinding, motiusVoiceCredentials(mogoiConfig))
          : null;
        if (pebbleSTT) {
      ...
      if (mogoiConfig.tts?.enabled) {
        try {
          const { createTTSProvider } = await import('../comms/voice.ts');
          const { effectiveTtsForBinding, motiusVoiceCredentials } = await import('./motius-ai.ts');
          const ttsBinding = effectiveTtsForBinding(mogoiConfig);
          pebbleTTS = ttsBinding
            ? createTTSProvider(ttsBinding, motiusVoiceCredentials(mogoiConfig))
            : null;
          if (pebbleTTS) {
      ...
      settingsReload?.registerApplier('stt', async (cfg) => {
        ...
        pebbleSTT = sttBinding ? createSTTProvider(sttBinding, motiusVoiceCredentials(cfg)) : null;
      ...
      settingsReload?.registerApplier('tts', async (cfg) => {
        ...
        if (cfg.tts?.enabled) {
          ...
          pebbleTTS = ttsBinding ? createTTSProvider(ttsBinding, motiusVoiceCredentials(cfg)) : null;
        }
      ...
      const ttsMimeType = (cfg: typeof mogoiConfig.tts): string => {
        ...
        switch (cfg.provider) {
          ...
          case 'elevenlabs':
          case 'edge':
          case 'motius': // proxy /audio/speech is asked for response_format: 'mp3'
          default:
            return 'audio/mp3';
        }
      };
      ...
      sidecarManager.onSidecarConnected(async (sidecar) => {
        ...
        try {
          ...
          const awarenessEnabled = (mogoiConfig.awareness as { enabled?: boolean })?.enabled ?? true;
      ...
      // cycle. The same MOGOI brain is shared across a user's machines, but
      // each machine has its own pebble showing what MOGOI does there, so a
      ...
      sidecarManager.onEvent(async (sidecarId, event) => {
        ...
        try {
          ...
          const id = (result && typeof result === 'object' && 'id' in (result as object))
            ? String((result as { id?: unknown }).id ?? '')
            : '';
          if (id) trackPanel(sidecarId, { id, key: 'answer_full', title: 'MOGOI answer' });
      ...
      sidecarManager.onEvent(async (sidecarId, event) => {
        ...
        try {
          ...
          const wasEnabled = mogoiConfig.awareness?.enabled ?? true;
          const nextEnabled = !wasEnabled;
          (mogoiConfig.awareness as { enabled?: boolean }).enabled = nextEnabled;
      ...
      // T20 — voice-driven settings mutation. Patches `mogoiConfig` and
      ...
      // Both appliers persist BEFORE mutating mogoiConfig and report failure
      ...
      const applyTTSEnabled = async (enabled: boolean): Promise<boolean> => {
        ...
        // instead of the included MotiusAI AI voice.
        try {
        ...
        if (!mogoiConfig.tts) mogoiConfig.tts = { enabled };
        else mogoiConfig.tts.enabled = enabled;
      ...
      const applySTTProvider = async (provider: 'openai' | 'groq' | 'sarvam' | 'local' | 'motius'): Promise<boolean> => {
        const { persistUserPatch } = await import('./user-settings.ts');
        const { hasMotiusAIAi } = await import('./motius-ai.ts');
        if (!mogoiConfig.stt) mogoiConfig.stt = { provider };
        // Refuse the switch if the target provider has no credentials
        // configured — we don't want to silently break STT. The hosted
        // 'motius' choice is gated on the system motius_ai block
        // (its key never lives in cfg.stt).
        const hasKey = (() => {
          if (provider === 'motius') return hasMotiusAIAi(mogoiConfig);
          if (provider === 'local') return !!mogoiConfig.stt!.local?.endpoint;
          const sub = (mogoiConfig.stt as unknown as Record<string, { api_key?: string } | undefined>)[provider];
          return !!sub?.api_key;
        })();
        ...
        if (!mogoiConfig.stt) mogoiConfig.stt = { provider };
        else mogoiConfig.stt.provider = provider;
      ...
      const tryHandleInPanelAction = async (
        ...
        // browser-tab sense, not a MOGOI panel sub-tab.
      ...
      const tryHandleSettingsIntent = async (
        ...
        // STT provider switch.
        const sttMatch =
          /\b(switch|change)\s+(?:the\s+)?(stt|speech[- ]to[- ]text|transcription|speech recognition|listening)\s+(?:to|provider to)\s+(openai|whisper|groq|sarvam|local|motius|use mogoi|mogoi)\b/.exec(t) ||
          /\buse\s+(openai|whisper|groq|sarvam|local|motius|use mogoi|mogoi)\s+(?:for\s+(?:stt|speech[- ]to[- ]text|transcription|speech recognition|listening|hearing))\b/.exec(t);
        if (sttMatch) {
          // 'whisper' → openai; 'mogoi' / 'use mogoi' (how STT typically
          // transcribes the brand name) → the hosted motius provider.
          type SttTarget = 'openai' | 'groq' | 'sarvam' | 'local' | 'motius';
          const canonical = (name: string): SttTarget =>
            (name === 'whisper' ? 'openai' : name === 'use mogoi' || name === 'mogoi' ? 'motius' : name) as SttTarget;
          ...
          if (candidate && /^(openai|whisper|groq|sarvam|local|motius|use mogoi|mogoi)$/.test(candidate)) {
      ...
      // open). Saved values come from `~/.mogoi/window-state.json`,
      ...
      // W3-T2 — persist per-room window bounds to ~/.mogoi/window-state.json
      ...
      const buildPanelContext = async (sidecarId: string): Promise<string> => {
        ...
        sections.push(
          ...
          '',
          '  - **Web apps** (Gmail, Notion, GitHub, the MOGOI dashboard, anything in a browser): use `browser_navigate` / `browser_snapshot` / `browser_click` / `browser_type`. These dispatch DOM events through CDP — no OS cursor involved, very reliable, very fast. The user can keep using their mouse for other things while you operate the page.',
          '',
          ...
          '',
          'The pebble is a *visual narrator only* — it flies to the action target so the user can see what MOGOI is doing, but it does NOT control the user\'s cursor. The user keeps full mouse freedom while you operate apps through the structured paths above.',
          '',
        ...
        if (foreground) {
          sections.push(
            ...
            '',
            'When they say "this", "here", "the window", "this app" without naming a MOGOI panel, they likely mean THIS foreground window. If they ask "where do I click for X?" or "show me Y", reason from this app\'s typical UI: e.g. Chrome\'s close button is in the top-right, VSCode\'s file explorer is the leftmost icon in the activity bar, CapCut\'s timeline is along the bottom. Emit a `[POINT:x,y:label]` tag with your best estimate based on the app and the user\'s screen layout. If the request is ambiguous between the foreground app and a MOGOI panel, prefer the foreground app unless they reference a MOGOI room by name.',
            '',
          );
        }
        if (hasPanels) {
          sections.push(
            '# MOGOI dashboard panels currently open as native windows',
        ...
        if (pebbleTTS && !ctrl.cancelled) {
          try {
            ...
            await sidecarManager.dispatchRPC(sidecarId, 'pebble.play_audio', {
              ...
              mime_type: ttsMimeType(mogoiConfig.tts),
              blocking: false,
            });
          } catch (err) {
  ...
  try {
    ...
    return JSON.stringify({
      ...
      const runResponseCycle = async (
          ...
          for await (const event of stream) {
            ...
            if (event.type === 'text' && event.text) {
              ...
              if (NARRATE_TOOLS.test(tcName)) {
                ...
                // the user SEES MOGOI reach for the button before it
        ...
        console.log(
          `[ambient-ui] MOGOI (${llmMs}ms LLM, ${firstTokenMs}ms to first token, ${firstAudioMs}ms to first audio) → ` +
        ...
        // to actually read what MOGOI said. User can dismiss anytime by
      ...
      // transcribe and word-search for "mogoi"; on a hit, we either
      //   - run the LLM directly with the trailing command (single-shot,
      //     "Mogoi play music" works without saying it twice), or
      //   - if only "mogoi" alone (bare, or trailed by nothing but
      ...
      sidecarManager.onEvent(async (sidecarId, event) => {
        ...
        // got MOGOI's attention via Ctrl+Space (or a prior wake).
        ...
        if (!command) {
          // Just "Mogoi" alone, "Hey Mogoi!" included (`wakeCommandFrom`
    ...
    if (mogoiConfig.tts?.enabled) {
      ...
      if (ttsProvider) {
        ...
        console.log(`[Daemon] TTS enabled: ${ttsBinding?.provider ?? 'edge'} (${mogoiConfig.tts.voice ?? 'en-US-AriaNeural'})`);
      }
    }
    ...
    const authorityConfig = mogoiConfig.authority ?? { default_level: 3 };
    ...
    emergencyController.setStateChangeCallback(async (state) => {
      ...
      try {
        ...
        if (!mogoiConfig.authority) mogoiConfig.authority = { default_level: 3 } as any;
        mogoiConfig.authority.emergency_state = state;
        saveUserSection('authority', mogoiConfig.authority);
      } catch (err) {
    ...
    settingsReload.registerApplier('stt', async (cfg) => {
      ...
      const { effectiveSttForBinding, motiusVoiceCredentials } = await import('./motius-ai.ts');
      const sttBinding = effectiveSttForBinding(cfg);
      wsService.setSTTProvider(sttBinding ? createSTTProvider(sttBinding, motiusVoiceCredentials(cfg)) : null);
    ...
    settingsReload.registerApplier('tts', async (cfg) => {
      ...
      wsService.setTTSProvider(ttsBinding ? createTTSProvider(ttsBinding, motiusVoiceCredentials(cfg)) : null);
    });
    ...
    let lastGoogleCreds = googleIdentity(mogoiConfig);
    ...
    // ── Tray menu live data (design: motius-tray §00) ──
    ...
    // Recent activity = the last few actions Mogoi actually carried out, newest
    ...
    // Tray → brain: the "Pause Mogoi" toggle drives the emergency controller.
    ...
    // ── Outbound OS notifications (design: motius-tray §01) ──
    // The four reasons Mogoi interrupts: it needs your OK (approval), it
    ...
    // Mogoi" only; the founder opted to allow Approve/Deny on every approval
      ...
      if (sc.updateStatus === 'suggested') {
        ...
        if (!notifiedUpdateKeys.has(key)) {
          ...
          void sidecarManager.dispatchRPC(sc.id, 'notify.show', {
            ...
            destructive: false,
            actions: [
              { id: 'review', label: 'Open Mogoi', primary: true },
    ...
    systemCron = new SystemCronService(sharedEventBus, mogoiConfig.cron);
    systemCron.start();
    ...
    // canonical PieceCatalog. /v1/mogoi/* service backends are wired below
    ...
    // Build the credential resolver early so we can register Mogoi-managed
    // sources (Google OAuth file, etc.) before pieces start asking for
    // connections. Each `mogoi:<source>` external id dispatches to the
    ...
    const credentialResolver = new CredentialResolver();
    {
      ...
      const { MogoiGoogleConnectionSource } = await import(
        "../workflows/credentials/google-source.ts"
      );
      credentialResolver.register(new MogoiGoogleConnectionSource(() => googleAuth));
      logWithTimestamp(
        "Workflow credential resolver: registered mogoi:google source",
      );
    }
    // mogoi:telegram bridges the same bot token the daemon's Telegram
    // adapter is using -- pieces (activepieces telegram-bot, custom flows)
    // can reference `mogoi:telegram` instead of asking the user to create
    ...
    if (mogoiConfig.channels?.telegram?.enabled && mogoiConfig.channels.telegram.bot_token) {
      const { MogoiTelegramConnectionSource } = await import(
        "../workflows/credentials/telegram-source.ts"
      );
      credentialResolver.register(
        new MogoiTelegramConnectionSource(
          () => mogoiConfig.channels?.telegram?.bot_token ?? null,
        ),
      );
      logWithTimestamp(
        "Workflow credential resolver: registered mogoi:telegram source",
      );
    }
    ...
    const sharedRuntime = resolveSharedRuntimePaths(mogoiConfig);
    ...
    try {
      engineBoot = await bootstrapWorkflowEngine({
        ...
        engineIdleTtlMs: resolveEngineIdleTtlMs(mogoiConfig.workflows?.engineIdleTtlMs),
      ...
      console.error(
        [
          ...
          `           - Disk full or ~/.mogoi/cache unwriteable`,
    ...
    if (mogoiConfig.auth?.insecure_open_access === true) {
      ...
      console.warn('[Daemon] (`mogoi enroll "<device-name>"`, paste the token into the');
    ...
    if ((mogoiConfig.auth as Record<string, unknown> | undefined)?.token !== undefined) {
      console.warn('[Daemon] auth.token is no longer supported and was ignored. Access is JWT-only:');
      console.warn('[Daemon] enroll a device (`mogoi enroll`) or, for setup only, set auth.insecure_open_access.');
    }
    ...
    if (mogoiConfig.browser?.local === false) {
    ...
    if (toolRegistry) {
      ...
      const composerToolRegistry = toolRegistry
        ? {
            ...
            // correct `params` and reject a mogoi-tool:invoke step that omits a
      ...
      const manageWorkflowTool = createManageWorkflowTool({
        ...
        // Gmail/Calendar via Mogoi tools).
    ...
    // 10.1. Workflow runtime: wire `/v1/mogoi/*` service backends and start
    ...
    if (workflowSandboxApi && workflowEngineRuntime) {
      ...
      // `mogoi-agent.delegate` runs the full sub-agent loop (LLM + tool
    ...
    const inSetupMode = !mogoiConfig.onboarding?.setup_completed_at;
    if (inSetupMode) {
    ...
    const startPostSetupServices = async (): Promise<void> => {
      ...
      const bgAgentService = new BackgroundAgentService(mogoiConfig, agentService.getLLMManager());
      ...
      if (mogoiConfig.awareness?.enabled !== false && !config.noLocalTools) {
    ...
    const startAwarenessService = async (): Promise<void> => {
      ...
      try {
          ...
          (event) => {
            ...
            if (event.type === 'suggestion_ready') {
              ...
              if (hasWsClients) {
                ...
                sendDesktopNotification(`MOGOI: ${title}`, body, { urgency: 'normal' });
                ...
                sendDesktopNotification(`MOGOI: ${title}`, body, { urgency: 'critical', expireMs: 30000 });
              }
            }
              ...
              if (errorText.length > 5) {
                ...
                bgAgent.handleMessage(
                  ...
                  if (solution && solution.length > 10) {
                    ...
                    sendDesktopNotification(`MOGOI: Fix for ${appName}`, solution.slice(0, 200), { urgency: 'critical', expireMs: 15000 });
              ...
              if (compositeScore >= 0.7 && (appCategory === 'code_editor' || appCategory === 'terminal')) {
                ...
                bgAgent.handleMessage(
                  ...
                  if (solution && solution.length > 10) {
                    ...
                    wsService.broadcastNotification(solutionText, 'urgent');
                    sendDesktopNotification(`MOGOI: Help for ${sAppName}`, solution.slice(0, 200), { urgency: 'critical', expireMs: 15000 });
                    const voiceText = solution
                      .replace(/#{1,6}\s*/g, '')
                      .replace(/\*{1,2}([^*]+)\*{1,2}/g, '$1')
                      .replace(/`([^`]+)`/g, '$1')
        ...
        sidecarManager.onConnect((sidecarId) => {
          const cfg = mogoiConfig.awareness;
          if (!cfg) return;
    ...
    if (mogoiConfig.sites?.enabled !== false) {
      try {
        const { SiteBuilderService } = await import('../sites/service.ts');
        const sitesConfig = mogoiConfig.sites ?? {
          enabled: true,
          projects_dir: '~/.mogoi/projects',
    ...
    const goalsConfig = mogoiConfig.goals;
    ...
    logWithTimestamp(`MOGOI daemon running on port ${config.port}`);
    console.log('');

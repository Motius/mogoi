...
import { hasMotiusAIAi, realtimeEnablement, resetRealtimeVaultWarningForTest } from './motius-ai.ts';
 ...
 * MOGOI_REALTIME_VOICE env var flipped it, so the hosted branch of
...
const hosted = (over: Record<string, unknown> = {}): MogoiConfig =>
  ({
    motius_ai: { base_url: 'https://llm.example/v1', api_key: 'sk-uj-x' },
    llm: { providers: {} },
    ...over,
  }) as unknown as MogoiConfig;

const selfHosted = (over: Record<string, unknown> = {}): MogoiConfig =>
  ({ llm: { providers: {} }, ...over }) as unknown as MogoiConfig;
...
describe('the operator kill switch', () => {
  const KEY = 'MOGOI_REALTIME_VOICE';
  const restore = (v: string | undefined) => {
    if (v === undefined) delete process.env[KEY];
    else process.env[KEY] = v;
  };

  test('MOGOI_REALTIME_VOICE=0 still disables realtime on a hosted install', () => {
...
describe('the two hosted predicates must never disagree', () => {
  // realtimeEnablement keys off hasMotiusAIAi; the resolver and the settings
  ...
  for (const [name, block] of cases) {
    test(`agree on: ${name}`, () => {
      const cfg = { llm: { providers: {} }, motius_ai: block } as unknown as MogoiConfig;
      expect(() => realtimeServedByPlan(cfg)).not.toThrow();
      expect(realtimeServedByPlan(cfg)).toBe(hasMotiusAIAi(cfg));
    });
  }
});
...
describe('a mistyped config block does not flood the log', () => {
  test('the malformed-block warning fires ONCE, not per read', () => {
    ...
    // runs hasMotiusAIAi, so one unquoted YAML scalar would print four times
    ...
    try {
      const cfg = {
        llm: { providers: {} },
        motius_ai: { base_url: 8080, api_key: 'sk-uj-x' },
      } as unknown as MogoiConfig;
      for (let i = 0; i < 5; i++) hasMotiusAIAi(cfg);
      expect(warnings.filter((w) => w.includes('malformed motius_ai'))).toHaveLength(1);
    } finally {

...
const hostedConfig = () => ({
  ...
  motius_ai: { base_url: 'https://llm.motius.host', api_key: 'sk-uj-abc' },
  llm: { providers: {} },
}) as unknown as MogoiConfig;

const makeService = (config: MogoiConfig = hostedConfig()) => {
  const fakeAgent = {
...
describe('mic frames on a socket with a realtime session', () => {
  ...
  test('frames still arriving after a PCM refusal are dropped without a warning each', async () => {
    const offConfig = { voice: { realtime: { enabled: false } }, llm: { providers: {} } } as unknown as MogoiConfig;

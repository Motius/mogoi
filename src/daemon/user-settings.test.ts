...
function freshConfig(): MogoiConfig {
...
describe('user-settings', () => {
  ...
  beforeEach(() => {
    ...
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-user-settings-'));
  ...
  test('workflows: file-provided SYSTEM path keys win over a saved user section', () => {
    ...
    expect(config.workflows?.engine_dir).toBe('/opt/mogoi-engine/${version}');
  ...
  test('persistUserPatch: an explicit provider in the patch is recorded as intent', () => {
    ...
    persistUserPatch('stt', { provider: 'motius' });
    expect(loadUserSection('stt')).toEqual({ provider: 'motius' });
  });
  ...
  test('end to end: legacy file -> import -> discard -> merge equals old behavior', async () => {
    ...
    const dir = `${process.env.TMPDIR ?? '/tmp'}/mogoi-user-settings-${Date.now()}`;

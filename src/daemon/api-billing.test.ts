...
function handlerFor(config: Partial<MogoiConfig>): Handler {
...
describe('GET /api/billing', () => {
  ...
  it('is ok:false (not 503) on a hosted install whose control plane rendered no billing block', async () => {
    ...
    const res = await handlerFor({
      daemon: { listen: 'unix:/run/mogoi/u_1/brain.sock' },
    } as Partial<MogoiConfig>)(req());
  ...
  it('serves the parsed summary and the account links', async () => {
    ...
    const res = await handlerFor({ motius_billing: billingBlock() } as Partial<MogoiConfig>)(req());
    expect(res.status).toBe(200);
  ...
  it('never echoes the endpoint, instance id or secret — on success or failure', async () => {
    for (const reply of [
      ...
      const text = await (await handlerFor({ motius_billing: block } as Partial<MogoiConfig>)(req())).text();
  ...
  it('reads are cached, and ?fresh=1 still shares a reading younger than the floor', async () => {
    ...
    const handler = handlerFor({ motius_billing: billingBlock() } as Partial<MogoiConfig>);
    await handler(req());
    await handler(req());
    expect(count).toBe(1);

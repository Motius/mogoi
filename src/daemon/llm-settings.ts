...
import type { MogoiConfig, LLMProviderEntry, LLMProviderKind } from '../config/types.ts';
import {
  applyMotiusAIAi,
...
export type LLMSettingsResponse = {
  ...
  /** True on hosted installs (the system-owned motius_ai block is live). */
  hosted_llm?: boolean;
...
export function getLLMSettings(config: MogoiConfig): LLMSettingsResponse {
  ...
  for (const [name, entry] of Object.entries(config.llm.providers ?? {})) {
    ...
    // `motius_ai` before the reservation existed still owns that entry.
    if (name === USEMOGOI_PROVIDER_NAME && hasMotiusAIAi(config)) continue;
  ...
  return {
    ...
    hosted_llm: hasMotiusAIAi(config),
...
export function saveLLMSettings(
  ...
  // hand-crafted POST carrying `motius_ai:gpt-5.5` or `motius_ai:uj-stt`
  // must not reach the proxy under the account key (review pr3#1, pr1#9).
  if (hasMotiusAIAi(config)) {
    ...
    for (const ref of candidateRefs) {
      ...
      if (parsed?.provider === USEMOGOI_PROVIDER_NAME) {
  ...
  if (body.providers) {
    ...
    for (const [name, update] of updates) {
      ...
      if (name === USEMOGOI_PROVIDER_NAME && hasMotiusAIAi(config)) {
        throw new Error(
          `Provider '${USEMOGOI_PROVIDER_NAME}' is managed by your hosting plan and cannot be edited or deleted`,
        );
      }
  ...
  setSetting(SETTING_PROVIDERS, JSON.stringify(
    stripSecretsFromProviders(config.llm.providers, { hosted: hasMotiusAIAi(config) }),
  ));
...
export function stripSecretsFromProviders(
  ...
  for (const [name, entry] of Object.entries(providers ?? {})) {
    ...
    if (name === USEMOGOI_PROVIDER_NAME && options.hosted !== false) continue;
...
export function mergeLLMSettingsIntoConfig(
  ...
  if (providersJson) {
    try {
      ...
      for (const [name, entry] of Object.entries(parsed)) {
        ...
        // loads (applyMotiusAIAi would overwrite it anyway; skipping keeps
  ...
  // 2. Hosted installs: inject the motius_ai provider from the config.yaml
  ...
  // injecting afterwards made every persisted motius_ai:* ref look dangling
 ...
 */
function pruneOrphanedModelRefs(config: MogoiConfig, persist = true): void {
...
function migrateDeprecatedGroqModels(config: MogoiConfig, persist = true): void {
 ...
 * shape is empty.
 */
function migrateLegacyDBSettings(config: MogoiConfig): void {
 ...
 */
export function hotReloadLLMProviders(config: MogoiConfig, llmManager: LLMManager): void {
...
export async function testLLMProvider(
  ...
  if (name === USEMOGOI_PROVIDER_NAME && hasMotiusAIAi(config)) {
    return { ok: false, error: 'motius_ai is system-managed and cannot be tested here' };
  }

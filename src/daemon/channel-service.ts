...
import { effectiveSttForBinding, motiusVoiceCredentials } from './motius-ai.ts';
...
export class ChannelService implements Service {
  ...
  constructor(config: MogoiConfig, agentService: AgentService) {
    this.config = config;
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // user section) picks the hosted MotiusAI AI default when the user
      ...
      if (sttBinding) {
        this.sttProvider = createSTTProvider(sttBinding, motiusVoiceCredentials(this.config));
        if (this.sttProvider) {
...
export async function routePerChannel(
  ...
  for (const name of channels) {
    ...
    if (!lastRecipient) {
      failed.push({
        channel: name,
        error: `no known recipient for "${name}" -- message Mogoi from that channel once to seed it`,
      });
      continue;
    }
    try {

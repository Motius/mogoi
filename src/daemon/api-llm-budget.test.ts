...
describe('GET /api/llm/budget', () => {
  ...
  it('is 503 when the hosted block is incomplete', async () => {
    // hasMotiusAIAi requires BOTH base_url and api_key; half a block is not a
    // hosted install.
    const res = await handlerFor({
      motius_ai: { base_url: 'https://llm.example/v1' },
    } as Partial<MogoiConfig>)(new Request('http://localhost/api/llm/budget'));
    expect(res.status).toBe(503);
  });

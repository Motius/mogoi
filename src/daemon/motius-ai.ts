import type { MogoiConfig, LLMConfig, STTConfig, TTSConfig } from '../config/types.ts';
import type { HostedVoiceCredentials } from '../comms/voice.ts';
import { loadUserSection, loadUserSectionStrict } from './user-settings.ts';

/**
 * Hosted "MotiusAI AI" wiring (the platform's LLM proxy).
 *
 * The `motius_ai` config.yaml block (base_url + the user's per-account
 ...
 * persisted motius_ai:* refs are never treated as dangling).
 *
...
export const USEMOGOI_PROVIDER_NAME = 'motius_ai';
 ...
 */
export const USEMOGOI_KIND = 'motius_ai';
...
export const USEMOGOI_TIER_DEFAULTS = {
  conversation: `${USEMOGOI_PROVIDER_NAME}:uj-chat`,
  low: `${USEMOGOI_PROVIDER_NAME}:uj-low`,
  medium: `${USEMOGOI_PROVIDER_NAME}:uj-medium`,
  high: `${USEMOGOI_PROVIDER_NAME}:uj-high`,
} as const;
...
function warnMalformedBlockOnce(): void {
  ...
  console.warn(
    '[MotiusAIAI] Ignoring malformed motius_ai config block: base_url and api_key must be strings (quote them in config.yaml).',
  );
}

function readMotiusAIAiBlock(
  config: MogoiConfig,
): { base_url: string; api_key: string } | null {
  const block = config.motius_ai;
  ...
  if (
    ...
    // GET /api/config/voice every ~15s and each read runs hasMotiusAIAi, so a
...
/** True when this install is hosted (a complete, well-formed motius_ai block exists). */
export function hasMotiusAIAi(config: MogoiConfig): boolean {
  return readMotiusAIAiBlock(config) !== null;
}
 ...
 * `hasMotiusAIAi` above. That one answers "is the hosted LLM available", which
 ...
 * `motius_ai` is renderable as null -- the control plane emits no block when
 ...
 * enrollment, for instance, remains available from the CLI (`mogoi enroll`),
 * which docs/SELF_HOSTING.md already calls the primary route.
 */
export function isHostedInstall(config: MogoiConfig): boolean {
  if (hasMotiusAIAi(config)) return true;
 ...
 */
export function applyMotiusAIAi(config: MogoiConfig): void {
  const block = readMotiusAIAiBlock(config);
  if (!block) return;
  ...
  config.llm.providers[USEMOGOI_PROVIDER_NAME] = {
    kind: USEMOGOI_PROVIDER_NAME,
    ...
    prompt_cache: config.motius_ai?.prompt_cache === true,
  };
}
 ...
 */
export function effectiveLlmForBinding(config: MogoiConfig): LLMConfig {
  if (!hasMotiusAIAi(config)) return config.llm;

  const tiers: Record<string, string> = { ...(config.llm.tiers ?? {}) };
  for (const [tier, alias] of Object.entries(USEMOGOI_TIER_DEFAULTS)) {
    if (!tiers[tier]) {
...
/**
 * Validate the model half of a `motius_ai:<model>` ref. Returns an error
 ...
 */
export function motiusVoiceCredentials(config: MogoiConfig): HostedVoiceCredentials | null {
  if (!hasMotiusAIAi(config)) return null;
  const block = config.motius_ai!;
 ...
 * STT provider default to the included MotiusAI AI transcription.
 *
 ...
 * the MOGOI_REALTIME_VOICE env var, and the provisioner sets neither. The
 ...
 *
 * It cannot be, and it should not be. The `motius_ai` block is
...
export type RealtimeEnablement =
  /** The user (or MOGOI_REALTIME_VOICE) said yes. On a SELF-HOSTED install
...
export function realtimeEnablement(
  ...
  if (!hasMotiusAIAi(config)) return configured ? 'user-on' : 'off';
  ...
  // come from the user or from MOGOI_REALTIME_VOICE. Only FALSE is ambiguous.
  ...
  if (process.env.MOGOI_REALTIME_VOICE !== undefined) return 'off';
...
function warnVaultOnce(err: unknown): void {
  ...
  vaultWarned = true;
  console.warn('[MotiusAIAI] could not read the stored voice section; leaving realtime as configured:', err);
}
...
export function effectiveSttForBinding(
  ...
  if (!hasMotiusAIAi(config)) return config.stt;
  if (storedProviderChoice(loadStored('stt'))) return config.stt;
  return { ...config.stt, provider: 'motius' };
}
...
export function effectiveTtsForBinding(
  ...
  if (!hasMotiusAIAi(config)) return config.tts;
  if (storedProviderChoice(loadStored('tts'))) return config.tts;
  return { ...(config.tts ?? { enabled: false }), provider: 'motius' };
}

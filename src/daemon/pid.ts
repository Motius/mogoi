/**
 * Process Lock Manager for M.O.G.O.I. Daemon
 *
...
/**
 * The daemon's own root: `MOGOI_HOME` when set (hosted wrappers export it for
 * every mogoi invocation on an instance), `~/.mogoi` otherwise. The lock
...
function daemonRootDir(): string {
  return process.env.MOGOI_HOME || MOGOI_DIR;
}

function defaultLockPath(): string {
  return join(daemonRootDir(), 'mogoi.pid');
}

/**
 * Resolve the lock-file path for a given data dir. With no argument this is
 * the daemon's own lock path (MOGOI_HOME-aware, see `daemonRootDir`); the
...
function getFlock(): FlockSymbols {
  if (process.platform === 'win32') {
    throw new Error(
      'The MOGOI daemon is not compatible with native Windows. Use WSL2 or Docker.',
    );
  }
 ...
 * default probes the daemon's own lock path (MOGOI_HOME-aware).
 */
 ...
 * daemon itself after port resolution so `mogoi stop` knows exactly which
 ...
 *
 * MOGOI_HOME-aware, like the lock path above. These used to be module-level
 * constants built from homedir() at import time, so on an instance that sets
 * MOGOI_HOME the daemon locked one root and logged to a different one —
 * `mogoi logs` then tailed a file the daemon wasn't writing.
 */
export function getLogPath(): string {
  const dir = getLogDir();
  mkdirSync(dir, { recursive: true });
  return join(dir, 'mogoi.log');
}

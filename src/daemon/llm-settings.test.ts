...
describe('LLM provider deletion', () => {
  it('clears every model ref the deleted provider owned', () => {
    initDatabase(':memory:');
    const config: MogoiConfig = structuredClone(DEFAULT_CONFIG);
...
function hostedConfig(): MogoiConfig {
  const config = structuredClone(DEFAULT_CONFIG);
  config.motius_ai = { base_url: 'https://llm.motius.host', api_key: 'sk-uj-LIFETIMEKEY0000000000' };
  applyMotiusAIAi(config); // what boot / every merge does
  return config;
}
...
describe('saveLLMSettings: hosted model refs pass the server-side allowlist (W8)', () => {
  ...
  test('static rule (no catalog seen): non-chat aliases and raw upstream ids are refused, chat aliases save', () => {
    ...
    for (const model of ['uj-stt', 'uj-tts-hd', 'uj-realtime', 'gpt-5.5']) {
      expect(() => saveLLMSettings(config, { tiers: { high: `motius_ai:${model}` } }))
        .toThrow(/not (in your plan|a chat alias)/);
    }
    expect(() => saveLLMSettings(config, { default: 'motius_ai:gpt-5.5' })).toThrow();
  ...
  test('a live catalog narrows the allowlist to exactly the plan; a degraded one never does', () => {
    ...
    saveLLMSettings(config, { tiers: { high: 'motius_ai:uj-pro' } });
    expect(getSetting('llm.tiers.high')).toBe('motius_ai:uj-pro');
    // uj-high passes the static rule but is not in THIS plan's catalog:
    expect(() => saveLLMSettings(config, { tiers: { medium: 'motius_ai:uj-high' } }))
    ...
    saveLLMSettings(config, { tiers: { medium: 'motius_ai:uj-high' } });
    expect(getSetting('llm.tiers.medium')).toBe('motius_ai:uj-high');
  });
  ...
  test('self-hosted installs have no gate: a legacy provider named motius_ai keeps free refs', () => {
    const config = structuredClone(DEFAULT_CONFIG);
    config.llm.providers = { motius_ai: { kind: 'openai_compatible', base_url: 'https://my.gw/v1' } };
    saveLLMSettings(config, { tiers: { high: 'motius_ai:gpt-5.5' } });
    expect(getSetting('llm.tiers.high')).toBe('motius_ai:gpt-5.5');
  });
});
...
describe('saveLLMSettings: the hosted provider is not editable', () => {
  ...
  test('refuses an edit rather than silently ignoring it', () => {
    const config = hostedConfig();
    expect(() => saveLLMSettings(config, {
      providers: { [USEMOGOI_PROVIDER_NAME]: { base_url: 'http://attacker/v1' } },
    })).toThrow(/managed by your hosting plan/);
    // The injected entry is untouched by the rejected write.
    expect(config.llm.providers?.[USEMOGOI_PROVIDER_NAME]?.base_url).toBe('https://llm.motius.host');
  });

  test('refuses a delete too', () => {
    const config = hostedConfig();
    expect(() => saveLLMSettings(config, {
      providers: { [USEMOGOI_PROVIDER_NAME]: null },
    })).toThrow(/managed by your hosting plan/);
    expect(config.llm.providers?.[USEMOGOI_PROVIDER_NAME]).toBeDefined();
  });
  ...
  test('a save on a hosted install leaves the reserved name out of the settings table', () => {
    ...
    expect(stored).not.toContain(USEMOGOI_PROVIDER_NAME);
    expect(stored).not.toContain('sk-uj-');
    expect(stored).not.toContain('llm.motius.host');
  });
});
...
describe('getLLMSettings: the block is reported, never exposed', () => {
  ...
  test('reports hosted_llm without any credential or endpoint material', () => {
    ...
    expect(view.providers[USEMOGOI_PROVIDER_NAME]).toBeUndefined();
    const serialized = JSON.stringify(view);
    expect(serialized).not.toContain('sk-uj-');
    expect(serialized).not.toContain('llm.motius.host');
  });
...
describe('getLLMSettings (hosted vs self-hosted surface)', () => {
  ...
  test('hosted: a user provider lists alone, the managed entry stays hidden', () => {
    ...
    expect(out.providers[USEMOGOI_PROVIDER_NAME]).toBeUndefined();
...
describe('getLLMSettings.effective: the dashboard reads routing reality, not a re-derivation', () => {
  ...
  test('hosted + everything silent: router-first on the four plan aliases, sourced "plan"', () => {
    ...
    expect(effective.tiers.conversation).toEqual({ ref: 'motius_ai:uj-chat', source: 'plan' });
    expect(effective.tiers.high).toEqual({ ref: 'motius_ai:uj-high', source: 'plan' });
    expect(effective.tiers.medium).toEqual({ ref: 'motius_ai:uj-medium', source: 'plan' });
    expect(effective.tiers.low).toEqual({ ref: 'motius_ai:uj-low', source: 'plan' });
  ...
  test('hosted + llm.default set: per-slot D1 resolution, explicit choice wins its slot', () => {
    ...
    const { effective } = getLLMSettings(config);
    expect(effective.tiers.high).toEqual({ ref: 'motius_ai:uj-high', source: 'choice' });
...
describe('self-hosted installs with a legacy provider literally named motius_ai (W10)', () => {
  beforeEach(() => { closeDb(); initDatabase(':memory:'); });
  afterEach(() => { closeDb(); });
  ...
  test('the entry survives an unrelated save, stays listed, and hosted_llm stays false', () => {
    ...
    expect(reloaded.llm.providers?.motius_ai?.base_url).toBe('https://my.gw/v1');
  });
});

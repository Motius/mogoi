...
Core daemon infrastructure for the MOGOI system.
...
```bash
# Run with defaults (port 1846, ~/.mogoi data directory)
bun run src/daemon/index.ts

# Run with custom configuration
bun run src/daemon/index.ts --port 1846 --data-dir ~/my-mogoi-data
...
|--------|---------|-------------|
| `--port` | 1846 | WebSocket server port |
| `--data-dir` | `~/.mogoi` | Data directory |
| `--db-path` | `~/.mogoi/mogoi.db` | Database file path |
...
```
[2026-02-22T22:17:03.230Z] Initializing database at /home/user/.mogoi/mogoi.db
...
- **Location**: `~/.mogoi/mogoi.db` (configurable)

Schema is managed by `~/mogoi/src/vault/schema.ts`.
...
timeout 3 bun run src/daemon/index.ts --data-dir /tmp/test-mogoi
```

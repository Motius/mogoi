...
describe('isVoiceStopCommand', () => {
  test('accepts stop-only voice controls without matching ordinary requests', () => {
    expect(isVoiceStopCommand('Mogoi stop')).toBe(true);
    expect(isVoiceStopCommand('HEY MOGOI, BE QUIET!')).toBe(true);
    expect(isVoiceStopCommand('stop')).toBe(true);
    expect(isVoiceStopCommand('Mogoi stop the timer')).toBe(false);

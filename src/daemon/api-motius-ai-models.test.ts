...
/**
 * Tests for GET /api/config/llm/motius/models.
 *
 * The interesting parts of this handler are (1) availability: the catalog
 * exists only on hosted installs (a complete system-owned motius_ai
...
const HOSTED_BASE = 'https://llm.motius.host';
...
function makeCtx(motiusAi?: { base_url?: string; api_key?: string }): ApiContext {
  return {
    daemonStartedAt: Date.now(),
    healthMonitor: {} as ApiContext['healthMonitor'],
    config: { llm: { providers: {} }, ...(motiusAi ? { motius_ai: motiusAi } : {}) } as MogoiConfig,
  } as ApiContext;
}

function callEndpoint(motiusAi?: { base_url?: string; api_key?: string }) {
  const routes = createApiRoutes(makeCtx(motiusAi));
  const handler = getHandler(routes, '/api/config/llm/motius/models', 'GET');
  return handler(new Request('http://x/api/config/llm/motius/models'));
}

describe('GET /api/config/llm/motius/models', () => {
  ...
  it('answers 503 on self-hosted installs (block absent or incomplete) without probing anything', async () => {
    for (const block of [undefined, { base_url: HOSTED_BASE }, { api_key: HOSTED_KEY }]) {
      ...
      expect(body.error).toBe('MotiusAI AI is only available on hosted installs.');

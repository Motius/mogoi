...
function handlerFor(config: Partial<MogoiConfig>) {
  const calls: string[] = [];
  const routes = createApiRoutes({
    daemonStartedAt: Date.now(),
    healthMonitor: {} as ApiContext['healthMonitor'],
    config: { llm: { providers: {} }, ...config } as MogoiConfig,
    sidecarManager: {
...
/** A complete hosted block -- hasMotiusAIAi needs both fields. */
const HOSTED = { motius_ai: { base_url: 'https://llm.motius.dev', api_key: 'sk-uj-abcdefghijklmnop' } };

describe('POST /api/sidecars/enroll', () => {
  it('refuses on a hosted install, and never reaches the manager', async () => {
    const { post, calls } = handlerFor(HOSTED as Partial<MogoiConfig>);
  ...
  it('an incomplete hosted block is not a hosted install', async () => {
    // hasMotiusAIAi requires both fields. Half a block is a misrendered config,
    // and treating it as hosted would lock a self-hoster out of their only way
    // to add a device.
    const { post, calls } = handlerFor({
      motius_ai: { base_url: 'https://llm.motius.dev' },
    } as Partial<MogoiConfig>);
    const res = await post(enroll());
  ...
  it('refuses a hosted instance that has no LLM proxy configured', async () => {
    ...
    // control plane, and every render path emits `motiusAi: null` without it
    ...
    const { post, calls } = handlerFor({
      daemon: { listen: 'unix:/run/mogoi/u_42/brain.sock' },
    } as unknown as Partial<MogoiConfig>);
    const res = await post(enroll());
  ...
  it('refuses when the hosted LLM block is malformed rather than absent', async () => {
    // hasMotiusAIAi fails OPEN on a non-string field (it warns and treats the
    // block as missing), which is right for a feature and wrong for a gate. The
    // socket signal covers it.
    const { post, calls } = handlerFor({
      motius_ai: { base_url: 123, api_key: 456 },
      daemon: { listen: 'unix:/run/mogoi/u_42/brain.sock' },
    } as unknown as Partial<MogoiConfig>);
    const res = await post(enroll());
  ...
  it('follows the LIVE config, so a SIGHUP reload flips the gate', async () => {
    // ctx.config is the object reloadMotiusAIAiBlock mutates in place. If a
    ...
    const config = { llm: { providers: {} } } as MogoiConfig;
    const calls: string[] = [];
    const routes = createApiRoutes({
      daemonStartedAt: Date.now(),
      healthMonitor: {} as ApiContext['healthMonitor'],
    ...
    (config as unknown as { motius_ai: unknown }).motius_ai = HOSTED.motius_ai;

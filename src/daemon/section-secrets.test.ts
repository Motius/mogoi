 ...
 *
 * The keychain writes to `~/.mogoi`, so every test redirects it to a
 * throwaway dir via MOGOI_SECRETS_DIR — never the developer's real store.
 */
...
describe('section secrets (stt/tts keys, channel tokens)', () => {
  ...
  beforeEach(() => {
    ...
    secretsDir = mkdtempSync(join(tmpdir(), 'mogoi-section-secrets-'));
  ...
  test('a failed keychain write never destroys the key', () => {
    ...
    process.env.MOGOI_SECRETS_DIR = blocked;

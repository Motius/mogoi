...
describe('resolveEngineIdleTtlMs', () => {
  test('env var wins over configured value', () => {
    expect(resolveEngineIdleTtlMs(60_000, { MOGOI_ENGINE_IDLE_TTL_MS: '30000' })).toBe(30_000);
  });
  ...
  test('rejects zero and negative values (0 would disable eviction)', () => {
    ...
    expect(resolveEngineIdleTtlMs(60_000, { MOGOI_ENGINE_IDLE_TTL_MS: '0' })).toBeUndefined();
  });

  test('rejects non-numeric env values', () => {
    expect(resolveEngineIdleTtlMs(60_000, { MOGOI_ENGINE_IDLE_TTL_MS: 'fast' })).toBeUndefined();
  });
});

...
export interface ComposerToolRegistry {
   ...
   * `mogoi-tool:invoke` step can't omit a tool's required param (e.g. `action`)
...
export interface ManageWorkflowDeps {
   ...
   * Mogoi tools so the LLM can wire `mogoi-tool { toolName: '...' }` correctly
   ...
   * roles in its prompt and rejects a `mogoi-agent:delegate` step whose `role`
...
export function createManageWorkflowTool(deps: ManageWorkflowDeps = {}): ToolDefinition {
  ...
  return {
    ...
    description: [
      "Create, run, and manage the user's Mogoi workflows (automations).",
      "",

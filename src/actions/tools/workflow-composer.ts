 ...
 * by prompting the configured Mogoi LLM with the piece catalog + a schema
...
const AGENT_PIECE_SHORT = "mogoi-agent";
...
const TOOL_PIECE_SHORT = "mogoi-tool";
 ...
 * catalog AND rejected in validation -- the LLM was picking `mogoi-test:echo`
 ...
 */
const COMPOSER_EXCLUDED_PIECES = new Set(["mogoi-test", "mogoi-validate"]);
...
/** One parameter of a Mogoi tool, as surfaced to the composer. */
...
/**
 * A Mogoi tool's invocation contract: its name plus the parameters it accepts.
 * The composer lists these so the LLM wires `mogoi-tool { toolName, params }`
...
export interface ComposeDeps {
  ...
  /**
   * Optional list of registered Mogoi tool names. When present, surfaced in
   * the planner prompt so the LLM can wire `mogoi-tool { toolName: '...' }`
   ...
   * `mogoi-tool:invoke` step's `params`. Falls back to `toolNames` (names
   * only) when absent.
   */
   ...
   * a `mogoi-agent:delegate` step, and (b) rejects any `delegate` step whose
...
function buildComposerToolDefs(hasToolSpecs: boolean, hasLibrary: boolean): ComposerToolDef[] {
  const defs: ComposerToolDef[] = [
    {
      ...
      description:
        ...
        "built-in trigger primitives (schedule, webhook), event types for mogoi-trigger:on_event, and registered Mogoi tools. " +
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          pieceName: {
            type: "string",
            description: "Piece name exactly as shown by list_pieces (e.g. 'mogoi-notify').",
          },
        },
        required: ["pieceName"],
      },
    ...
    },
    {
      name: "report_blocked",
      description:
        "Give up because the request cannot be built from the installed pieces and Mogoi tools. " +
        (hasLibrary
  ...
  if (hasToolSpecs) {
    defs.push({
      ...
      description:
        "Parameter contract of one registered Mogoi tool (for wiring a mogoi-tool:invoke step): " +
        "each param's name, type, and whether it is REQUIRED. Call this for every Mogoi tool you plan to invoke.",
      parameters: {
        type: "object",
        properties: {
  ...
  if (hasLibrary) {
    defs.push({
      ...
      description:
        ...
        "Use when no installed piece or Mogoi tool covers a service the user asked for, " +
...
/** Short piece name shown to the model ('@mogoipieces/piece-mogoi-tool' -> 'mogoi-tool'). */
 ...
 * the event-type index, and the Mogoi tool names. Schemas deliberately
...
function renderPieceIndex(deps: ComposeDeps): string {
  ...
  lines.push("");
  lines.push("Event types for mogoi-trigger:on_event (payload fields via get_piece_details('mogoi-trigger')):");
  ...
  if (toolNames && toolNames.length > 0) {
    lines.push("");
    lines.push("Available Mogoi tools (wire via the mogoi-tool piece's invoke action; params via get_tool_details):");
...
function sharedRuleSections(mode: "one-shot" | "tools", hasRoles: boolean): string[] {
  ...
  return [
    ...
    "  - For event triggers: pieceName='mogoi-trigger', triggerName='on_event', input.eventType='<event type>'.",
    ...
    "  - For mogoi-trigger:on_event, the trigger output is an event envelope shaped",
    ...
    mode === "tools"
      ? "  - When the user asks for an integration that isn't an installed piece (Gmail, Slack, ...), check the Mogoi tools in list_pieces and use the `mogoi-tool` piece with `toolName` set to one of them."
      : "  - When the user asks for an integration that isn't a registered piece (Gmail, Slack, ...), use the `mogoi-tool` piece with `toolName` set to a registered Mogoi tool. Available tools are listed below.",
    hasRoles
      ? "  - To hand a goal to a sub-agent, use the `mogoi-agent` piece's `delegate` action. Its optional `input.role` MUST be one of the specialist role ids listed below VERBATIM (e.g. `research-analyst`, not `researcher`). If none fits, OMIT `role` and the default agent handles it."
      : "  - To hand a goal to a sub-agent, use the `mogoi-agent` piece's `delegate` action with `input.goal`. Omit `input.role` to use the default agent.",
    "",
...
function buildSystemPrompt(
  ...
  return [
    "You are the Mogoi workflow composer. Convert the user's description into a workflow definition.",
    "",
    envText,
...
function buildToolLoopSystemPrompt(
  ...
  return [
    ...
    "",
    "## Process",
    "  1. Call list_pieces to see what is installed: pieces, built-in trigger primitives, event types, and Mogoi tools.",
    "  2. Call get_piece_details for EVERY piece you plan to use (and get_tool_details for every Mogoi tool).",
    ...
    hasLibrary
      ? "  4. If no installed piece or Mogoi tool covers the request, call search_library for an installable piece,\n     then call report_blocked suggesting the best match. Never force a wrong piece to fit."
      : "  4. If no installed piece or Mogoi tool covers the request, call report_blocked explaining what is missing.\n     Never force a wrong piece to fit.",
...
/**
 * Render the Mogoi tool listing for the prompt. Prefers the richer `tools`
...
function renderTools(
  ...
  if (tools && tools.length > 0) {
    const header =
      "Available Mogoi tools (call via `mogoi-tool { toolName, params: { ... } }`; include EVERY param marked REQUIRED):";
    const lines = [header];
  ...
  if (toolNames && toolNames.length > 0) {
    return [
      "Available Mogoi tools (call via `mogoi-tool { toolName, params }`):",
      ...toolNames.map((n) => `  - ${n}`),
    ].join("\n");
  }
  return "";
...
/**
 * List the valid specialist role ids for a `mogoi-agent:delegate` step. The
...
function renderSpecialistRoles(roles: ComposerSpecialistRole[] | undefined): string {
  ...
  return [
    "Available specialist sub-agent roles (for `mogoi-agent:delegate` -> `input.role`; use the id verbatim):",
    ...roles.map((r) => {
...
/**
 * One Mogoi tool's contract as prompt/tool-result lines: name + description,
...
function renderCatalog(registry: PieceLookup): string {
  ...
  // Workflow event-type catalog (used by mogoi-trigger:on_event flows).
  lines.push("");
  lines.push(...renderEventTypeDetailLines());
  return lines.join("\n");
}
...
function renderPieceBlockLines(
  ...
  for (const trigger of Object.values(piece.triggers ?? {})) {
    ...
    // also set `outputSample` (Mogoi extension). Either is a valid
...
/**
 * The event-type catalog for mogoi-trigger:on_event, with payload examples.
...
function renderEventTypeDetailLines(): string[] {
  const lines = [
    "Available event types for mogoi-trigger:on_event (settings.input.eventType):",
 ...
 * short human identity (`mogoi-trigger`, `mogoi-tool`), but the catalog is
 * keyed by the full npm package name (`@mogoipieces/piece-mogoi-trigger`).
...
function validateStep(
  ...
  // Without this resolution the composer would lose every Mogoi-piece flow
  ...
  // Delegate-role check: a `mogoi-agent:delegate` step's optional `role` must
  ...
  // Invoke-params check: a `mogoi-tool:invoke` step names a tool in

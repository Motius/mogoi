...
describe("composeFlow", () => {
    ...
    const reply = JSON.stringify({
      ...
      trigger: {
        ...
        nextAction: {
          name: "step_1",
          type: "PIECE",
          settings: { pieceName: "mogoi-ask", actionName: "ask", input: {} }, // prompt is required
        },
      },
    });
    const result = await composeFlow(
  ...
  test("system prompt includes the piece catalog and primitives", async () => {
    ...
    expect(sys).toContain("mogoi-ask");
    expect(sys).toContain("mogoi-notify");
    expect(sys).toContain("mogoi-trigger");
  ...
  test("system prompt spells out the {{trigger.payload.<field>}} access path for on_event triggers", async () => {
    ...
    expect(sys).toContain("mogoi-trigger:on_event");
  ...
  test("system prompt emits per-eventType output samples when the trigger declares dynamicSampleData", async () => {
    ...
    const onEventCatalog = new PieceCatalog([
      {
        name: "@mogoipieces/piece-mogoi-trigger",
        ...
        triggers: {
          on_event: {
            ...
            description: "Fires on a Mogoi event.",
    ...
    const reply = JSON.stringify({
      ...
      trigger: {
        ...
        nextAction: {
          ...
          type: "PIECE",
          settings: {
            pieceName: "@mogoipieces/piece-mogoi-trigger",
    ...
    const reply = JSON.stringify({
      ...
      trigger: {
        ...
        nextAction: {
          name: "Step 1",
          type: "PIECE",
          settings: { pieceName: "mogoi-ask", actionName: "ask", input: { prompt: "hi" } },
        },
      },
    });
    const result = await composeFlow(
  ...
  test("system prompt includes tool names when provided", async () => {
    ...
    expect(sys).toContain("Available Mogoi tools");
  });
  ...
  describe("delegate sub-agent role validation", () => {
    ...
    function agentCatalog(): PieceLookup {
      const entries: PieceCatalogEntry[] = [
        {
          name: "@mogoipieces/piece-mogoi-agent",
          displayName: "Mogoi: Agent",
          description: "Run a Mogoi sub-agent with a goal.",
          actions: {
            delegate: {
              name: "delegate",
  ...
  describe("mogoi-tool invoke param validation", () => {
    function toolCatalog(): PieceLookup {
      const entries: PieceCatalogEntry[] = [
        {
          name: "@mogoipieces/piece-mogoi-tool",
          displayName: "Mogoi: Tool",
          description: "Invoke a registered Mogoi tool.",
          actions: {
            invoke: {
              name: "invoke",
              displayName: "Invoke",
      ...
      JSON.stringify({
        ...
        trigger: {
          ...
          nextAction: {
            name: "step_1",
            type: "PIECE",
            settings: { pieceName: "mogoi-tool", actionName: "invoke", input: { toolName, params } },
          },
        },
      });
  ...
  describe("internal pieces are excluded from composition", () => {
    function catalogWithTestPiece(): PieceLookup {
      const entries: PieceCatalogEntry[] = [
        ...
        {
          name: "@mogoipieces/piece-mogoi-test",
    ...
    test("mogoi-test is hidden from the prompt catalog", async () => {
      const llm = new StubLlm(JSON.stringify({ displayName: "X", trigger: { name: "trigger", type: "EMPTY" } }));
      await composeFlow({ llm, pieceRegistry: catalogWithTestPiece() }, { name: "X", description: "x" });
      const sys = llm.calls[0]?.system ?? "";
      expect(sys).toContain("mogoi-ask");
      expect(sys).not.toContain("mogoi-test");
    });

    test("rejects a step that uses mogoi-test:echo even if the model names it directly", async () => {
      const reply = JSON.stringify({
        displayName: "X",
        trigger: {
          name: "trigger",
          ...
          nextAction: {
            name: "step_1",
            type: "PIECE",
            settings: { pieceName: "mogoi-test", actionName: "echo", input: { value: "ignored" } },
          },
        },
      });
      const result = await composeFlow(
      ...
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.errors.some((e) => /internal piece "mogoi-test"/.test(e))).toBe(true);
      }
    });
  });
        ...
        trigger: {
          ...
          nextAction: {
            ...
            nextAction: {
              ...
              children: [
                { name: "b_match", type: "PIECE", settings: { pieceName: "mogoi-notify", actionName: "notify", input: { message: "hit" } } },
                null,
              ],
            },
          },
      ...
      const reply = JSON.stringify({
        ...
        trigger: {
          ...
          nextAction: {
            name: "step_1",
            type: "PIECE",
            settings: { pieceName: "mogoi-ask", actionName: "ask", input: { prompt: "list things" } },
            nextAction: {
              name: "loop_1",
              type: "LOOP_ON_ITEMS",
              ...
              firstLoopAction: {
                name: "body_1",
                type: "PIECE",
                settings: { pieceName: "mogoi-notify", actionName: "notify", input: { message: "{{loop_1.item}}" } },
                nextAction: {
                  name: "body_2",
                  type: "PIECE",
                  settings: { pieceName: "mogoi-ask", actionName: "ask", input: { prompt: "{{loop_1.item}}" } },
                },
              },
            },
          },
            ...
            children: [
              {
                name: "branch_hit",
                type: "PIECE",
                settings: { pieceName: "mogoi-notify", actionName: "notify", input: { message: "matched" } },
              },
              null,
            ],
          },
        ...
        trigger: {
          ...
          nextAction: {
            ...
            nextAction: {
              name: "after_1",
              type: "PIECE",
              settings: { pieceName: "mogoi-ask", actionName: "ask", input: { prompt: "done" } },
            },
          },
        },
      });
  ...
  describe("system prompt surfaces output samples", () => {
    ...
    // (mogoi-tool.invoke returns { result, toolName }) and a bare-array
...
describe("tool-loop composer", () => {
  ...
  const validFlowArgs = () => ({
    ...
    trigger: {
      ...
      nextAction: {
        name: "step_1",
        type: "PIECE",
        settings: { pieceName: "mogoi-notify", actionName: "notify", input: { message: "hi" } },
      },
    },
  });
  ...
  test("explores with tools then submits a valid flow", async () => {
    const llm = new StubToolLlm([
      ...
      { tool_calls: [tc("2", "get_piece_details", { pieceName: "mogoi-notify" })] },
    ...
    expect(listResult.content).toContain("mogoi-notify");

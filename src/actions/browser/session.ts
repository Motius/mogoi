...
const SNAPSHOT_SCRIPT = `(() => {
  ...
  window.__mogoi_elements = els.map(e => e._el);
...
export class BrowserController {
  ...
  async type(
    ...
    const focusResult = await this.cdp.send('Runtime.evaluate', {
      expression: `(() => {
        const el = window.__mogoi_elements && window.__mogoi_elements[${elementId - 1}];
   ...
   */
  async screenshot(filePath: string = '/tmp/mogoi-screenshot.png'): Promise<string> {
    await this.ensureConnected();

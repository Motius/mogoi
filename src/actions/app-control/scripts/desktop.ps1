<#
  Mogoi desktop helper — Windows fallback path (no sidecar).
...
public static class MogoiWin32 {
...
try { [MogoiWin32]::MakeDpiAware() } catch { }
...
try {
  switch ($Command) {
    'get-active-window' {
      $win = ConvertFrom-WindowLine -Line ([MogoiWin32]::ActiveWindow())
      Write-Output (ConvertTo-Json -InputObject $win -Compress)
    }
    'list-windows' {
      $wins = @([MogoiWin32]::ListWindows() | ForEach-Object { ConvertFrom-WindowLine -Line $_ })
      Write-Output (ConvertTo-Json -InputObject $wins -Compress)
    }
    'click-at' {
      $p = Read-Payload
      [MogoiWin32]::ClickAt([int]$p.x, [int]$p.y)
    }
    'send-keys' {
    ...
    'capture-window' {
      $p = Read-Payload
      $r = [MogoiWin32]::WindowRect([int]$p.pid)
      Write-Output (Get-RectCaptureBase64 -X $r[0] -Y $r[1] -Width $r[2] -Height $r[3])
    }
    'focus-window' {
      $p = Read-Payload
      [MogoiWin32]::Focus([int]$p.pid)
    }
    'close-window' {
      $p = Read-Payload
      [MogoiWin32]::Close([int]$p.pid)
    }
    'launch-app' {
      $p = Read-Payload

 ...
 * Checks: ~/.mogoi/sidecar/ then repo build output.
 */
  ...
  if (WSLBridge.isWSL()) {
    ...
    try {
      ...
      if (userProfile && !userProfile.includes('%')) {
        ...
        const wslPath = `/mnt/${drive}${rest}/.mogoi/sidecar/desktop-bridge.exe`;
    ...
    candidates.push('/mnt/c/Users/' + (process.env.USER ?? 'user') + '/.mogoi/sidecar/desktop-bridge.exe');
  } else {
    // Native Windows
    candidates.push(join(homedir(), '.mogoi', 'sidecar', 'desktop-bridge.exe'));
  }

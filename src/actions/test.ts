...
async function testActionLayer() {
  console.log('Testing M.O.G.O.I. Action Layer\n');
  ...
  try {
    const result = await executor.execute('echo "Hello from MOGOI"');
  ...
  try {
    const result = await registry.execute('echo', { message: 'Hello MOGOI!' });

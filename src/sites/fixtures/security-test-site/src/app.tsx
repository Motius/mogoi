  ...
  const runTests = async () => {
    ...
    const tests: TestResult[] = [
      // --- #2: Same-Origin Iframe via Proxy Path ---
      {
        ...
        description: "Attempts fetch('/api/health') — if same-origin and no sandbox, this returns Mogoi health data.",
      ...
      },
      {
        ...
        description: "Attempts fetch('/api/config') to steal Mogoi config including API keys.",
        auditRef: "#2 — Same-Origin Iframe via Proxy Path",
        status: "pending",
        detail: "",
      },
      ...
      // --- #3: WebSocket ---
      {
        name: "WebSocket to Mogoi /ws",
        description: "Attempts new WebSocket('/ws') to open a direct WebSocket to the Mogoi daemon.",
      ...
      },
      {
        ...
        description: "Attempts WebSocket to the dashboard's port (:1846/ws) directly rather than relative path.",
    ...
    try {
      ...
      const dashboardKeys = keys.filter(
        (k) => k.startsWith("mogoi") || k === "conversations" || k === "auth" || k === "token"
      );
    ...
    try {
      ...
      const dashboardDbs = names.filter(n => n!.includes("mogoi") || n!.includes("vault"));
    ...
    try {
      ...
      // not from Mogoi. Only flag as vulnerable if the response looks like a Mogoi
      // API response (JSON with "error" or "ok" keys).
      const text = await resp.text();
      const isMogoiResponse = text.includes('"error"') || text.includes('"ok"');
      if (resp.ok && isMogoiResponse) {
        updateResult(i, { status: "vulnerable", detail: `DELETE succeeded: ${text.slice(0, 100)}` });
      } else if (resp.status === 404 && isMogoiResponse) {
        updateResult(i, { status: "vulnerable", detail: `Mogoi API reachable (404): ${text.slice(0, 100)}` });
      } else {
        updateResult(i, { status: "blocked", detail: `Returned ${resp.status} (not a Mogoi API response)` });
      }
    } catch (err) {
      updateResult(i, { status: "blocked", detail: `fetch() threw: ${(err as Error).message}` });
    }
    ...
    try {
      const ws = new WebSocket("ws://localhost:1846/ws");
      await new Promise<void>((resolve) => {
        const timeout = setTimeout(() => { ws.close(); updateResult(19, { status: "blocked", detail: "Connection timed out (3s)" }); resolve(); }, 3000);
        ws.onopen = () => { clearTimeout(timeout); ws.close(); updateResult(19, { status: "vulnerable", detail: "Connected to ws://localhost:1846/ws!" }); resolve(); };
        ws.onerror = () => { clearTimeout(timeout); updateResult(19, { status: "blocked", detail: "WebSocket to :1846 refused" }); resolve(); };
      });
    } catch (err) {
      updateResult(i, { status: "blocked", detail: `Threw: ${(err as Error).message}` });
    }
    ...
    try {
      const bc = new BroadcastChannel("mogoi");
      let received = false;
      ...
      bc.close();
      if (received) {
        updateResult(i, { status: "vulnerable", detail: "Received message on 'mogoi' BroadcastChannel!" });
      } else {
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", maxWidth: 960, margin: "0 auto", padding: 20 }}>
      ...
      <p style={{ color: "#666", fontSize: 14 }}>
        ...
        <br />
        Open this project in the Mogoi Sites page to test both <strong>proxy mode</strong> and{" "}

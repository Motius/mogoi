...
/**
 * Hosted "MotiusAI AI" credentials for the voice factories. Sourced from the
 * SYSTEM-owned `motius_ai` config.yaml block at the call sites (see
 * daemon/motius-ai.ts) and passed as a separate argument on purpose:
...
/**
 * Hosted "MotiusAI AI" STT — the platform proxy's OpenAI-compatible
 ...
 * come from the system-owned `motius_ai` block via the factory's `hosted`
 * argument — never from cfg.stt.
 */
export class MotiusAISTT implements STTProvider {
  ...
  constructor(baseUrl: string, apiKey: string, model: string = 'uj-stt', language?: string) {
    ...
    // exactly like MotiusAIAIProvider (src/llm/motius.ts) does.
  ...
  async transcribe(audio: Buffer): Promise<string> {
    ...
    const response = await fetch(`${this.baseUrl}/audio/transcriptions`, {
      ...
      headers: { 'Authorization': `Bearer ${this.apiKey}` },
      body: formData,
      signal: AbortSignal.timeout(MotiusAISTT.TIMEOUT_MS),
    });

    if (!response.ok) {
      const err = await MotiusAISTT.boundedText(response, MotiusAISTT.MAX_ERROR_BODY_BYTES);
      ...
      throw hostedProxyError('MotiusAI AI STT', response.status, err);
    }
    ...
    const raw = await MotiusAISTT.boundedText(response, MotiusAISTT.MAX_BODY_BYTES);
    let result: { text?: string };
    try {
      result = JSON.parse(raw) as { text?: string };
    } catch {
      throw new Error(
        `MotiusAI AI STT returned a non-JSON body: ${redactSecrets(raw).slice(0, 200)}`,
      );
    }
    if (typeof result.text !== 'string') {
      ...
      throw new Error(
        `MotiusAI AI STT returned no transcript: ${redactSecrets(JSON.stringify(result)).slice(0, 200)}`,
      );
    }
    return result.text;
  }
 ...
 *
 * `hosted` carries the MotiusAI AI proxy credentials as a SEPARATE argument
 * (see HostedVoiceCredentials): cfg.stt only ever stores the string choice
 * `provider: 'motius'`, so the key can never leak into the persisted
...
export function createSTTProvider(
  ...
  switch (config.provider) {
    case 'motius':
      ...
      return new MotiusAISTT(hosted.baseUrl, hosted.apiKey, undefined, config.language);
    case 'openai':
...
/**
 * Hosted "MotiusAI AI" TTS — the platform proxy's OpenAI-compatible
 ...
 * the system-owned `motius_ai` block via the factory's `hosted` argument —
 * never from cfg.tts.
 */
...
export class MotiusAITTS implements TTSProvider {
  private baseUrl: string;
  private apiKey: string;
  private voice: string;

  constructor(baseUrl: string, apiKey: string, voice: string = 'alloy') {
    // Same origin→/v1 normalization as MotiusAISTT / MotiusAIAIProvider.
    const trimmed = baseUrl.replace(/\/+$/, '');
    this.baseUrl = /\/v1$/.test(trimmed) ? trimmed : `${trimmed}/v1`;
    this.apiKey = apiKey;
    this.voice = voice;
  ...
  async synthesize(text: string): Promise<Buffer> {
    ...
    if (text.length > MAX_TTS_INPUT_CHARS) {
      ...
      console.warn(`[MotiusAITTS] Input truncated from ${text.length} to ${input.length} chars (per-request cap; the spoken reply will cut off)`);
    }
    ...
    if (!response.ok) {
      ...
      throw hostedProxyError('MotiusAI AI TTS', response.status, err);
    }

    const arrayBuffer = await response.arrayBuffer();
    const audio = Buffer.from(arrayBuffer);

    // A 200 that isn't audio is the SAME class as MotiusAISTT's
    ...
    if (audio.length > 0 && !contentType.toLowerCase().startsWith('audio/') && !looksLikeMp3(audio)) {
      ...
      throw new Error(
        `MotiusAI AI TTS returned a non-audio body (content-type: ${contentType || 'none'}): ` +
 ...
 * cfg.tts only ever stores the string choice `provider: 'motius'`.
 */
export function createTTSProvider(
  ...
  if (config.provider === 'motius') {
    ...
    return new MotiusAITTS(hosted.baseUrl, hosted.apiKey, voice ?? 'alloy');
  }

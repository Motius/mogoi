...
test('WebSocketServer - health endpoint', async () => {
  ...
  // version reflects the MOGOI_VERSION pin (null when unset, e.g. self-host).
  expect(data).toHaveProperty('version');

  server.stop();
});

test('WebSocketServer - health reports the MOGOI_VERSION pin', async () => {
  const prev = process.env.MOGOI_VERSION;
  process.env.MOGOI_VERSION = '2026.07.01';
  try {
    ...
    server.stop();
    if (prev === undefined) delete process.env.MOGOI_VERSION;
    else process.env.MOGOI_VERSION = prev;
  }
});
...
test('WebSocketServer - dashboard HTML is never cached by a reverse proxy', async () => {
  const staticDir = mkdtempSync(path.join(tmpdir(), 'mogoi-static-'));
  ...
  try {
    await Bun.write(path.join(staticDir, 'index.html'), '<!doctype html><title>Mogoi</title>');
...
test('WebSocketServer - WebSocket connection', async () => {
  ...
  expect(received.payload).toEqual({ echo: 'Hello M.O.G.O.I.' });
...
test('WebSocketServer - tearing down a session hangs up on its live socket and refuses the reconnect', async () => {
  // The end-to-end shape of `mogoi revoke` against an open panel: not just

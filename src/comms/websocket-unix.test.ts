...
describe('resolveListen', () => {
  test('defaults to TCP on daemon.port', () => {
    expect(resolveListen({ port: 1846 })).toEqual({ kind: 'tcp', port: 1846 });
    expect(resolveListen({ port: 1846, listen: '' })).toEqual({ kind: 'tcp', port: 1846 });
  });

  test('parses unix specs and requires an absolute path', () => {
    expect(resolveListen({ port: 1846, listen: 'unix:/run/mogoi/u_42.sock' })).toEqual({
      kind: 'unix',
      path: '/run/mogoi/u_42.sock',
    });
    expect(() => resolveListen({ port: 1846, listen: 'unix:relative.sock' })).toThrow(/absolute/);
  });
  ...
  test('rejects unknown schemes loudly (never a silent TCP fallback)', () => {
    ...
    expect(() => resolveListen({ port: 1846, listen: 'tcp:0.0.0.0:80' })).toThrow(/Unsupported/);
  });
});
...
describe('unix-domain socket listener', () => {
  ...
  test('serves HTTP over the socket, no TCP port bound, 0660 perms, stale file replaced', async () => {
    const sockPath = join(tmpdir(), `mogoi-test-${process.pid}-${Date.now()}.sock`);
...
test('stop() removes the socket file (no dead-but-present socket)', async () => {
  ...
  const sockPath = join(tmpdir(), `mogoi-test-stop-${process.pid}-${Date.now()}.sock`);
...
test('the socket is removed on graceful process exit even when stop() is never called', async () => {
  ...
  // can run late or be cut off by mogoi stop's SIGKILL. A process.on('exit')
  ...
  const { existsSync } = await import('node:fs');
  const sockPath = join(tmpdir(), `mogoi-test-exit-${process.pid}-${Date.now()}.sock`);
  const child = `
...
test('stop() deregisters the exit handler (restart-in-place on a new path is untouched)', async () => {
  ...
  const pathA = join(tmpdir(), `mogoi-test-a-${process.pid}-${Date.now()}.sock`);
  const pathB = join(tmpdir(), `mogoi-test-b-${process.pid}-${Date.now()}.sock`);

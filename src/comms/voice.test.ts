...
describe('createSTTProvider factory', () => {
  ...
  test('returns MotiusAISTT when provider=motius and hosted creds passed', () => {
    const config: STTConfig = { provider: 'motius' };
    const provider = createSTTProvider(config, {
      baseUrl: 'https://llm.motius.host',
      apiKey: 'sk-uj-not-real',
    });
    expect(provider).toBeInstanceOf(MotiusAISTT);
  });

  test('returns null when provider=motius and no hosted creds (self-hosted)', () => {
    const config: STTConfig = { provider: 'motius' };
    expect(createSTTProvider(config)).toBeNull();
    expect(createSTTProvider(config, null)).toBeNull();
    expect(createSTTProvider(config, { baseUrl: '', apiKey: 'sk-uj-not-real' })).toBeNull();
    expect(createSTTProvider(config, { baseUrl: 'https://llm.motius.host', apiKey: '' })).toBeNull();
  });

  test('cfg.stt sub-blocks never feed the motius case (key stays out of user config)', () => {
    ...
    const config = { provider: 'motius', motius: { api_key: 'from-db-row' } } as unknown as STTConfig;
    expect(createSTTProvider(config)).toBeNull();
  });
});
...
describe('createTTSProvider factory', () => {
  ...
  test('returns MotiusAITTS when provider=motius and hosted creds passed', () => {
    const config: TTSConfig = { enabled: true, provider: 'motius' };
    const provider = createTTSProvider(config, {
      baseUrl: 'https://llm.motius.host',
      apiKey: 'sk-uj-not-real',
    });
    expect(provider).toBeInstanceOf(MotiusAITTS);
  });
  ...
  test('returns null for motius when tts disabled, even with creds', () => {
    const config: TTSConfig = { enabled: false, provider: 'motius' };
    expect(createTTSProvider(config, { baseUrl: 'https://llm.motius.host', apiKey: 'k' })).toBeNull();
  });
});
...
describe('MotiusAISTT.transcribe', () => {
  const originalFetch = globalThis.fetch;
  afterEach(() => { globalThis.fetch = originalFetch; });

  test('POSTs multipart WAV to <origin>/v1/audio/transcriptions with bearer + uj-stt', async () => {
    const stt = new MotiusAISTT('https://llm.motius.host', 'sk-uj-not-real');
    const wav = makeWavBuffer();
    let calledUrl = '';
    let auth = '';
    let model = '';
    ...
    expect(calledUrl).toBe('https://llm.motius.host/v1/audio/transcriptions');
  ...
  test('does not double the /v1 prefix when the base already carries it', async () => {
    const stt = new MotiusAISTT('https://llm.motius.host/v1/', 'sk-uj-not-real');
    let calledUrl = '';
  ...
  test('throws with status on HTTP error', async () => {
    ...
    await expect(stt.transcribe(makeWavBuffer())).rejects.toThrow(/MotiusAI AI STT error \(403\)/);
  });
  ...
  test('throws when the response carries no text field', async () => {
    ...
    await expect(stt.transcribe(makeWavBuffer())).rejects.toThrow('MotiusAI AI STT returned no transcript');
  });
});
...
describe('MotiusAITTS', () => {
  ...
  test('POSTs JSON to <origin>/v1/audio/speech with bearer, uj-tts and mp3 format', async () => {
    const tts = new MotiusAITTS('https://llm.motius.host', 'sk-uj-not-real');
    let calledUrl = '';
    let auth = '';
    let contentType = '';
    ...
    expect(calledUrl).toBe('https://llm.motius.host/v1/audio/speech');
    expect(auth).toBe('Bearer sk-uj-not-real');
  ...
  test('does not double the /v1 prefix when the base already carries it', async () => {
    const tts = new MotiusAITTS('https://llm.motius.host/v1', 'sk-uj-not-real');
    let calledUrl = '';
  ...
  test('throws with status on HTTP error', async () => {
    ...
    await expect(tts.synthesize('hi')).rejects.toThrow(/MotiusAI AI TTS error \(429\)/);
  });
  ...
  test('rejects a 200 carrying a CDN interstitial instead of returning it as audio', async () => {
    ...
    const tts = new MotiusAITTS('https://llm.motius.host', key);
    globalThis.fetch = mock(async () => new Response(
      `<html><body>Access denied. token=${key} for llm.motius.host</body></html>`,
  ...
  test('sends an abort signal so a hung proxy cannot wedge the sentence queue', async () => {
    ...
    await new MotiusAITTS('https://llm.motius.host', 'sk-uj-not-real').synthesize('hi');
  ...
  test('factory passes a non-Edge cfg voice through; Edge neural names fall back to alloy', async () => {
    ...
    const hosted = { baseUrl: 'https://llm.motius.host', apiKey: 'sk-uj-not-real' };
    await createTTSProvider({ enabled: true, provider: 'motius', voice: 'nova' }, hosted)!.synthesize('hi');
    ...
    await createTTSProvider({ enabled: true, provider: 'motius', voice: 'en-US-AriaNeural' }, hosted)!.synthesize('hi');
...
describe('magic-byte labeling of the shared STT upload buffer', () => {
  ...
  for (const [label, make] of [
    ...
    ['MotiusAISTT', () => new MotiusAISTT('https://llm.motius.host', 'sk-uj-not-real')],
  ] as const) {
...
describe('hosted STT error redaction', () => {
  ...
  test('never lets key-shaped material out of either STT throw', async () => {
    ...
    const stt = new MotiusAISTT('https://llm.motius.host', 'sk-uj-abc');
...
describe('MotiusAISTT non-JSON 200', () => {
  const originalFetch = globalThis.fetch;
  afterEach(() => { globalThis.fetch = originalFetch; });
...
describe('STT language is configurable (was hardcoded to en everywhere)', () => {
  ...
  test('unset language omits the param entirely — Whisper auto-detects', async () => {
    ...
    expect(await languageSentBy({ provider: 'motius' }, hosted)).toBeNull();
  });
  ...
  test('a configured language reaches every Whisper-shaped provider', async () => {
    ...
    expect(await languageSentBy({ provider: 'motius', language: 'it' }, hosted)).toBe('it');
  });
});

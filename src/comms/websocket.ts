...
export class WebSocketServer {
   ...
   * services before it reaches this one, and `mogoi stop`'s grace window can
  ...
  constructor(port: number = 1846, unixPath?: string) {
    this.port = port;
  ...
  start(): void {
    ...
    this.server = Bun.serve<{ sidecar_id?: string; channel?: string; proxy_target?: string; panel_session?: string; _proxyUpstream?: WebSocket }>({
      ...
      async fetch(req, server) {
        ...
        //    (e.g., dev server iframes on different ports attempting ws://localhost:1846/ws).
        ...
        // process was launched from (systemd sets MOGOI_VERSION from the
        ...
        if (pathname === '/health') {
          return Response.json({
            status: 'ok',
            version: process.env.MOGOI_VERSION ?? null,
        ...
        //    frameworks emit and that don't match any MOGOI route.
        if (self.siteProxy) {

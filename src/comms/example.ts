/**
 * Example usage of M.O.G.O.I. Communication Layer
 *
...
async function main() {
  console.log('🤖 Starting M.O.G.O.I. Communication Layer...\n');

  // 1. Initialize WebSocket server
  const wsServer = new WebSocketServer(1846);
  ...
  const simulatedStream = (async function* () {
    ...
    await new Promise(r => setTimeout(r, 100));
    yield { type: 'text' as const, text: 'M.O.G.O.I.!' };
    await new Promise(r => setTimeout(r, 100));
    yield { type: 'done' as const, response: { content: 'Hello from M.O.G.O.I.!', tool_calls: [], usage: { input_tokens: 0, output_tokens: 10 }, model: 'demo', finish_reason: 'stop' as const } };
  })();

...
export type TelemetryServiceOptions = {
  ...
  sender?: Sender;
  /** Override debug logging (tests). Defaults to the MOGOI_TELEMETRY_DEBUG env. */
  debug?: boolean;
};
...
export class TelemetryService implements Service {
  ...
  constructor(private readonly opts: TelemetryServiceOptions) {
    ...
    this.debug = opts.debug ?? envFlag(process.env.MOGOI_TELEMETRY_DEBUG);
  }
    ...
    if (firstRun) {
      console.log(
        [
          ...
          '| MOGOI sends a small anonymous ping at startup and every hour     |',
          ...
          '| or run with MOGOI_TELEMETRY=0 (DO_NOT_TRACK=1 is honored too).    |',
          '+-------------------------------------------------------------------+',
          '',
        ].join('\n'),
      );
      ...
      console.log(
        '[Telemetry] Anonymous metrics on (opt out: telemetry.enabled=false or MOGOI_TELEMETRY=0).',
      );
    }
  }
}

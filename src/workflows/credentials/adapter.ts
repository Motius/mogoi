 ...
 *
 *   1. If the externalId starts with `mogoi:`, dispatch to a registered
 *      `MogoiConnectionSource` that pulls live from Mogoi' existing stores
 *      (Google OAuth file, Telegram/Discord bot token in config, etc.). This
 *      means a user who already wired Gmail into Mogoi sees a "Mogoi Gmail"
 ...
 *
 * The adapter does not import Mogoi modules directly; sources are injected so
...
/** Prefix that marks a connection as "managed by Mogoi itself". */
export const MOGOI_PREFIX = "mogoi:";
...
/**
 * A source of Mogoi-managed credentials. One per backing store (Google file,
 * channel config, etc.). Sources only handle externalIds with the MOGOI_PREFIX.
 */
export interface MogoiConnectionSource {
...
/** Composite resolver: Mogoi sources first, then the workflow DB. */
export class CredentialResolver {
  private sources: MogoiConnectionSource[] = [];

  register(source: MogoiConnectionSource): void {
  ...
  list(): readonly MogoiConnectionSource[] {
    return this.sources;
  }

  async resolve(input: ResolveInput): Promise<ResolvedConnection | null> {
    if (input.externalId.startsWith(MOGOI_PREFIX)) {
...
// Live Mogoi credential sources -- one per backing store -- live in their

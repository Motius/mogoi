/**
 * Coverage for `MogoiGoogleConnectionSource`. The refresh-token dance
...
import { MogoiGoogleConnectionSource, MOGOI_GOOGLE_PREFIX } from "./google-source";
...
describe("MogoiGoogleConnectionSource", () => {
  test("returns null when GoogleAuth is unauthenticated", async () => {
    ...
    const src = new MogoiGoogleConnectionSource(auth);
    expect(await src.resolve(MOGOI_GOOGLE_PREFIX)).toBeNull();
  });

  test("canResolve matches mogoi:google + mogoi:google:<sub>", () => {
    const src = new MogoiGoogleConnectionSource(makeFakeAuth({ authed: false, freshAccessToken: "x", tokens: null }));
    expect(src.canResolve("mogoi:google")).toBe(true);
    expect(src.canResolve("mogoi:google:gmail")).toBe(true);
    expect(src.canResolve("mogoi:telegram")).toBe(false);
  });
  ...
  test("calls getAccessToken so an expired cache surfaces the refreshed token", async () => {
    ...
    const src = new MogoiGoogleConnectionSource(makeFakeAuth(state));
    const resolved = await src.resolve(MOGOI_GOOGLE_PREFIX);

 ...
 * `workflows.pieces_dir` / `MOGOI_SHARED_PIECES_DIR`. It holds the WHOLE
 ...
 * `~/.mogoi/pieces` stays writable and still SHADOWS the shared copy
...
export function sharedPiecesDir(): string | null {
  const dir = process.env.MOGOI_SHARED_PIECES_DIR?.trim();
 ...
 * Keying off hostedness signals like the `motius_ai` block would be both

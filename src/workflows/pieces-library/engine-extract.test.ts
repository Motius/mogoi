 ...
 *
 * Run with:
 *   MOGOI_TEST_ENGINE_EXTRACT_PIECE=1 bun test src/workflows/pieces-library/engine-extract.test.ts
 *
 * The double gate (`MOGOI_TEST_ENGINE_BUILD` for the engine bundle +
 * `MOGOI_TEST_ENGINE_EXTRACT_PIECE` for this test) keeps the regular
...
const EXTRACT_GATE = process.env.MOGOI_TEST_ENGINE_EXTRACT_PIECE === "1";
const BUILD_GATE = process.env.MOGOI_TEST_ENGINE_BUILD === "1";
...
describe("engine extracts metadata for an npm-installed piece (gated)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) {
      throw new Error(
        "Engine bundle not cached. Pass MOGOI_TEST_ENGINE_BUILD=1 alongside this gate to build one.",
      );
    }

    // Set up a real pieces dir with one piece installed. Same shape as the
    // production `~/.mogoi/pieces/` -- package.json + bun install ->
    // node_modules/@activepieces/piece-<id>/...
    piecesDir = mkdtempSync(join(tmpdir(), "mogoi-engine-extract-"));

/**
 * Pieces installer + manifest. Manages `~/.mogoi/pieces/`:
 *
...
const ROOT_ENV = "MOGOI_PIECES_DIR";

/** Default `~/.mogoi/pieces`. Override with `MOGOI_PIECES_DIR` for tests. */
export function piecesBaseDir(): string {
  const override = process.env[ROOT_ENV];
  if (override) return resolve(override);
  return resolve(homedir(), ".mogoi", "pieces");
}

export interface InstalledPiece {
  /** Mogoi-side id; matches the catalog entry's `id`. */
  id: string;

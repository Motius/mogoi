...
This directory owns the list of activepieces community pieces that Mogoi
users can install at runtime via the Library tab in the Workflows room.
The catalog is split into two tiers:

- **Verified** -- hand-reviewed by a Mogoi maintainer + smoke-tested
...
The catalog is the *only* path by which pieces reach a Mogoi install.
...
3. Daemon writes the requested piece into `~/.mogoi/pieces/installed.json`,
   then synthesizes `~/.mogoi/pieces/package.json` from the manifest and
   runs `bun install` in that directory.
4. Bun resolves `^0.12.2` against npm to a concrete version (e.g. 0.12.3)
   and places it under `~/.mogoi/pieces/node_modules/@activepieces/piece-gmail/`.
...
Docker: `~/.mogoi` should be mounted as a persistent volume. The manifest
...
`MOGOI_SHARED_PIECES_DIR`). `piecesManagedByHost()` in `shared.ts` is the
...
`~/.mogoi/pieces` is still writable and still shadows the shared tree
...
- A piece installed into `~/.mogoi/pieces` BEFORE the instance became managed
...
- Managedness rides on the RESOLVED dir, so it fails OPEN if the path cannot
  be resolved. A host writes `pieces_dir: /opt/mogoi-pieces/${version}`, and
  `resolveSharedRuntimePaths` refuses to expand `${version}` unless
  `MOGOI_VERSION` names a concrete version — which it does not if the
  per-instance env file is missing and systemd falls back to its
  `MOGOI_VERSION=current` default. That instance reads as self-managed: full
...
```ts
{
  id: "gmail",                        // stable Mogoi-side id; do not rename
  ...
  vettedVersion: "0.12.3",            // exact version Mogoi verified
...
upstream patch fixes between Mogoi releases.
...
3. **Native-deps check.** If transitive deps include native bindings,
   ...
   Anything found needs verification that it loads under the Mogoi Bun
...
5. **Gated install integration test**:
   ...
   ```sh
   MOGOI_TEST_PIECES_LIBRARY=1 bun test src/workflows/pieces-library/integration.test.ts
   ```
...
- A future Mogoi release widens it back once upstream fixes.
...
uninstall explicitly via the Library UI; future Mogoi releases can ship a

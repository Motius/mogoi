 ...
 * `require()`d. Skipped unless `MOGOI_TEST_PIECES_LIBRARY=1` because it
 ...
 *     the engine-end-to-end suite for Mogoi-native pieces.
 *
...
const ENABLED = process.env.MOGOI_TEST_PIECES_LIBRARY === "1";
...
beforeEach(() => {
  if (!ENABLED) return;
  tempDir = mkdtempSync(join(tmpdir(), "mogoi-pieces-integration-"));
});
...
describe("pieces-library integration (gated on MOGOI_TEST_PIECES_LIBRARY=1)", () => {

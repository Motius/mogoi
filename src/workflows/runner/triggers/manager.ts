 ...
 *         direct-subscribe path for `mogoi-trigger:on_event` (kept until
...
export interface TriggerManagerDeps {
  /**
   * In-process event bus. Used by the legacy `mogoi-trigger:on_event`
   ...
   * supported is `mogoi-trigger:on_event`, which falls back to direct
...
export class TriggerManager {
  ...
  private async register(flow: FlowRow): Promise<void> {
    ...
    if (trigger.type === "PIECE_TRIGGER") {
      ...
      // Engine-less fallback for mogoi-trigger:on_event. In
      ...
      // `manager.test.ts:"TriggerManager: mogoi-trigger on_event"`.
      ...
      // engine.
      if (pieceName === "mogoi-trigger") {
        return this.registerMogoiEvent(flow.id, versionId, trigger);
      }
      this.log(
  ...
  private registerMogoiEvent(flowId: string, versionId: string, trigger: TriggerNode): void {
    if (trigger.settings?.triggerName !== "on_event") {
      this.log(
        `flow ${flowId}: mogoi-trigger has triggerName="${trigger.settings?.triggerName}"; only "on_event" is supported`,
      );
      return;
    }
    const input = (trigger.settings?.input ?? {}) as Record<string, unknown>;
  ...
  /**
   * Engine-managed trigger fire (polling sources, e.g. mogoi-trigger

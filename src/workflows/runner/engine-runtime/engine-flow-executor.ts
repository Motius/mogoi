 ...
 *
 * Override via `MOGOI_WORKFLOW_TERMINAL_TIMEOUT_MS` for unusual environments.
 */
const DEFAULT_TERMINAL_TIMEOUT_MS = parsePositiveIntEnv(
  process.env["MOGOI_WORKFLOW_TERMINAL_TIMEOUT_MS"],
  60_000,
);
...
export interface EngineFlowExecutorOptions {
   ...
   * (`~/.mogoi/workflow-logs/`). Tests pass a per-test temp dir to avoid
   * mutating `process.env.MOGOI_WORKFLOW_DATA_DIR`.
   */
...
export class EngineFlowExecutor implements FlowExecutor {
  ...
  async execute(ctx: FlowExecutorContext): Promise<FlowExecutorResult> {
    ...
    try {
      ...
      // On Mogoi's local-first, single-user scale this is negligible
      ...
      //
      // Override via `MOGOI_WORKFLOW_STREAM_STEP_PROGRESS=NONE` for
      ...
      const streamStepProgress: "WEBSOCKET" | "NONE" =
        process.env["MOGOI_WORKFLOW_STREAM_STEP_PROGRESS"] === "NONE"
          ? "NONE"
          : "WEBSOCKET";

/**
 * Bundle each Mogoi-authored piece (under `packages/pieces/mogoi/*`) into
...
async function ensureStagingInstalled(): Promise<void> {
  ...
  // `buildAllMogoiPieces()` in parallel, and if pieces won the race
...
/**
 * Build every piece directly under `packages/pieces/mogoi/`. Returns the
 ...
 */
export async function buildAllMogoiPieces(
  opts: BuildPieceOptions = {},
): Promise<BuildPieceResult[]> {
...
export const PIECE_BUILD_PATHS = {
  mogoiPiecesRoot: resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/mogoi"),
} as const;

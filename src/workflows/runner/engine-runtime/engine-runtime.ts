...
import type { FlowVersion as MogoiFlowVersion } from "../../db/repos/flow-version";
...
export interface EngineRuntimeOptions {
   ...
   * `~/.mogoi/workflow-codes`. The engine appends `${flowVersionId}/${stepName}/index.js`.
   */
  baseCodeDir?: string;
  /**
...
export interface ExecuteFlowOnHandleOptions {
  ...
  flowVersion: MogoiFlowVersion | UpstreamFlowVersion;
 ...
 * dir").
 */
function discoverMogoiDevPieces(): string[] {
  ...
  for (const name of readdirSync(root).sort()) {
    ...
    try {
      ...
      if (typeof pkg.name === "string" && pkg.name.length > 0) {
        ...
        // expects: `@mogoipieces/piece-mogoi-foo` -> `mogoi-foo`.
...
function envHandshakeTimeoutMs(): number | undefined {
  const raw = process.env["MOGOI_ENGINE_HANDSHAKE_TIMEOUT_MS"]?.trim();
  if (!raw) return undefined;
  const parsed = Number.parseInt(raw, 10);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    console.warn(
      `[engine-runtime] ignoring invalid MOGOI_ENGINE_HANDSHAKE_TIMEOUT_MS="${raw}"`,
    );
    return undefined;
  }
  return parsed;
...
export class EngineRuntime {
  ...
  constructor(opts: EngineRuntimeOptions) {
    ...
    // packages/pieces/mogoi/*/dist/package.json without further env setup.
    ...
    //
    // Auto-discover Mogoi pieces from the vendored tree so adding a
    ...
    this.devPieces = opts.devPieces ?? discoverMogoiDevPieces();
  }
      ...
      if (handshakeMs > SLOW_HANDSHAKE_WARN_MS) {
        console.warn(
          `[engine-runtime] engine ${sandboxId.slice(0, 8)} handshake took ${handshakeMs}ms ` +
            ...
            `MOGOI_ENGINE_HANDSHAKE_TIMEOUT_MS if acquires start timing out.`,
        );
      }
...
function isUpstreamFlowVersion(
  v: MogoiFlowVersion | UpstreamFlowVersion,

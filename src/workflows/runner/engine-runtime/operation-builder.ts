 ...
 *
 * Override via `MOGOI_WORKFLOW_FLOW_TIMEOUT_SECONDS` for deployments that
 * want to fail long flows sooner (or allow longer ones).
 */
export const DEFAULT_TIMEOUT_S = parsePositiveIntEnv(
  process.env["MOGOI_WORKFLOW_FLOW_TIMEOUT_SECONDS"],
  600,
);
...
export interface ExecuteTriggerHookOptions {
   ...
   * non-webhook triggers (most mogoi triggers) still validate.
   */
  webhookUrl?: string;

 ...
 * and gated here on `MOGOI_TEST_ENGINE_BUILD=1` because it pulls ~30MB of
...
describe("engine bundle build", () => {
  ...
  describe("shared bundle root (MOGOI_ENGINE_CACHE_ROOT)", () => {
    afterEach(() => {
      delete process.env.MOGOI_ENGINE_CACHE_ROOT;
    });
    ...
    test("a prebuilt shared bundle is found WITHOUT any staging dir precondition", async () => {
      ...
      try {
        process.env.MOGOI_ENGINE_CACHE_ROOT = root;
  ...
  test.skipIf(process.env.MOGOI_TEST_ENGINE_BUILD !== "1")(

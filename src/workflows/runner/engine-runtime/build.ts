 ...
 *
 * Staging lives outside the repo (under `~/.mogoi/cache/engine-build`) so
...
const CACHE_ROOT = resolve(homedir(), ".mogoi/cache");
...
function sharedBundleRoot(explicit?: string | null): string | null {
  ...
  const dir = process.env.MOGOI_ENGINE_CACHE_ROOT?.trim();
 ...
 * from MOGOI (e.g. axios in pieces/common feeds the HTTP node, which webhook
...
function buildStagingPackageJson(): string {
  ...
  return JSON.stringify(
    {
      name: "mogoi-engine-build-staging",
      private: true,
      type: "commonjs",
      dependencies: deps,
...
const PATCHED_VENDOR_SOURCES = [
  ...
  // Mogoi-only `outputSample` extension on actions + the matching
  ...
  // Mogoi-only BranchOperator additions (TEXT_MATCHES_REGEX +
  ...
  // Mogoi: the executeFromTrigger short-circuit that runs a flow's steps when
  ...
  // Mogoi: upstream sets process.env.NODE_TLS_REJECT_UNAUTHORIZED='0'

/**
 * Phase F end-to-end smoke: spawn the engine, load Mogoi pieces from disk,
 ...
 * of the Mogoi pieces is mechanical. Skipped when the engine bundle isn't on
...
import { buildAllMogoiPieces } from "./build-pieces";
...
const PIECE_TEST_NAME = "@mogoipieces/piece-mogoi-test";
const PIECE_ASK_NAME = "@mogoipieces/piece-mogoi-ask";
const PIECE_TOOL_NAME = "@mogoipieces/piece-mogoi-tool";
const PIECE_NOTIFY_NAME = "@mogoipieces/piece-mogoi-notify";
const PIECE_CONTEXT_NAME = "@mogoipieces/piece-mogoi-context";
const PIECE_AGENT_NAME = "@mogoipieces/piece-mogoi-agent";
const PIECE_TRIGGER_NAME = "@mogoipieces/piece-mogoi-trigger";
...
describe("Engine end-to-end (F gate)", () => {
  ...
  test.skipIf(skipE2eTests)(
    "manual trigger + mogoi-ask action calls daemon's /v1/mogoi/llm/chat",
    async () => {
      llmCalls = [];
      const flow = createFlow({ projectId: DEFAULT_IDS.project });
      const trigger: FlowTriggerNode = {
...
describe("Engine end-to-end (G+H pieces)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) return;
    if (buildOptIn) await buildAllMogoiPieces();
    runtime = new EngineRuntime({ api, bundlePath: cached.bundlePath });
  });

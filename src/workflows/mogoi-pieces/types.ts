/**
 * Mogoi-native pieces -- shared types.
 *
 * These pieces live in Mogoi (not in the vendored activepieces tree) and use
 ...
 *
 * When engine integration lands, each Mogoi piece is wrapped in upstream's
 * `createPiece({ actions: [createAction(...)] })` by a thin adapter. The
 * adapter is mechanical; the action handlers don't need to change.
 *
 * Naming:
 *   piece.name      = "mogoi-ask" (kebab; matches activepieces convention)
 *   action.name     = "ask"        (verb; the operation)
 *   workflow ref    = "mogoi-ask:ask"
 */

export interface MogoiPieceContext {
 ...
 */
export interface MogoiPieceServices {
  llm?: PieceLlmClient;
 ...
 * adapts the Mogoi event reactor (M5/M13) to this surface.
 *
 ...
 */
export interface MogoiTrigger<I = unknown> {
  name: string;
  displayName: string;
  description: string;
  ...
  subscribe: (input: I, ctx: MogoiTriggerContext) => Promise<TriggerSubscription>;
}

export interface MogoiTriggerContext extends MogoiPieceContext {
...
/**
 * Read-only surface over Mogoi state used by `mogoi-context`. The daemon's
...
/**
 * Channel-aware delivery surface used by `mogoi-notify`. The daemon's
...
/**
 * Minimal slice of the Mogoi tool registry that pieces use. The daemon's
 ...
 *
 * Keep this set small and Mogoi-native rather than mirroring activepieces'
 ...
 */
export interface MogoiAction<I = unknown, O = unknown> {
  name: string;
  displayName: string;
  description: string;
  inputSchema?: PieceInputSchema;
  ...
  parseInput: (raw: unknown) => I;
  execute: (input: I, ctx: MogoiPieceContext) => Promise<O>;
}

export interface MogoiPiece {
  name: string;
  displayName: string;
  description: string;
  actions: Record<string, MogoiAction>;
  triggers?: Record<string, MogoiTrigger>;
}

/**
 * In-memory registry of Mogoi-native pieces. Lookup is by piece name +
 * action name. Used by the future engine adapter and by tests.
 */
export class MogoiPieceRegistry {
  private readonly pieces: Map<string, MogoiPiece> = new Map();

  register(piece: MogoiPiece): void {
  ...
  get(name: string): MogoiPiece | null {
  ...
  resolveAction(reference: string): MogoiAction | null {
  ...
  resolveTrigger(reference: string): MogoiTrigger | null {
    const colon = reference.indexOf(":");
    if (colon < 0) return null;
    const pieceName = reference.slice(0, colon);
...
export class MogoiActionInputError extends Error {
  override readonly name = "MogoiActionInputError";
}
 ...
 */
function validatePiece(piece: MogoiPiece): void {

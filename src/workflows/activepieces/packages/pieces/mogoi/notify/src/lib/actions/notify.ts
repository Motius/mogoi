 ...
 * `/v1/mogoi/notify` and surface the delivery report.
 *
...
export const notifyAction = createAction({
  name: "notify",
  displayName: "Send a Mogoi notification",
  description:
    "Deliver a message through Mogoi's configured channels. Use 'auto' to let Mogoi pick a sensible default given the priority.",
  ...
  props: {
    ...
    channels: Property.StaticMultiSelectDropdown({
      ...
      description:
        "Empty / [auto] lets Mogoi pick a default fan-out across the user's connected channels.",
      required: false,
      defaultValue: ["auto"],
      options: {
        disabled: false,
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/mogoi/notify";
    const message = context.propsValue["message"];
    if (typeof message !== "string" || message.length === 0) {
      throw new Error("mogoi-notify: message is required and must be a non-empty string");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `mogoi-notify: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }

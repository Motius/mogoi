/**
 * `ask` action -- POST to the daemon's `/v1/mogoi/llm/chat` endpoint with
...
export const askAction = createAction({
  name: "ask",
  displayName: "Ask",
  description: "Send a prompt to Mogoi's LLM and receive the reply.",
  ...
  props: {
    ...
    system: Property.LongText({
      displayName: "System",
      description:
        "Extra system instructions APPENDED to the standard Mogoi system prompt. Use this to bias the reply (e.g. \"answer in JSON\"). Turn on Override to replace the Mogoi prompt entirely instead.",
      required: false,
    }),
    overrideSystem: Property.Checkbox({
      displayName: "Override Mogoi system prompt",
      description:
        "When ON, the System field above becomes the ONLY system prompt -- Mogoi's identity, role, personality, and vault context are not sent. Use for generic LLM tasks (text transforms, summarisation of plain inputs) where Mogoi context would bias the reply. Leave OFF (default) when you want the model to answer as Mogoi.",
      required: false,
      defaultValue: false,
    }),
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/mogoi/llm/chat";
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `mogoi-ask: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }

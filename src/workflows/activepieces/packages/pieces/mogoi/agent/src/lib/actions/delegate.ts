 ...
 * `/v1/mogoi/agent/delegate` and surface the agent's final message + tool-call
...
export const delegateAction = createAction({
  name: "delegate",
  displayName: "Delegate to a Mogoi sub-agent",
  description:
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/mogoi/agent/delegate";
    const goal = context.propsValue["goal"];
    if (typeof goal !== "string" || goal.length === 0) {
      throw new Error("mogoi-agent: goal is required and must be a non-empty string");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `mogoi-agent: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }

 ...
 * `/v1/mogoi/workflows/start`. Fire-and-forget: returns the started
 * run id.
 *
 ...
 *
 * The daemon route at `sandbox-api/routes/mogoi-workflows.ts` is the
...
export const runWorkflowAction = createAction({
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/mogoi/workflows/start";
    ...
    if (typeof flowId !== "string" || flowId.length === 0) {
      throw new Error("mogoi-trigger.run_workflow: `flow` is required");
    }
    const body: Record<string, unknown> = { flowId };
    if (payload !== undefined && payload !== null) {
      if (typeof payload !== "object" || Array.isArray(payload)) {
        throw new Error("mogoi-trigger.run_workflow: payload must be a JSON object if provided");
      }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `mogoi-trigger.run_workflow: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }

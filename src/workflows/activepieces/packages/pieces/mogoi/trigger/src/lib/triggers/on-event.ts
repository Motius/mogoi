/**
 * `on_event` polling trigger -- fires the workflow when a Mogoi event of the
 ...
 *
 * Why not sub-minute: Mogoi's own CronScheduler supports an `@every <n>(s|m|h)`
 * extension, BUT the engine validates a trigger's cron with `cron-validator`'s
 * `isValidCron` inside `setSchedule` BEFORE Mogoi's scheduler ever sees it --
...
const CURSOR_KEY = "mogoi-trigger:on-event:since";
...
async function poll(
  ...
  const url = trimSlash(apiUrl) + "/v1/mogoi/events/poll";
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `mogoi-trigger.on_event: daemon responded ${response.status}: ${text.slice(0, 500)}`,
    );
  }
...
export const onEventTrigger = createTrigger({
  name: "on_event",
  displayName: "On Mogoi event",
  description:
    "Fire the workflow when a Mogoi event of the given type is published. Use the daemon's event-type catalog (awareness.*, commitment.*, voice.*, tool.*) to pick a value.",
  type: TriggerStrategy.POLLING,
  props: {
    eventType: Property.ShortText({
      displayName: "Event type",
      description: "Fully-qualified Mogoi event type (e.g. awareness.context_changed).",
      required: true,
    }),

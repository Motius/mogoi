/**
 * `invoke` action -- POST to `/v1/mogoi/tools/invoke` with `{ toolName, params }`
...
export const invokeAction = createAction({
  name: "invoke",
  displayName: "Invoke a Mogoi tool",
  description:
    "Call a registered Mogoi tool by name with the given parameters. Returns the tool's raw result.",
  // The envelope is always `{ result, toolName }`; the shape of
  // `result` is the called tool's concern. We declare a string here
  // as a representative example -- many Mogoi tools return a
  ...
  props: {
    toolName: Property.ShortText({
      ...
      description:
        "Exact id of the registered Mogoi tool (e.g. run_command, vault_search).",
      required: true,
    }),
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/mogoi/tools/invoke";
    const toolName = context.propsValue["toolName"];
    const params = context.propsValue["params"] ?? {};
    if (typeof toolName !== "string" || toolName.length === 0) {
      throw new Error("mogoi-tool: toolName is required and must be a non-empty string");
    }
    if (typeof params !== "object" || params === null || Array.isArray(params)) {
      throw new Error("mogoi-tool: params must be a JSON object");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `mogoi-tool: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }

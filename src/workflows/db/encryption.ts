 ...
 *
 * Key sourcing:
 *   1. `MOGOI_WORKFLOW_ENCRYPTION_KEY` env var (64-char hex = 32 bytes).
 *   2. Otherwise: generate a fresh random key on first call and persist it
 *      to `~/.mogoi/cache/workflow-encryption.key` with 0600 perms. Same
...
const DEFAULT_KEY_FILE = resolve(homedir(), ".mogoi", "cache", "workflow-encryption.key");
...
function getKey(): Buffer {
  ...
  const env = process.env["MOGOI_WORKFLOW_ENCRYPTION_KEY"];
  if (env) {
    if (!/^[0-9a-fA-F]{64}$/.test(env)) {
      throw new Error(
        "MOGOI_WORKFLOW_ENCRYPTION_KEY must be 64 hex characters (32 bytes); got length " + env.length,
      );
    }
    cachedKey = Buffer.from(env, "hex");
    return cachedKey;
  }
  const file = process.env["MOGOI_WORKFLOW_ENCRYPTION_KEY_FILE"] ?? DEFAULT_KEY_FILE;
  if (existsSync(file)) {

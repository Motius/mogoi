 ...
 *
 * Plus a parallel set of `/v1/mogoi/*` endpoints for our own ported pieces
 * (added in F-H).
 *
...
import {
  createMogoiLlmChatRoute,
  type LlmChatFn,
} from "./routes/mogoi-llm";
import {
  createMogoiToolsInvokeRoute,
  type ToolsInvokeFn,
} from "./routes/mogoi-tools";
import {
  createMogoiNotifyRoute,
  type NotifyFn,
} from "./routes/mogoi-notify";
import {
  createMogoiContextVaultSearchRoute,
  createMogoiContextVaultGetEntityRoute,
  createMogoiContextAwarenessRecentRoute,
  createMogoiContextCommitmentsListRoute,
  type MogoiContextProvider,
} from "./routes/mogoi-context";
import {
  createMogoiAgentDelegateRoute,
  type AgentDelegateFn,
} from "./routes/mogoi-agent";
import {
  createMogoiEventsPollRoute,
  type EventsPollFn,
} from "./routes/mogoi-events";
import {
  createMogoiWorkflowsStartRoute,
  type WorkflowsStartFn,
} from "./routes/mogoi-workflows";
...
export interface SandboxApiServices {
  ...
  /**
   * LLM chat backend for the `mogoi-ask` piece. If unset, the endpoint
   * returns 503; the daemon wires this in at startup with the LLMManager.
   */
  llmChat?: LlmChatFn;
  /**
   * Tool invocation backend for the `mogoi-tool` piece. If unset, the
   * endpoint returns 503. The daemon wires this in with `ToolRegistry`.
   */
  toolsInvoke?: ToolsInvokeFn;
  /**
   * Notification backend for the `mogoi-notify` piece. If unset, returns 503.
   */
  notify?: NotifyFn;
  /**
   * Read-only Mogoi state provider for the `mogoi-context` piece. If unset,
   * each context route returns 503.
   */
  contextProvider?: MogoiContextProvider;
  /**
   * Sub-agent delegation backend for the `mogoi-agent` piece. If unset, the
   * endpoint returns 503.
   */
  agentDelegate?: AgentDelegateFn;
  /**
   * Recent-events poll backend for the `mogoi-trigger` `on_event` trigger.
   * If unset, returns 503.
   */
  eventsPoll?: EventsPollFn;
  /**
   * Workflow start backend for the `mogoi-trigger` `run_workflow` action.
   * If unset, returns 503.
   */
...
export class SandboxApi {
  ...
  /**
   * Patch service backends after construction. Each `/v1/mogoi/*` route's
  ...
  private registerRoutes(): void {
    this.routes.push(
      ...
      // /v1/mogoi/* routes pass `this.services` itself (not a snapshot) so
      ...
      {
        path: "/v1/mogoi/llm/chat",
        method: "POST",
        handler: createMogoiLlmChatRoute(this.services),
      },
      {
        path: "/v1/mogoi/tools/invoke",
        method: "POST",
        handler: createMogoiToolsInvokeRoute(this.services),
      },
      {
        path: "/v1/mogoi/notify",
        method: "POST",
        handler: createMogoiNotifyRoute(this.services),
      },
      {
        path: "/v1/mogoi/context/vault-search",
        method: "POST",
        handler: createMogoiContextVaultSearchRoute(this.services),
      },
      {
        path: "/v1/mogoi/context/vault-get-entity",
        method: "POST",
        handler: createMogoiContextVaultGetEntityRoute(this.services),
      },
      {
        path: "/v1/mogoi/context/awareness-recent",
        method: "POST",
        handler: createMogoiContextAwarenessRecentRoute(this.services),
      },
      {
        path: "/v1/mogoi/context/commitments-list",
        method: "POST",
        handler: createMogoiContextCommitmentsListRoute(this.services),
      },
      {
        path: "/v1/mogoi/agent/delegate",
        method: "POST",
        handler: createMogoiAgentDelegateRoute(this.services),
      },
      {
        path: "/v1/mogoi/events/poll",
        method: "POST",
        handler: createMogoiEventsPollRoute(this.services),
      },
      {
        path: "/v1/mogoi/workflows/start",
        method: "POST",
        handler: createMogoiWorkflowsStartRoute(this.services),
      },
    );
  }

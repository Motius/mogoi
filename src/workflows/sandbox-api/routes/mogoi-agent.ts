/**
 * `/v1/mogoi/agent/delegate` -- backs the `mogoi-agent` piece's `delegate`
...
export interface MogoiAgentRouteDeps {
  agentDelegate?: AgentDelegateFn;
}

export function createMogoiAgentDelegateRoute(
  deps: MogoiAgentRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.agentDelegate) {
      return err("mogoi agent.delegate not configured", 503);
    }

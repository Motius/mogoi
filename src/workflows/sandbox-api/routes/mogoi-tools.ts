/**
 * `/v1/mogoi/tools/invoke` -- backs the `mogoi-tool` piece's `invoke` action.
 *
...
export interface MogoiToolsRouteDeps {
  toolsInvoke?: ToolsInvokeFn;
}

export function createMogoiToolsInvokeRoute(
  deps: MogoiToolsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.toolsInvoke) {
      return err("mogoi tools.invoke not configured", 503);
    }

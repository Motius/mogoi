/**
 * `/v1/mogoi/events/poll` -- backs the `mogoi-trigger` `on_event` polling
 * trigger.
 *
 * Two request shapes:
 *
...
export interface MogoiEventsRouteDeps {
  eventsPoll?: EventsPollFn;
}

export function createMogoiEventsPollRoute(
  deps: MogoiEventsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.eventsPoll) {
      return err("mogoi events.poll not configured", 503);
    }

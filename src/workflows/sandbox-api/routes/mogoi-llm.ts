/**
 * `/v1/mogoi/llm/chat` -- backs the `mogoi-ask` piece's `ask` action.
 *
 ...
 *   - default                   : Mogoi identity + role + personality
 *   - `system` set              : Mogoi prompt + "\n\n" + `system`
 *   - `system` + overrideSystem : `system` only (Mogoi context dropped)
 *
...
export interface LlmChatRequest {
  ...
  system?: string;
  /**
   * When true, the `system` field replaces the Mogoi system prompt
   * entirely. When false / unset, `system` is appended to the Mogoi
   * prompt. Default off so the common case ("ask Mogoi to do X") still
   * carries the Mogoi identity.
   */
...
export interface MogoiLlmRouteDeps {
  /**
...
export function createMogoiLlmChatRoute(deps: MogoiLlmRouteDeps): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.llmChat) {
      return err("mogoi llm chat not configured", 503);
    }

/**
 * `/v1/mogoi/notify` -- backs the `mogoi-notify` piece's `notify` action.
 *
...
export interface MogoiNotifyRouteDeps {
  notify?: NotifyFn;
}

export function createMogoiNotifyRoute(
  deps: MogoiNotifyRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.notify) {
      return err("mogoi notify not configured", 503);
    }

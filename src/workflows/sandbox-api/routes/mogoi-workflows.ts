/**
 * `/v1/mogoi/workflows/start` -- backs the `mogoi-trigger` piece's
...
export interface MogoiWorkflowsRouteDeps {
  workflowsStart?: WorkflowsStartFn;
}

export function createMogoiWorkflowsStartRoute(
  deps: MogoiWorkflowsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.workflowsStart) {
      return err("mogoi workflows.start not configured", 503);
    }
    ...
    try {
      ...
      // Typed errors from MogoiWorkflowRunnerAdapter carry a `code`

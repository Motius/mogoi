/**
 * `/v1/mogoi/context/*` -- backs the `mogoi-context` piece's four read-only
 * actions. One route per action so each can validate its own envelope; all
 * four delegate to a single injected `MogoiContextProvider` shaped exactly
 * like the legacy adapter, so the daemon can supply a `MogoiContextProviderAdapter`
 * verbatim.
 *
 * Routes:
 *   POST /v1/mogoi/context/vault-search
 *   POST /v1/mogoi/context/vault-get-entity
 *   POST /v1/mogoi/context/awareness-recent
 *   POST /v1/mogoi/context/commitments-list
 */
...
export interface MogoiContextProvider {
...
export interface MogoiContextRouteDeps {
...
export function createMogoiContextVaultSearchRoute(
  deps: MogoiContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("mogoi context not configured", 503);
...
export function createMogoiContextVaultGetEntityRoute(
  deps: MogoiContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("mogoi context not configured", 503);
...
export function createMogoiContextAwarenessRecentRoute(
  deps: MogoiContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("mogoi context not configured", 503);
...
export function createMogoiContextCommitmentsListRoute(
  deps: MogoiContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("mogoi context not configured", 503);

 ...
 *   1. `mogoi:*` external ids -> live Mogoi-managed credential sources
 ...
 *
 * Per CredentialResolver contract for Mogoi-managed Google connections, the
...
export function createConnectionsRoute(deps: ConnectionsRouteDeps): RouteHandler {
  return async (ctx: RouteContext) => {
    ...
    // needs the pieceName for app_connection lookups. For mogoi:* external

 ...
 *   - No handler-side authn/authz: Mogoi is single-tenant, the dashboard is
...
export interface CreateWorkflowRoutesOptions {
  ...
  /**
   * Optional piece catalog. Either the legacy `MogoiPieceRegistry` (during
   ...
   * report which `MogoiConnectionSource` adapters are registered (e.g.
   * `mogoi:google` is wired) so the dashboard's piece-side auth picker
   * can highlight reusable Mogoi-managed credentials. The repo-backed
   ...
   *
   * When omitted, install/uninstall still mutate `~/.mogoi/pieces/` and the
 ...
 * race on the shared `~/.mogoi/pieces/package.json` + bun-install
...
export function createWorkflowRoutes(opts: CreateWorkflowRoutesOptions = {}): WorkflowRouteMap {
  ...
  return {
        ...
        trapErrors(() => {
          ...
          const list = opts.pieceRegistry.list().map((p) => ({
            ...
            actions: Object.values(p.actions).map((a) => ({
              ...
              // Optional declared output sample (Mogoi extension to AP).
            ...
            triggers: p.triggers
              ? Object.values(p.triggers).map((t) => ({
                  ...
                  // Dynamic-output triggers (mogoi-trigger:on_event):
    ...
    // available" badges. Install / uninstall mutate `~/.mogoi/pieces/`
    ...
    // `~/.mogoi/pieces` is still writable, so nothing but this guard stops a
    ...
    // Event-buffer overflow signal for the `mogoi-trigger:on_event`
    ...
    // ------------------------------------------------------------- connections
    // CRUD over `app_connection` rows + a list of registered Mogoi
    ...
    "/api/workflows/connections": {
      GET: () =>
        trapErrors(() => {
          ...
          return ok({ connections: list, mogoiSources: sources });
        }),
      POST: (req) =>
        trapErrors(async () => {

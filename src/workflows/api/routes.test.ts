...
describe("workflow API: piece catalog", () => {
  ...
  test("returns registered pieces with actions and triggers", async () => {
    ...
    expect(names).toEqual(["mogoi-ask", "mogoi-notify", "mogoi-trigger"]);
    const trigger = (body as Array<{ name: string; triggers: Array<{ name: string }> }>).find((p) => p.name === "mogoi-trigger");
    expect(trigger?.triggers.map((t) => t.name)).toEqual(["on_event"]);
    const ask = (body as Array<{ name: string; actions: Array<{ name: string }> }>).find((p) => p.name === "mogoi-ask");
...
describe("workflow API: pieces library on a MANAGED install", () => {
   ...
   * touch the tenant's own writable `~/.mogoi/pieces` -- which is the only
   * thing stopping a hand-rolled POST, since that directory stays writable.
   */
  const MANAGED_DIR = "/opt/mogoi-pieces/1.2.3";

  /** A temp MOGOI_PIECES_DIR, so a developer's own installs can't leak in. */
  async function withTempPiecesDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
    const { mkdtempSync, rmSync } = await import("node:fs");
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");
    const dir = mkdtempSync(join(tmpdir(), "mogoi-lib-managed-"));
    const prev = process.env.MOGOI_PIECES_DIR;
    process.env.MOGOI_PIECES_DIR = dir;
    try {
      return await fn(dir);
    } finally {
      if (prev === undefined) delete process.env.MOGOI_PIECES_DIR;
      else process.env.MOGOI_PIECES_DIR = prev;
 ...
 * `sharedPiecesDir`, `piecesManagedByHost` consults MOGOI_SHARED_PIECES_DIR,
...
describe("workflow API: pieces library", () => {
  test("GET /api/workflows/pieces/library returns the catalog with per-entry installed status", async () => {
    ...
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");
    const tempDir = mkdtempSync(join(tmpdir(), "mogoi-lib-api-"));
    const prev = process.env.MOGOI_PIECES_DIR;
    process.env.MOGOI_PIECES_DIR = tempDir;
    try {
  ...
  test("DELETE works for a piece installed but no longer in the catalog (security-yank scenario)", async () => {
    ...
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");
    const tempDir = mkdtempSync(join(tmpdir(), "mogoi-lib-delete-"));
    const prev = process.env.MOGOI_PIECES_DIR;
    process.env.MOGOI_PIECES_DIR = tempDir;
    try {
  ...
  test("DELETE returns 404 only when neither catalog nor manifest knows the id", async () => {
    ...
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");
    const tempDir = mkdtempSync(join(tmpdir(), "mogoi-lib-delete-404-"));
    const prev = process.env.MOGOI_PIECES_DIR;
    process.env.MOGOI_PIECES_DIR = tempDir;
    try {
      const r = createWorkflowRoutes(SELF_MANAGED);

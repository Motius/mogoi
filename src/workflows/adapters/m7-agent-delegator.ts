/**
 * `M7AgentDelegator` -- backs `mogoi-agent.delegate` with the full M7 sub-agent
 ...
 * action should expose it via a separate piece (e.g., `mogoi-tool` or
 * `mogoi-notify` + waitpoint), not piggy-back on the sub-agent.
 */

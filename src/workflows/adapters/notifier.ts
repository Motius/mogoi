/**
 * Adapter: PieceNotifier over Mogoi' ChannelService + WebSocketService +
 ...
 *                  dropped here so a vanilla Mogoi without telegram doesn't
...
export interface NotifierDeps {
  ...
  /**
   * Optional voice / TTS surface. When provided, `mogoi-notify` calls with
   ...
   * connected (e.g. `{"telegram"}` on a Mogoi with only telegram wired).
...
export class MogoiNotifierAdapter implements PieceNotifier {
...
function expandChannels(requested: PieceNotifyChannel[], deps: NotifierDeps): Set<string> {
  ...
  for (const c of requested) {
    if (c === "auto") {
      ...
      // Mogoi without telegram/discord would report two failures the
      // user can't act on.
      out.add("dashboard");
...
function titleForPriority(p: "urgent" | "normal" | "low"): string {
  if (p === "urgent") return "Mogoi (urgent)";
  return "Mogoi";
}

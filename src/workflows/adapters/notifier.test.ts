/**
 * `MogoiNotifierAdapter` covers the four channels the workflow notifier
...
import { MogoiNotifierAdapter, type NotifierDeps } from "./notifier";
...
describe("MogoiNotifierAdapter: voice channel", () => {
  test("delivers via sendVoice when wired", async () => {
    const spoken: string[] = [];
    const adapter = new MogoiNotifierAdapter(
      makeDeps({
        sendVoice: async (t) => {
          spoken.push(t);
        },
  ...
  test("reports failed when sendVoice isn't wired", async () => {
    const adapter = new MogoiNotifierAdapter(makeDeps());
...
describe("MogoiNotifierAdapter: channel expansion", () => {
  ...
  test('"auto" only fans out to *connected* external channels when the dep is wired', async () => {
    // Mogoi with only telegram wired -> auto goes to dashboard + telegram,
  ...
  test('"auto" on a Mogoi with no external channels connected only delivers to dashboard', async () => {
    let m8Called = false;
    const adapter = new MogoiNotifierAdapter(
      makeDeps({

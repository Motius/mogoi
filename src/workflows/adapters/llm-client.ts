/**
 * Adapter: PieceLlmClient over Mogoi' LLMManager.
 *
 ...
 * text content. Streaming and tool use are out of scope for mogoi-ask.
 */
...
export class MogoiLlmClient implements PieceLlmClient {
  ...
  async chat(input: PieceLlmInput): Promise<PieceLlmResponse> {
    ...
    if (input.systemParts !== undefined) {
      ...
      // so repeated workflow LLM calls reuse the (large) Mogoi role prompt.
 ...
 * blocks are dropped -- mogoi-ask is a text piece. If there's no text at

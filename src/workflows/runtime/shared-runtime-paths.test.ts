...
describe("resolveSharedRuntimePaths", () => {
  ...
  test("plain config paths resolve as-is (config wins over env)", () => {
    const paths = resolveSharedRuntimePaths(
      ...
      { MOGOI_SHARED_PIECES_DIR: "/somewhere/else" },
    );
  ...
  test("${version} placeholder expands from MOGOI_VERSION", () => {
    const paths = resolveSharedRuntimePaths(
      wf({
        engine_dir: "/opt/mogoi-engine/${version}",
        pieces_dir: "/opt/mogoi-pieces/${version}",
        piece_metadata_cache: "/opt/mogoi-cache/${version}/piece-metadata.json",
      }),
      { MOGOI_VERSION: "1.2.3" },
    );
    expect(paths.engineCacheRoot).toBe("/opt/mogoi-engine/1.2.3");
    expect(paths.piecesDir).toBe("/opt/mogoi-pieces/1.2.3");
    expect(paths.metadataCacheFile).toBe("/opt/mogoi-cache/1.2.3/piece-metadata.json");
    expect(paths.warnings).toEqual([]);
  });
  ...
  test("placeholder with an unusable version warns and falls back per path", () => {
    for (const version of [undefined, "current", "../evil", "bad/slash"]) {
      const paths = resolveSharedRuntimePaths(
        ...
        {
          ...(version !== undefined ? { MOGOI_VERSION: version } : {}),
          MOGOI_ENGINE_CACHE_ROOT: "/env/engine",
        },
      );
  ...
  test("no config: bare env vars are the fallback; nothing anywhere: all null", () => {
    const fromEnv = resolveSharedRuntimePaths(
      ...
      {
        MOGOI_ENGINE_CACHE_ROOT: "/env/engine",
        MOGOI_SHARED_PIECES_DIR: "/env/pieces",
        MOGOI_PIECE_METADATA_CACHE: "/env/cache/piece-metadata.json",
      },
    );

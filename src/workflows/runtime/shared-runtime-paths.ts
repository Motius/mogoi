 ...
 * placeholder expanded from the `MOGOI_VERSION` env var — useful when one
 ...
 * bare MOGOI_* env vars are honored as fallbacks; config wins when both
 * are set. Every consumer treats a null as "no ready-made artifact" and
 * does the work itself.
 */

import { resolve } from "node:path";
import type { MogoiConfig } from "../../config/types";
...
export function resolveSharedRuntimePaths(
  cfg: Pick<MogoiConfig, "workflows">,
  env: Record<string, string | undefined> = process.env,
): SharedRuntimePaths {
  const warnings: string[] = [];
  const wf = cfg.workflows;
  const version = env.MOGOI_VERSION?.trim() ?? "";
    ...
    if (configured?.trim()) {
      ...
      warnings.push(
        `workflows config path ${JSON.stringify(raw)} uses ${VERSION_PLACEHOLDER} but ` +
          `MOGOI_VERSION (${JSON.stringify(version)}) is not a usable concrete version — ` +
  ...
  return {
    engineCacheRoot: resolvePath(wf?.engine_dir, "MOGOI_ENGINE_CACHE_ROOT"),
    piecesDir: resolvePath(wf?.pieces_dir, "MOGOI_SHARED_PIECES_DIR"),
    metadataCacheFile: resolvePath(wf?.piece_metadata_cache, "MOGOI_PIECE_METADATA_CACHE"),
    warnings,
  };
}

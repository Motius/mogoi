 ...
 * legacy `mogoi-trigger:on_event` direct-subscribe path inside
 * `TriggerManager` (kept as a fallback when no engine runtime is wired) and
 * the workflow event buffer mirror used by `/v1/mogoi/events/poll`.
 *
 * Replaces the previous `MogoiEventBusAdapter` in `adapters/` (deleted with
...
export class WorkflowEventBus {
   ...
   * `/v1/mogoi/events/poll`, so engine-managed `on_event` triggers see the
  ...
  /** Publish from Mogoi daemon code. Errors in handlers are swallowed-and-logged. */

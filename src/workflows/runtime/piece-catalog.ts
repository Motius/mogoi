/**
 * `PieceCatalog` -- canonical view of installed pieces (Mogoi-native + future
 ...
 * `MogoiPieceRegistry` (`{name, displayName, description, actions, triggers}`
...
/**
 * Piece-name + trigger-name pair for mogoi-trigger:on_event. Hoisted so
 * the dynamic-sample injector and the composer prompt agree on the key.
 */
export const MOGOI_ON_EVENT_TRIGGER = {
  piece: "@mogoipieces/piece-mogoi-trigger",
  trigger: "on_event",
} as const;
...
export interface PieceCatalogAction {
   ...
   * `createAction({ outputSample: ... })` (Mogoi extension to upstream AP).
...
export interface PieceCatalogTrigger extends PieceCatalogAction {
   ...
   * mogoi-trigger `on_event` is the canonical case (the `payload`
 ...
 * mogoi-ask, schedule) leave it undefined and the editor renders no
...
export interface PieceCatalogEntry {
  /** Upstream package name -- e.g. `@mogoipieces/piece-mogoi-ask`. */
  name: string;
  displayName: string;
  description: string;
   ...
   * (mogoi-ask, schedule, webhook, code).
   */
  auth?: PieceCatalogAuth;
}

/**
 * Structural interface that both `PieceCatalog` and `MogoiPieceRegistry`
 ...
 *         resolution for mogoi-trigger:on_event).
 *   v4 -- promoted mogoi-trigger:on_event's `eventType` input field
 ...
 *         dropdown).
 *   v6 -- promoted mogoi-trigger:run_workflow's `flow` input from
...
export function computeCatalogCacheKey(opts: { bundlePath: string }): string {
  ...
  // mogoi-trigger:on_event projection synthesizes its
...
/** Convert an upstream `PieceMetadata` blob into the legacy `MogoiPiece`-style entry. */
export function metadataToCatalogEntry(meta: RawPieceMetadata | unknown): PieceCatalogEntry {
  ...
  // Apply any Mogoi-specific catalog enrichments after the generic
  // projection finishes. Today this covers two upgrades on the
  // mogoi-trigger piece (on_event eventType -> enum, run_workflow
  // flow -> flow_ref). Centralising the gating keeps the generic
  // projection above piece-agnostic and gives the next "Mogoi tweak"
  // a single named home.
  enrichMogoiCatalogEntry(out);
...
/**
 * Apply Mogoi-specific catalog enrichments AFTER the generic
 ...
 * trigger, so an upstream rename (or a Mogoi-piece source edit that
 * drops a sub-action) silently no-ops rather than crashing the catalog
 * build.
 *
 * The set of enrichments here is small and well-known:
 *   - mogoi-trigger:on_event   -- attach `dynamicSampleData` and
 *                                  promote `eventType` to enum.
 *   - mogoi-trigger:run_workflow -- promote `flow` to flow_ref.
 *
 ...
 */
export function enrichMogoiCatalogEntry(entry: PieceCatalogEntry): void {
  if (entry.name !== MOGOI_ON_EVENT_TRIGGER.piece) return;
  const onEvent = entry.triggers?.[MOGOI_ON_EVENT_TRIGGER.trigger];
  if (onEvent) {
...
/**
 * Build the `dynamicSampleData` map for mogoi-trigger:on_event. Each
...
/**
 * Rewrite the `flow` input field on mogoi-trigger's `run_workflow`
...
function rawActionToCatalogAction(
  ...
  // (Mogoi extension); triggers use upstream `sampleData`. Both can be

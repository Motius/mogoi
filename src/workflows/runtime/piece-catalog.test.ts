 ...
 *   - Engine-level (gated on MOGOI_TEST_ENGINE_BUILD=1): spawn the real
 *     engine, run EXTRACT_PIECE_METADATA against the seven vendored Mogoi
...
import { MogoiPieceRegistry } from "../mogoi-pieces/types";
...
import { buildAllMogoiPieces } from "../runner/engine-runtime/build-pieces";
...
describe("PieceCatalog (unit)", () => {
  test("PieceCatalog.list/get round-trips initial entries", () => {
    ...
    expect(catalog.get("@mogoipieces/piece-mogoi-test")?.displayName).toBe("Mogoi: Test");
  ...
  test("metadataToCatalogEntry maps actions + triggers verbatim", () => {
    ...
    expect(entry.name).toBe("@mogoipieces/piece-x");
  ...
  test("metadataToCatalogEntry attaches dynamicSampleData to mogoi-trigger:on_event", () => {
    // mogoi-trigger's on_event surfaces an envelope whose payload shape
  ...
  test("MogoiPieceRegistry structurally satisfies the PieceLookup interface", () => {
    // Compile-time + runtime check: assigning a MogoiPieceRegistry instance
    ...
    const reg = new MogoiPieceRegistry();
    reg.register({
      name: "p",
      displayName: "P",
      description: "p",
...
const buildOptIn = process.env.MOGOI_TEST_ENGINE_BUILD === "1";
...
const piecesAlreadyBuilt = existsSync(
  resolve(
    ...
    "pieces/mogoi/test/dist/src/index.js",
  ),
);
...
describe("PieceCatalog (engine end-to-end)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) return;
    if (!piecesAlreadyBuilt && buildOptIn) await buildAllMogoiPieces();
  ...
  test.skipIf(skipE2eTests)(
    "extracts metadata for every Mogoi-native piece via the real engine",
    async () => {
      const root = resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/mogoi");
      ...
      try {
        ...
        expect(names).toContain("@mogoipieces/piece-mogoi-ask");
        expect(names).toContain("@mogoipieces/piece-mogoi-tool");
        expect(names).toContain("@mogoipieces/piece-mogoi-notify");
        expect(names).toContain("@mogoipieces/piece-mogoi-context");
        expect(names).toContain("@mogoipieces/piece-mogoi-agent");
        expect(names).toContain("@mogoipieces/piece-mogoi-trigger");
        expect(names).toContain("@mogoipieces/piece-mogoi-test");
        // ask: single action `ask` with at least the `prompt` field.
        const ask = catalog.get("@mogoipieces/piece-mogoi-ask");
        ...
        const trigger = catalog.get("@mogoipieces/piece-mogoi-trigger");

 ...
 * bundle, ensures Mogoi-native pieces are compiled to dist/, starts a
...
export interface BootstrapWorkflowEngineOptions {
  /** Service backends for the `/v1/mogoi/*` routes. Each unset slot returns 503. */
   ...
   * `~/.mogoi/cache/piece-metadata.json`. Tests can pass their own path.
   */
  cacheFile?: string;
  /**
   * Optional override for the piece root dirs scanned during catalog build.
   * Default: the vendored Mogoi piece tree only. Future community pieces
   * will be added here.
   */
  pieceRoots?: string[];
  /**
...
const DEFAULT_CACHE_FILE = resolve(homedir(), ".mogoi", "cache", "piece-metadata.json");
...
export async function bootstrapWorkflowEngine(
  ...
  // `~/.mogoi/pieces/installed.json`. Empty manifest = no-op. Failures are
  ...
  const [cached] = await Promise.all([
    ...
    labelled("piece-compile", buildAllMogoiPieces()),
  ...
  //
  // customPiecesPaths includes the user pieces dir (`~/.mogoi/pieces`)
  // alongside the vendored Mogoi pieces. The engine's piece-loader walks
  ...
  //
  // MOGOI_SHARED_PIECES_DIR (multi-tenant hosting): a root-owned READ-ONLY
  ...
  // never touch it — they operate solely on `~/.mogoi/pieces`.
  const sharedPiecesDir =
    opts.sharedPiecesDir !== undefined
      ? opts.sharedPiecesDir
      : process.env.MOGOI_SHARED_PIECES_DIR?.trim() || null;
  ...
  //   1. vendored Mogoi-native pieces (always present)
  //   2. user-installed community pieces under `~/.mogoi/pieces/node_modules/@activepieces/`
  //      (may be missing; discoverPieces handles a missing dir gracefully)
  const pieceRoots = opts.pieceRoots ?? [
    resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/mogoi"),
  ...
  // MOGOI_PIECE_METADATA_CACHE: a READ-ONLY prebuilt metadata cache for the
  ...
  const sharedCacheFile =
    opts.sharedCacheFile !== undefined
      ...
      : process.env.MOGOI_PIECE_METADATA_CACHE?.trim() || null;

 ...
 * the real Mogoi pieces. Gated on `MOGOI_TEST_ENGINE_BUILD=1` because
...
const piecesAlreadyBuilt = existsSync(
  resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/mogoi/ask/dist/src/index.js"),
);
...
const FIXTURE_TO_REAL: Record<string, string> = {
  "mogoi-ask": "@mogoipieces/piece-mogoi-ask",
  "mogoi-notify": "@mogoipieces/piece-mogoi-notify",
  "mogoi-trigger": "@mogoipieces/piece-mogoi-trigger",
};
...
describe("sampleCatalog drift vs real piece metadata", () => {
  ...
  test.skipIf(skipE2eTests)(
    ...
    async () => {
      const pieceRoots = [resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/mogoi")];

/**
 * Glue layer: wraps existing Mogoi adapters into the function-shape that the
 * SandboxApi service-backend slots expect. Each `/v1/mogoi/*` route takes a
...
import { MogoiLlmClient } from "../adapters/llm-client";
import { MogoiToolRegistryAdapter } from "../adapters/tool-registry";
import { MogoiNotifierAdapter, type NotifierDeps } from "../adapters/notifier";
import { MogoiContextProviderAdapter } from "../adapters/context-provider";
import { LlmOnlyAgentDelegator } from "../adapters/agent-delegator";
import { M7AgentDelegator } from "../adapters/m7-agent-delegator";
import { MogoiWorkflowRunnerAdapter } from "../adapters/workflow-runner";
import type { LlmChatFn } from "../sandbox-api/routes/mogoi-llm";
import type { SystemPromptParts } from "../../roles/prompt-builder";
import type { ToolsInvokeFn } from "../sandbox-api/routes/mogoi-tools";
import type { NotifyFn } from "../sandbox-api/routes/mogoi-notify";
import type { MogoiContextProvider } from "../sandbox-api/routes/mogoi-context";
import type { AgentDelegateFn } from "../sandbox-api/routes/mogoi-agent";
import type { EventsPollFn } from "../sandbox-api/routes/mogoi-events";
import type { WorkflowsStartFn } from "../sandbox-api/routes/mogoi-workflows";
...
export interface BuildServiceBackendsOptions {
  ...
  /** Recent-events buffer for `mogoi-trigger:on_event` polling. */
  ...
  /**
   * M7 sub-agent dependencies. When all of these are supplied, `mogoi-agent.delegate`
  ...
  /**
   * Optional callback that builds the Mogoi-flavoured system prompt for
   * a workflow LLM call. When set, the `mogoi-ask` piece will pass this
   * prompt to the LLM so the model knows it's Mogoi (role, personality,
   ...
   */
  buildMogoiSystemPrompt?: (userMessage: string) => SystemPromptParts;
}

export function buildSandboxServiceBackends(
  opts: BuildServiceBackendsOptions,
): SandboxApiServices {
  const llmClient = new MogoiLlmClient(opts.llmManager);
  const llmChat: LlmChatFn = async (req) => {
    // System-prompt composition:
    //   - overrideSystem=true       : use `req.system` only. Mogoi
    ...
    //                                 by Mogoi's identity).
    //   - `req.system` set, default : Mogoi prompt + "\n\n" + req.system.
    //                                 Lets the user steer the reply (e.g.
    //                                 "respond in JSON") while keeping the
    //                                 Mogoi identity.
    //   - no `req.system`           : Mogoi prompt alone. Default for
    //                                 plain "ask Mogoi" steps.
    ...
    const mogoiParts = opts.buildMogoiSystemPrompt
      ? opts.buildMogoiSystemPrompt(req.prompt)
      : undefined;
    ...
    if (req.overrideSystem) {
      ...
      // Static Mogoi prefix stays cacheable; per-call context and the
      // piece's steering prompt ride on the dynamic half. Rendered text is
      // identical to the old `mogoi + '\n\n' + req.system` join.
      const dynamic = [mogoiParts.dynamic, req.system].filter(Boolean).join('\n\n');
      systemParts = { static: mogoiParts.static, ...(dynamic ? { dynamic } : {}) };
    } else {
      system = req.system;
    }
  ...
  const toolAdapter = opts.toolRegistry
    ? new MogoiToolRegistryAdapter(opts.toolRegistry)
    : null;
  ...
  const notifierAdapter = new MogoiNotifierAdapter(notifierDeps);
  ...
  const contextAdapter = new MogoiContextProviderAdapter();
  const contextProvider: MogoiContextProvider = {
    vaultSearch: (input) =>
  ...
  const eventsPoll: EventsPollFn = async (req) => {
    ...
    // The route's `MogoiEvent` types `id` as a string (consistent with all
  ...
  const runnerAdapter = new MogoiWorkflowRunnerAdapter();

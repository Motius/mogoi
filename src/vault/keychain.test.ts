 ...
 *
 * The legacy dir is `~/.mogoi` and cannot be redirected (homedir() is fixed
 ...
 * MOGOI_SECRETS_DIR, which never touches the developer's real store.
 */
...
describe('keychain', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'mogoi-keychain-'));
  ...
  function seed(dir: string, entries: Record<string, string>): void {
    ...
    process.env.MOGOI_SECRETS_DIR = dir;
    setSecrets(entries);
    delete process.env.MOGOI_SECRETS_DIR;
  }

  // ── location ────────────────────────────────────────────────────────────

  // keychainDir() consults the REAL ~/.mogoi as the legacy dir — homedir() is
  ...
  // assertion fails for every developer who runs Mogoi and passes only on a
  ...
  const realLegacyHasKeychain =
    existsSync(join(homedir(), '.mogoi', KEY_FILE)) ||
    existsSync(join(homedir(), '.mogoi', SECRETS_FILE));

  test.skipIf(realLegacyHasKeychain)(
    'MOGOI_HOME is where a fresh install stores its keychain',
    () => {
  ...
  test('a fresh install writes and reads its keychain under the configured dir', () => {
    const dataDir = join(root, 'data');
    process.env.MOGOI_HOME = dataDir;
    // MOGOI_SECRETS_DIR pins the location, so this exercises the read/write
  ...
  test('MOGOI_SECRETS_DIR overrides MOGOI_HOME and skips the fallback', () => {
    process.env.MOGOI_HOME = join(root, 'data');
    process.env.MOGOI_SECRETS_DIR = join(root, 'override');

 ...
 *   machine-readable (mogoi enroll/sidecars/revoke) set this so the token
...
export function initDatabase(dbPath: string = ":memory:", opts?: { quiet?: boolean }): Database {
  try {
    ...
    // than the daemon: `mogoi enroll` and `mogoi revoke` run in their own
...
function createTables(db: Database): void {
  ...
  // Content pipeline: per-stage notes from user or MOGOI
  db.run(`
  ...
  // self-cleaning across PROCESSES: `mogoi revoke` deletes the sidecars row
  ...
  // Documents table: vault-stored documents created by MOGOI
  db.run(`

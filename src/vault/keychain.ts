/**
 * Encrypted secrets store for MOGOI.
 *
 ...
 * `MOGOI_HOME` when set, else ~/.mogoi — the same resolution pid.ts and the
 * config loader use. Installs that predate that awareness keep reading their
 * ~/.mogoi pair until migrateKeychainToDataDir() relocates it.
 *
...
/** Where the keychain lived before MOGOI_HOME was honored. */
function legacyDir(): string {
  return join(homedir(), '.mogoi');
}
 ...
 *
 * `MOGOI_SECRETS_DIR` is an explicit override that wins over everything and
 ...
 * (e.g. `mogoi export`) must resolve it through here rather than assume a
 * path, or the two drift apart.
 */
function configuredDir(): string {
  const override = process.env.MOGOI_SECRETS_DIR || process.env.MOGOI_HOME;
  return override ? resolve(override) : legacyDir();
}

/**
 * Which of the two candidate dirs holds the live keychain. An install whose
 * data dir has no keychain yet, but whose ~/.mogoi does, keeps using the
...
export function keychainDir(): string {
  ...
  // file missing its MOGOI_SECRETS_DIR setup once deleted real keys. Fail
  // loudly instead of touching it.
  if (process.env.NODE_ENV === 'test' && !process.env.MOGOI_SECRETS_DIR && !process.env.MOGOI_HOME) {
    throw new Error('Refusing to touch the real keychain under test: point MOGOI_SECRETS_DIR at a temp dir in your test setup (see user-settings.test.ts)');
  }
  const configured = configuredDir();
  if (process.env.MOGOI_SECRETS_DIR) return configured;
...
/**
 * Relocate a pre-MOGOI_HOME keychain into the configured data dir.
 *
...
export function migrateKeychainToDataDir(): void {
  if (process.env.MOGOI_SECRETS_DIR) return; // explicit override: leave it alone

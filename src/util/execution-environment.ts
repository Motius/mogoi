...
export function hostTarget(name = "Mogoi host (this brain)"): ExecutionTarget {
  return {
    id: "",
    name,
    os: process.platform,
...
function isToolPiece(pieceName: string): boolean {
  return pieceName === "mogoi-tool" || pieceName.endsWith("/piece-mogoi-tool");
}

/**
 * Problems with one `mogoi-tool:invoke` step's OS fit, as messages meant to

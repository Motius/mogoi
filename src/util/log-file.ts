 ...
 *
 * Mogoi only ever had a log file in two launch modes: `mogoi start -d`
 * (bin/mogoi.ts redirects the detached child's fds at spawn) and launchd.
 * Under systemd - how the hosted fleet runs it, and `files/mogoi@.service`
 ...
 * every one of mogoi's ~1150 `console.*` call sites is invisible to the sink
 ...
 * disable the sink for the life of the process, so an `rm -rf ~/.mogoi/logs`
 ...
 * attacker-influenced (anything mogoi echoes back). Bounded repetition over
 ...
 * that redirects the daemon's fds 1/2 into the log file (`mogoi start -d`,
 ...
 * and `$MOGOI_HOME` spellings of the same file.
 */
...
export function installLogFileSink(opts: LogFileSinkOptions): () => void {
  ...
  // nothing crashed. Mogoi runs shell commands as its own user, so
  // `mkfifo ~/.mogoi/logs/mogoi.log` is inside the threat model. lstat (not
   ...
   * flag, so one transient failure - `rm -rf ~/.mogoi/logs`, a full disk that
   ...
   * half-truncated one. The inode changes, which is why `mogoi logs -f` uses

...
export function normalizePublicOrigin(raw: string): string {
  ...
  if (!value) throw new Error('Public Mogoi URL cannot be empty');
  ...
  if (!['http:', 'https:', 'ws:', 'wss:'].includes(parsed.protocol)) {
    throw new Error(`Unsupported public Mogoi URL scheme: ${parsed.protocol}`);
  }
  if (parsed.username || parsed.password || parsed.search || parsed.hash) {
    throw new Error('Public Mogoi URL must not contain credentials, a query, or a fragment');
  }
  if (parsed.pathname !== '/' && parsed.pathname !== '') {
    throw new Error('Public Mogoi URL must be an origin without a path');
  }
...
/**
 * Resolve the canonical externally reachable Mogoi origin.
 *
 ...
 */
export function resolveExternalOrigin(config: MogoiConfig, req?: Request): ExternalOriginResolution {
  ...
  if (source === 'fallback' && observed.detected) {
    warnings.push(
      ...
      + 'Set daemon.public_url (or MOGOI_PUBLIC_URL) to the public origin.',
    );

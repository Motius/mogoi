...
beforeEach(async () => {
  DIR = await mkdtemp(join(tmpdir(), 'mogoi-test-logfile-'));
  LOG = join(DIR, 'logs', 'mogoi.log');
});
...
describe('installLogFileSink', () => {
  ...
  test('strips ANSI escapes (src/cli/helpers.ts colours unconditionally)', () => {
    uninstall = installLogFileSink({ path: LOG });
    process.stdout.write('\x1b[36mM.O.G.O.I.\x1b[0m \x1b[2mv0.13.0\x1b[0m\n');
    ...
    process.stdout.write('\x1b]0;mogoi\x07\x9b1mbold\x9b0m \x1b(Bplain\n');
    uninstall();
    uninstall = null;

    const body = read();
    expect(body).toContain('M.O.G.O.I. v0.13.0');
  ...
  test('a pathological escape run does not stall the sink (CVE-2021-3807 shape)', () => {
    ...
    // `ESC` + 10-20k semicolons, and log lines carry text mogoi echoes back.
  ...
  test("captures console.* even though bun's console bypasses process.stdout.write", () => {
    ...
    // straight to fd 1, so a stream-only patch records none of mogoi's ~1150
  ...
  test('an unopenable path degrades to no sink instead of throwing', async () => {
    ...
    const bad = join(blocker, 'nested', 'mogoi.log');
  ...
  test('refuses a FIFO at log_file_path instead of hanging the daemon forever', () => {
    ...
    // no supervisor restarted it, because nothing had crashed. Mogoi runs
    // shell commands as its own user, so `mkfifo ~/.mogoi/logs/mogoi.log` is
  ...
  test('a compaction failure warns once and recovers when the path works again', () => {
    ...
    // process: one `rm -rf ~/.mogoi/logs` or one ENOSPC meant no log file
  ...
  test('cleans stale <path>.<pid>.tmp siblings left by a crash mid-compaction', () => {
    ...
    expect(left).not.toContain(`mogoi.log.${process.pid}.tmp`);
    expect(left).toContain('mogoi.log.1.tmp');
    expect(left).toContain('mogoi.log.keep');
  });
});

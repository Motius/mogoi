...
describe('hostedProxyError', () => {
  test('the generic branch never carries the proxy body (hostname stays out of chat copy)', () => {
    ...
    const err = hostedProxyError(
      'MotiusAI AI API',
      502,
      '<html><title>502 Bad Gateway</title>error at proxy host llm.motius.host: upstream timeout</html>',
    );
    expect(err.message).toContain('(502)');
    expect(err.message).not.toContain('llm.motius.host');
  ...
  test('budget copy quotes a reset time ONLY when handed one (never parsed from the body)', () => {
    ...
    const invalid = hostedProxyError('MotiusAI AI API', 429, 'budget exceeded', new Date('nonsense'));
  ...
  test('403 maps to model-not-in-plan even without model text (team_model_access_denied)', () => {
    const err = hostedProxyError('MotiusAI AI API', 403, '{"error":{"code":"team_model_access_denied"}}');
  ...
  test('model-denial TEXT still precedes the auth branch (historical 401 shape)', () => {
    const err = hostedProxyError('MotiusAI AI API', 401, 'key not allowed to access model uj-video');
    expect(err.message).toContain('not included in your plan');
  });

  test('401 without model text is the credential/plan copy', () => {
    const err = hostedProxyError('MotiusAI AI API', 401, 'Authentication Error: key is blocked');
  ...
  test('an ordinary 429 rate limit is NOT budget copy (stays retryable-generic)', () => {
    const err = hostedProxyError('MotiusAI AI API', 429, 'rate limited, retry shortly');

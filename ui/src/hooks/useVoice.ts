...
function getWakePrefix(normalized: string): "hey mogoi " | "mogoi " | null {
  if (normalized.startsWith("hey mogoi ")) return "hey mogoi ";
  if (normalized.startsWith("mogoi ")) return "mogoi ";
  return null;
}

/**
 * Strict matcher used during TTS playback (voiceState === "speaking").
 * Accepts bare wake phrases ("mogoi", "hey mogoi") or wake + a short
 * whitelisted interrupt command. Prevents Mogoi's own TTS from self-triggering
 * when the reply contains the word "mogoi" inside a sentence.
 */
export function matchesSpeechWakePhrase(transcript: string): boolean {
  const normalized = normalizeTranscript(transcript);
  if (!normalized) return false;
  if (normalized === "mogoi" || normalized === "hey mogoi") return true;
 ...
 * wake phrase, so natural "hey mogoi turn off the lights" wakes in one breath
 * without waiting for Chrome to emit a bare-wake interim first.
 * NOT safe to use while Mogoi is speaking — use matchesSpeechWakePhrase there.
 */
...
export function isSpeechStopCommand(transcript: string): boolean {
  const normalized = normalizeTranscript(transcript);
  return normalized === "mogoi stop"
    || normalized === "hey mogoi stop"
    || normalized === "mogoi quiet"
    || normalized === "hey mogoi quiet"
    || normalized === "mogoi be quiet"
    || normalized === "hey mogoi be quiet";
}
...
export function shouldSpeechWakeBeRunning(inputs: {
  ...
  // so "Mogoi" can interrupt mid-thought, not just kick off from idle.
...
export type UseVoiceReturn = {
  ...
  /** Mid-turn flip: a later sentence in the same TTS turn contains "Mogoi"
...
export function useVoice({ wsRef, wakeWordEnabled = true, nativeWakeActive = false, wakeEngine = "openwakeword", getCurrentRoom, speakingTailCooldownMs = 700, errorFlashMs = 3000 }: UseVoiceOptions): UseVoiceReturn {
  ...
  // True if the in-flight TTS turn contains "Mogoi" anywhere. The
  ...
  // stays running so a real human "Mogoi" can interrupt the reply.
  const ttsContainsWakeRef = useRef(false);
  // Same lifecycle as ttsContainsWakeRef, but for spoken *stop phrases*
  // ("Mogoi, stop"). Stop phrases normally bypass the containsWake echo
  ...
  const startWakeWordEngine = useCallback(async () => {
    ...
    try {
      ...
      console.log("[Voice] Wake word engine ready — say 'Hey MOGOI'");
    } catch (err) {
  ...
  const startSpeechWakeIfNeeded = useCallback((): void => {
    ...
    if (!speechWakeRef.current) {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        // so "Mogoi" can interrupt mid-thought.
        ...
        // loose prefix matcher when idle so "hey mogoi <command>" wakes in one breath.
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          // "Mogoi", but let the explicit stop phrase through.
    ...
    try {
      ...
      console.log("[Voice] Speech wake recognizer starting — say 'Mogoi' or 'Hey Mogoi'");
    } catch {
  ...
  // "Mogoi" through speakers would self-trigger via the mic.
  useEffect(() => {

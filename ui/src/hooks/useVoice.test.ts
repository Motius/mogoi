...
describe("isSpeechStopCommand", () => {
  test("recognizes direct stop/quiet commands", () => {
    expect(isSpeechStopCommand("Mogoi stop")).toBe(true);
    expect(isSpeechStopCommand("Hey Mogoi, stop!")).toBe(true);
    expect(isSpeechStopCommand("Mogoi be quiet")).toBe(true);
  });

  test("does not turn ordinary wake phrases into stop-only commands", () => {
    expect(isSpeechStopCommand("Mogoi")).toBe(false);
    expect(isSpeechStopCommand("Mogoi what time is it")).toBe(false);
  });
});

describe("matchesSpeechWakePhrase (strict; used during TTS playback)", () => {
  test("accepts direct wake phrases", () => {
    expect(matchesSpeechWakePhrase("Mogoi")).toBe(true);
    expect(matchesSpeechWakePhrase("hey mogoi")).toBe(true);
    expect(matchesSpeechWakePhrase("Mogoi, stop")).toBe(true);
    expect(matchesSpeechWakePhrase("Hey Mogoi, hold on")).toBe(true);
  });

  test("rejects longer sentences that merely mention mogoi (echo protection)", () => {
    expect(matchesSpeechWakePhrase("Mogoi is already working on that")).toBe(false);
    expect(matchesSpeechWakePhrase("Can you tell Mogoi to send that")).toBe(false);
    expect(matchesSpeechWakePhrase("I said Mogoi in the middle of a sentence")).toBe(false);
    expect(matchesSpeechWakePhrase("Hey Mogoi can you help")).toBe(false);
  });
});

describe("matchesSpeechWakePrefix (loose; used when idle)", () => {
  test("accepts bare wake phrases", () => {
    expect(matchesSpeechWakePrefix("Mogoi")).toBe(true);
    expect(matchesSpeechWakePrefix("hey mogoi")).toBe(true);
  });

  test("accepts wake phrase followed by any command in one utterance", () => {
    expect(matchesSpeechWakePrefix("hey mogoi turn off the lights")).toBe(true);
    expect(matchesSpeechWakePrefix("mogoi what's the weather")).toBe(true);
    expect(matchesSpeechWakePrefix("Hey Mogoi, play some music")).toBe(true);
    expect(matchesSpeechWakePrefix("mogoi can you help me")).toBe(true);
  });

  test("rejects utterances that merely contain mogoi mid-sentence", () => {
    expect(matchesSpeechWakePrefix("Can you tell Mogoi to send that")).toBe(false);
    expect(matchesSpeechWakePrefix("I said Mogoi in the middle of a sentence")).toBe(false);
    expect(matchesSpeechWakePrefix("that was Mogoi talking")).toBe(false);
  });
...
describe("matcher normalization (shared by both matchers)", () => {
  ...
  test("collapses stacked punctuation and multiple spaces", () => {
    expect(matchesSpeechWakePhrase("Mogoi... stop!!")).toBe(true);
    expect(matchesSpeechWakePrefix("hey   mogoi    play   music")).toBe(true);
  });

  test("is not fooled by mid-word 'mogoi' substrings", () => {
    // "starmogoi" is one token; without a word boundary it must not match.
    expect(matchesSpeechWakePrefix("starmogoi")).toBe(false);
    expect(matchesSpeechWakePhrase("starmogoi stop")).toBe(false);
  });
});
...
describe("planContainsWakeFlip — regression boundary for the mid-turn cooldown stamp bug", () => {
  ...
  // and a later sentence introduces "Mogoi", the daemon flips containsWake

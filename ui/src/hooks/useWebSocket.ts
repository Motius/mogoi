...
export function useWebSocket() {
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (msg.type === "tts_text") {
          // Mid-turn signal that an upcoming sentence contains "Mogoi" —
  ...
  useEffect(() => {
    ...
    window.addEventListener("mogoi:ws-reconnect", onManualRetry);
    return () => {
      window.removeEventListener("mogoi:ws-reconnect", onManualRetry);

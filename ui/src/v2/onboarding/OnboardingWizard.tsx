...
/* ═══════════════════ Onboarding · the nine-screen first-run flow ═══════════
   Faithful to the design (motius-onboarding.html): Welcome · Permissions
...
export function OnboardingWizard({
  ...
  const applyTheme = (t: "light" | "dark") => {
    ...
    try { localStorage.setItem("mogoi-theme", t); } catch { /* ignore */ }
  };

  // brain
  // Hosted install? GET /api/config/llm reports hosted_llm when the
  // system-owned motius_ai block is live. TRI-state on purpose: a slow or
  ...
  const provList = useMemo(
    () => PROVIDERS.map((p) => (p.id === "mogoi" ? { ...p, soon: !hosted } : p)),
    [hosted],
  );
  ...
  const saveSetup = useCallback(async () => {
    ...
    try {
      ...
      // the included MotiusAI AI voice when TTS is enabled later).
    ...
    switch (key) {
        ...
        <div className="obw-body mid"><div className="obw-wrap">
          ...
          <div className="obw-hint" style={{ maxWidth: "44ch", margin: "12px auto 0" }}>
            {hosted
              ...
              : "Choose your AI and optional voice, then tell Mogoi a little about your work. You can skip optional steps and return later."}
          </div>
      ...
      case "speak": {
        const opts: Array<["off" | "edge" | "elevenlabs", string, string, string]> = [
          ...
          ["edge", "vol", "Edge TTS", "Free, clean, ships with Mogoi. Pick a voice below."],
  ...
  function recapLine() {
    if (!configuredThisSession) return "Mogoi is ready for your next step.";

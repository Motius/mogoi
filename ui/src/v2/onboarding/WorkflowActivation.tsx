...
export function WorkflowActivation({ onContinue }: {
  ...
  const continueToMogoi = async (prompt?: string) => {
    ...
    try {
      ...
      setError("Couldn’t open Mogoi. Your task is still here. Try again.");
    } finally {
      busy.current = false;
      setPending(false);
    }
  ...
  return (
    <form className="obw-activation" onSubmit={(event) => {
      ...
      if (routine.trim()) void continueToMogoi(buildWorkflowRequest(routine));
    }}>
      <label htmlFor={id}>What’s one task you keep doing?</label>
      <p className="obw-activation__intro" id={`${id}-help`}>
        Describe the repetition. Mogoi can help turn it into your first workflow.
      </p>
      <textarea
        ref={input}
        id={id}

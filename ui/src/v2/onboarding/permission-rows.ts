...
export const PERM_COPY: Record<string, PermCopy> = {
  ...
  screen: {
    ...
    glyph: "screen",
    required: true,
    body: "Screen context for Awareness, so Mogoi can notice repeated work and suggest routines worth turning into workflows.",
  },
  microphone: {
    label: "Microphone",
    glyph: "mic",
    required: false,
    body: "Optional voice commands and the wake word. The onboarding interview uses written answers. Mogoi will ask again when you use voice.",
  },
  notifications: {
    label: "Notifications",
    glyph: "bell",
...
export function requestFeedback(result: PermRequestResult | null, label: string): string | null {
  if (!result) return "Mogoi sent an answer this page couldn't read.";

...
/* ═══════════════════ Cold-start splash ═══════════════════
   ...
   temperaments (design: motius-loading-screens), one picked per launch by
...
const VARIANTS: Record<VariantId, { name: string; status: string[]; weight: number }> = {
  summon: { name: "The Summon", status: ["starting the brain…", "mounting the vault…", "waking your agents…", "pebble live · :1846"], weight: 4 },
...
function pickVariant(): VariantId | null {
  let setting = "rotate";
  try { setting = localStorage.getItem("mogoi-startup-anim") || "rotate"; } catch { /* ignore */ }
  if (setting === "none") return null;
  if (setting !== "rotate" && setting in VARIANTS) return setting as VariantId;
  let last = "";
  try { last = localStorage.getItem("mogoi-boot-last") || ""; } catch { /* ignore */ }
  ...
  try { localStorage.setItem("mogoi-boot-last", chosen); } catch { /* ignore */ }
  return chosen;
}
...
const Word = () => <div className="lword"><span className="u">use</span>mogoi</div>;
...
export function BootSplash() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mogoi:boot-ready", handOff);
    ...
    return () => {
      ...
      window.removeEventListener("mogoi:boot-ready", handOff);
    };
  }, [variant, done]);
      ...
      {variant === "boot" && (
        <div className="lstage st-boot">
          ...
          <div className="bootlog">
            <div className="bl" style={{ ["--i" as string]: 0 }}><span className="mut">$</span> mogoi start</div>
            ...
            <div className="bl ok" style={{ ["--i" as string]: 4 }}>✓ pebble live · localhost:1846</div>
          </div>
        </div>
      )}

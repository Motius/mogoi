...
/* ═══════════════════ System states · Monochrome Lab ═══════════════════
   ...
   motius-system-states.html. The rule: say what's wrong, say what
...
function TakeoverBody({ kind, on, data }: { kind: TakeoverKind; on: TakeoverHandlers; data: TakeoverData }) {
  ...
  if (kind === "offline") {
    ...
    // servers" (align with NowRoom's mogoi-start notice).
    return (
      <div className="sys-wrap">
        ...
        <div className="sys-fine">if it stays down, start it yourself: <span style={{ fontFamily: "var(--mono)" }}>mogoi start</span></div>
      </div>
    );
  }
...
export function UpdateBanner({ version, on = {} }: { version?: string; on?: BannerHandlers }) {
  ...
  return (
    <div className="sys-bnr info" role="status">
      ...
      <span className="bm"><b>Mogoi {v} is ready.</b> Restart to get the latest, or keep working and update later.</span>
      <span className="ba">
 ...
 * backend). Gated behind the mogoi-devtools opt-in so a stray localStorage
 * key can't fake an "Out of tokens" takeover for a real user. Reads two
 * localStorage keys, live across tabs:
 *   mogoi-system-state  → one TakeoverKind ("offline"|"updating"|"crash"|"quota")
 *   mogoi-system-banner → comma list of BannerKind ("update","rate")
 */
export function useSystemStateOverride(): { takeover: TakeoverKind | null; banners: BannerKind[] } {
  const read = () => {
    ...
    try {
      const t = localStorage.getItem("mogoi-system-state");
      if (t === "offline" || t === "updating" || t === "crash" || t === "quota") takeover = t;
      const b = localStorage.getItem("mogoi-system-banner") ?? "";
  ...
  useEffect(() => {
    ...
    // `storage` only fires in OTHER tabs; `mogoi:system-state` lets the same
    // tab update live so a devtools one-liner works without a reload.
    const onStorage = (e: StorageEvent) => {
      if (e.key === "mogoi-system-state" || e.key === "mogoi-system-banner") reRead();
    };
    window.addEventListener("storage", onStorage);
    window.addEventListener("mogoi:system-state", reRead);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("mogoi:system-state", reRead);
    };
  }, []);
  return state;
}

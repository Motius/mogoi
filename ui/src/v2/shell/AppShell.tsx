...
function AppShellLive() {
  ...
  const latestAssistantReply = useMemo(() => {
    for (let i = live.items.length - 1; i >= 0; i--) {
      ...
      if (item.kind === "mogoi-speech" && item.text) {
        return {
          text: item.text,
...
function ShellLayout({
  ...
  const sysHandlers = useMemo(
    () => ({
      onRetry: () => window.dispatchEvent(new Event("mogoi:ws-reconnect")),
      onCheckStatus: () => window.open("https://status.motius.com", "_blank", "noopener"),
      onReopen: () => window.location.reload(),
      onSendReport: () => window.open("https://motius.com/support", "_blank", "noopener"),
      onUpgrade: () => window.open("https://motius.com/pricing", "_blank", "noopener"),
      onTopUp: () => window.open("https://motius.com/pricing", "_blank", "noopener"),

...
export interface MogoiSourceMeta {
  /** Source id, e.g. "google". External ids it claims look like `mogoi:<id>` or `mogoi:<id>:*`. */
  id: string;
}
...
export function useConnections(): ConnectionsState {
  ...
  const [mogoiSources, setMogoiSources] = useState<MogoiSourceMeta[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setMogoiSources(body.mogoiSources);
    } catch (e) {
  ...
  return { loading, error, connections, mogoiSources, refresh, create, update, remove };
}

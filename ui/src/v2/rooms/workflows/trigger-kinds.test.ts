...
describe("detectTriggerKind", () => {
  ...
  test("mogoi-trigger pieceName -> event", () => {
    expect(detectTriggerKind(pieceTrigger("t", "@mogoipieces/piece-mogoi-trigger", "on_event"))).toBe("event");
  });
...
describe("transitionTriggerKind", () => {
  ...
  test("TRIGGER_KINDS table contains exactly the four canonical kinds in presentation order", () => {
    ...
    expect(TRIGGER_KINDS.find((tk) => tk.kind === "event")?.pieceName).toBe("@mogoipieces/piece-mogoi-trigger");
  ...
  describe("kind -> kind transitions stash the outgoing kind so nothing is lost on a click", () => {
    ...
    test("schedule -> manual -> event keeps the schedule recoverable via a future schedule click", () => {
      ...
      // Event starts fresh with the canonical mogoi-trigger pieceName.
      expect(r2.settings?.pieceName).toBe("@mogoipieces/piece-mogoi-trigger");
    ...
    test("event -> event is a no-op-ish (fresh settings only because stash is empty)", () => {
      const eventSettings = { pieceName: "@mogoipieces/piece-mogoi-trigger", triggerName: "on_event", input: { eventType: "observer.clipboard_changed" } };
      ...
      expect(r.settings?.pieceName).toBe("@mogoipieces/piece-mogoi-trigger");

...
export type STTProvider = "openai" | "groq" | "sarvam" | "local" | "motius";
export type TTSProvider = "edge" | "elevenlabs" | "sarvam" | "motius";
...
export interface LLMConfig {
  ...
  /**
   * True on hosted installs: the system-owned motius_ai provider is
   ...
   * "included with your plan" card and offers `motius_ai:*` model refs.
   */
  hosted_llm?: boolean;
  /**
   * True when this install is HOSTED at all, which is wider than `hosted_llm`:
   * a hosted deployment with no LLM proxy configured has no motius_ai block
...
export interface STTConfig {
  provider: string;
  /** True on hosted installs: the "MotiusAI AI (included)" option applies. */
  motius_available?: boolean;
...
export interface VoiceConfig {
  ...
  realtime: {
    ...
    /** This install is hosted (a motius_ai block is present). */
    hosted?: boolean;
...
export function useSettingsData() {
   ...
   * recorded choice — not a write of 'motius'. The hosted defaults key off

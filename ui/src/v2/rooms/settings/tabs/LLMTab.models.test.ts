...
import { providerModels, seedModelForProvider, unsetSlotPlaceholder, USEMOGOI_TIER_ALIASES } from "./LLMTab";
...
describe("providerModels: hosted catalog filtering", () => {
  const providers = { motius_ai: { kind: "motius_ai" as const, has_api_key: true } };
  const catalogs = {
    motius_ai: ["uj-chat", "uj-high", "uj-low", "uj-medium", "uj-realtime", "uj-stt", "uj-tts"],
  };

  test("keeps the chat tiers and drops every voice alias", () => {
    expect(providerModels(providers, "motius_ai", null, catalogs)).toEqual([
      "uj-chat",
      "uj-high",
      "uj-low",
      "uj-medium",
  ...
  test("an UNKNOWN future alias is excluded (allowlist fails closed)", () => {
    // The platform can ship a new slot with no mogoi release. A denylist
    ...
    const withFuture = { motius_ai: ["uj-audio", "uj-chat", "uj-vision"] };
    expect(providerModels(providers, "motius_ai", null, withFuture)).toEqual(["uj-chat"]);
  });
...
describe("seedModelForProvider: per-tier seeding", () => {
  ...
  test("each tier seeds its OWN alias, not the alphabetically-first one", () => {
    ...
    expect(seedModelForProvider(hostedChatModels, USEMOGOI_TIER_ALIASES.high)).toBe("uj-high");
    expect(seedModelForProvider(hostedChatModels, USEMOGOI_TIER_ALIASES.medium)).toBe("uj-medium");
    expect(seedModelForProvider(hostedChatModels, USEMOGOI_TIER_ALIASES.low)).toBe("uj-low");
    expect(seedModelForProvider(hostedChatModels, USEMOGOI_TIER_ALIASES.conversation)).toBe("uj-chat");
  });
  ...
  test("every tier alias is one the picker actually offers (no drift)", () => {
    const offered = providerModels(
      { motius_ai: { kind: "motius_ai" as const, has_api_key: true } },
      "motius_ai",
      null,
      { motius_ai: ["uj-chat", "uj-high", "uj-low", "uj-medium", "uj-stt"] },
    );
    for (const alias of Object.values(USEMOGOI_TIER_ALIASES)) expect(offered).toContain(alias);
  });
});
...
describe("unsetSlotPlaceholder: unset slots show routing truth", () => {
  test("plan-sourced fill names the plan default", () => {
    expect(unsetSlotPlaceholder({ ref: "motius_ai:uj-high", source: "plan" }))
  ...
  test("a choice-sourced ref is not a placeholder case", () => {
    ...
    expect(unsetSlotPlaceholder({ ref: "motius_ai:uj-pro", source: "choice" }))
      .toBe("Select a model…");
  });
});

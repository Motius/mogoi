...
/** Reserved hosted provider name (mirrors the daemon's motius_ai carve-out). */
const USEMOGOI_NAME = "motius_ai";
/** The hosted provider's KIND (mirrors the daemon's USEMOGOI_KIND). Same
 ...
 * slots are data (a new alias ships without any mogoi release), and
...
export const USEMOGOI_TIER_ALIASES: Record<LLMTier, string> = {
...
const CHAT_USEMOGOI_ALIASES = new Set(Object.values(USEMOGOI_TIER_ALIASES));
 ...
 * on hosted installs, the system-owned MotiusAI AI provider. The backend
...
function ProvidersList({
  ...
  return (
    <div>
      {llm.hosted_llm && <ManagedMotiusAIRow />}
...
function NewProviderRow({
  ...
  const reserved = effectiveName === USEMOGOI_NAME;
      ...
      <div className="v2-set__row-body">
        ...
        <div className="v2-set__field">
          ...
          {duplicate && (
            <div className="v2-set__hint v2-set__hint--warn">
              {reserved
                ? <>&quot;{USEMOGOI_NAME}&quot; is reserved for the hosted provider. Pick a different name.</>
      ...
      {TIERS.map((t) => (
        <div key={t.id} className="v2-set__field">
          <ModelSelector
            ...
            catalogState={catalogState}
            allowClear
            preferredModel={USEMOGOI_TIER_ALIASES[t.id]}
            effectiveHint={llm.effective?.tiers[t.id]}
            clearHint={
              llm.default
                ? { ref: llm.default, source: "default" }
                : llm.hosted_llm
                  ? { ref: `motius_ai:${USEMOGOI_TIER_ALIASES[t.id]}`, source: "plan" }
                  : undefined
            }
            onChange={async (ref) => {
...
function ModelSelector({
  ...
  useEffect(() => {
    if (parsed) {
      ...
      const hostedRef = providers[parsed.provider]?.kind === USEMOGOI_KIND;
  ...
  const isHosted = providers[selectedProvider]?.kind === USEMOGOI_KIND;
...
export function providerModels(
  ...
  if (
    (entry.kind === "omniroute" || entry.kind === "groq" || entry.kind === "motius_ai")
    ...
    return entry.kind === "motius_ai"
      ? catalog.filter((id) => CHAT_USEMOGOI_ALIASES.has(id))
      : catalog;
  }
 ...
 * OmniRoute (user-defined routes/combos) and the hosted MotiusAI AI proxy
  ...
  useEffect(() => {
    ...
    Promise.all(targets.map(async ({ name, kind }) => {
      try {
        ...
        const response = kind === USEMOGOI_KIND
          ? await fetch("/api/config/llm/motius/models")
          : await fetch(
              kind === "groq"

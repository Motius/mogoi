...
export function BillingTab({ onToast }: { data: SettingsHook; onToast: Toast }) {
  ...
  const open = (url: string) => {
    if (!openedOrHandedOff(openExternal(url))) {
      onToast("Couldn't open your browser. Open your motius account's billing page to continue.", "warn");
    }
  };
  ...
  if (billing.state === "unknown") {
    return <Soon title="Loading billing…" sub="Reading your plan and payments from your motius account." />;
  }
  ...
  if (billing.state === "unavailable" || !billing.summary || !billing.links) {
    return (
      <Soon
        ...
        sub="Your plan and payments couldn't be read from your motius account. Reading them here never changes them."
      >

#!/usr/bin/env bun
/**
 * CLI: build the workflow runtime in one go -- every Mogoi piece's
...
console.log("Building Mogoi pieces...");
const piecesStart = Date.now();
const pieces = await buildAllMogoiPieces({ force });
const piecesElapsed = Date.now() - piecesStart;
if (pieces.length === 0) {
  console.log("  (no Mogoi pieces found)");
} else {
  let builtCount = 0;
  let cachedCount = 0;

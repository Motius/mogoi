...
if (process.platform === 'win32') {
  console.error('Native Windows installs are not supported for the MOGOI daemon.');
  console.error('Use WSL2 for the Bun install, or run MOGOI with Docker on Windows.');
...
//
// When `bun install -g @motius/brain` runs, the package root usually ends
...
if (
  isGlobalInstall &&
  !isDevCheckout &&
  !process.env.MOGOI_INSTALL_METHOD &&

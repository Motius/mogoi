 ...
 *
 * Steps:
 *   1. Read the current key from `~/.mogoi/cache/workflow-encryption.key`
 *      (or `--key-file`).
 ...
 *
 * Pre-conditions:
 *   - `MOGOI_WORKFLOW_ENCRYPTION_KEY` env var must be unset. With the env
...
function parseArgs(): CliArgs {
  const defaultDataDir = resolve(homedir(), ".mogoi");
  ...
  return {
    ...
    dbPath: dbPath ?? resolve(dataDir, "mogoi.db"),
  };
}
...
async function main(): Promise<void> {
  if (process.env["MOGOI_WORKFLOW_ENCRYPTION_KEY"]) {
    console.error(
      [
        "MOGOI_WORKFLOW_ENCRYPTION_KEY is set in the environment. That env var",
  ...
  // (not just the default `~/.mogoi/mogoi.pid`). Override the safety with

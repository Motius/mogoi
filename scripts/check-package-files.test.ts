...
describe("check-package-files parseBunPack", () => {
  ...
  test("keeps paths containing spaces, and yields nothing for unparseable output", () => {
    ...
    expect(parseBunPack("added 1.0KB bin/mogoi.ts\nTotal files: 1")).toEqual([]);
  });
});

describe("check-package-files parseNpmPack", () => {
  const files = [{ path: "bin/mogoi.ts" }, { path: "package.json" }];
  const want = ["bin/mogoi.ts", "package.json"];
  // npm 11 shape.
  const asArray = JSON.stringify([{ name: "@motius/brain", files }]);
  ...
  const asObject = JSON.stringify({ "@motius/brain": { id: "@motius/brain@0.13.1", files } });

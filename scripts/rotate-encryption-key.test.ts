 ...
 *   - Refuses to run when MOGOI_WORKFLOW_ENCRYPTION_KEY is set.
...
beforeEach(() => {
  dataDir = mkdtempSync(join(tmpdir(), "mogoi-rotate-"));
  keyFile = join(dataDir, "cache", "workflow-encryption.key");
  dbPath = join(dataDir, "mogoi.db");
...
function runScript(opts: {
   ...
   * The rotation script now probes the lock at `<dataDir>/mogoi.pid` (not
   * the global `~/.mogoi/mogoi.pid`), so tests are naturally isolated
  ...
  // The script reads MOGOI_WORKFLOW_ENCRYPTION_KEY from env -- clear it
  // unless the test explicitly wants it set.
  if (!opts.env || !("MOGOI_WORKFLOW_ENCRYPTION_KEY" in opts.env)) {
    delete env["MOGOI_WORKFLOW_ENCRYPTION_KEY"];
  }
...
describe("rotate-encryption-key", () => {
  ...
  test("ignores a daemon holding the default lock when --data-dir points elsewhere", async () => {
    ...
    // --data-dir, so a daemon running against `~/.mogoi` blocked rotations
    // for arbitrary other data dirs. Now isLocked is probed at
    // <dataDir>/mogoi.pid, so a default-path daemon doesn't interfere.
  ...
  test("refuses to run when MOGOI_WORKFLOW_ENCRYPTION_KEY is set", () => {
    const res = runScript({
      env: { MOGOI_WORKFLOW_ENCRYPTION_KEY: originalKey.toString("hex") },
    });
    expect(res.status).toBe(1);
    expect(res.stderr).toMatch(/MOGOI_WORKFLOW_ENCRYPTION_KEY/);
    // Keychain unchanged.
    expect(readFileSync(keyFile, "utf8").trim()).toBe(originalKey.toString("hex"));
  }, TEST_TIMEOUT_MS);

...
# a given version and re-apply the mogoi patches (mogoi.patch).
#
...
# sidecar/third_party/webview_go/MOGOI_PATCH.md.
#
# Usage:
...
PATCH_FILE="$VENDOR_DIR/mogoi.patch"

# Files we add on top of pristine upstream; preserved across the re-copy.
KEEP_FILES=(MOGOI_PATCH.md mogoi.patch .gitattributes)
...
# must regenerate mogoi.patch. Regenerate it by diffing pristine upstream
# against the patched tree for EVERY file we touch, not just the header.
echo "Applying mogoi.patch..."
...
# KEEP_FILES, so anything mogoi.patch stops carrying is silently reverted, the
...
# both leave the string "mogoi_host_owns_run_loop()) {" in place while breaking
# it in the direction where first run hangs.
grep -A1 'if (!mogoi_host_owns_run_loop()) {' "$HEADER" | grep -q 'stop_run_loop();'
grep -q "PATCHED (mogoi)" "$VENDOR_DIR/webview.go"         # nil WebView on NULL handle
grep -q "PATCHED (mogoi)" "$VENDOR_DIR/mogoi_native.go"   # browser controller accessor
grep -q "SetHostOwnsRunLoop" "$VENDOR_DIR/mogoi_native.go" # cocoa: the flag's Go binding
...
echo "Done: vendored $MODULE@$VERSION with mogoi.patch applied."

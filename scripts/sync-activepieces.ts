...
// 5. Wipe vendor tree except Mogoi-authored docs
mkdirSync(VENDOR_DIR, { recursive: true });
const PRESERVE = new Set([
  "UPSTREAM.md",
  "SPIKE-SANDBOXING.md",
  "LICENSE.activepieces",
  // Mogoi-added stub. Vendored package tsconfigs all `extends` this file
...
/**
 * Files we overwrite with a Mogoi-specific stub after copy. The original
...
const STUB_FILES: Record<string, string> = {
  "packages/server/engine/src/lib/core/code/v8-isolate-code-sandbox.ts": `// THIS FILE IS A MOGOI STUB.
// The upstream activepieces engine uses \`isolated-vm\` (a Node N-API native
// addon) to run user code in a V8 isolate. Mogoi runs the engine exclusively
...
// or SANDBOX_CODE_AND_PROCESS -- neither of which Mogoi supports. Reset
// AP_EXECUTION_MODE to SANDBOX_PROCESS.

import type { CodeSandbox } from '../../core/code/code-sandbox-common'

const message = 'v8-isolate-code-sandbox is not available in Mogoi. Use AP_EXECUTION_MODE=SANDBOX_PROCESS.'
...
const STRIP_LINES: Record<string, RegExp[]> = {
  ...
  // on every outbound HTTPS request MOGOI makes. Strip it; if a user
...
/**
 * Mogoi-specific source patches re-applied after each sync. Each entry names
 ...
 * `// Mogoi: ...` comment) so the rationale survives even without this
 * script in scope.
 */
...
> = {
  ...
  // daemon's /v1/mogoi/* endpoints with the engineToken. The engine
  ...
  "packages/pieces/framework/src/lib/context/index.ts": [
    {
      ...
      insert:
        "  // Mogoi: the engine runtime (trigger-helper.ts) sets `server` unconditionally\n" +
        ...
        "  // daemon's `/v1/mogoi/*` endpoints with the engineToken without unsafe casts.\n" +
        "  server: ServerContext;",
    },
  ],
  // Mogoi-only extension: optional `outputSample` declaration on actions.
  ...
  "packages/pieces/framework/src/lib/action/action.ts": [
    {
      ...
      insert:
        "  // === MOGOI PATCH: optional outputSample declaration ===\n" +
    ...
    },
    {
      ...
      insert:
        "    // === MOGOI PATCH: outputSample (see CreateActionParams) ===\n" +
    ...
    },
    {
      ...
      position: "before",
      insert:
        "    // === MOGOI PATCH: forward outputSample to the IAction instance ===\n" +
        "    params.outputSample,\n" +
        "    // === END MOGOI PATCH ===",
    },
  ],
  // Mogoi-only extension: outputSample on ActionBase metadata too, so the
  ...
  "packages/pieces/framework/src/lib/piece-metadata.ts": [
    {
      ...
      insert:
        "  // === MOGOI PATCH: outputSample (see action.ts) ===\n" +
  ...
  // Mogoi-only: TEXT_MATCHES_REGEX + TEXT_DOES_NOT_MATCH_REGEX added
  ...
  "packages/shared/src/lib/automation/flows/actions/action.ts": [
    {
      ...
      insert:
        "    // === MOGOI PATCH: regex condition operators ===\n" +
    ...
    },
    {
      ...
      insert:
        "    // === MOGOI PATCH: regex operators (see BranchOperator enum) ===\n" +
  ...
  // Mogoi-only: case branches for the two regex operators added above.
  ...
  "packages/server/engine/src/lib/handler/flow-executor.ts": [
    {
      ...
      insert:
        "            // === MOGOI PATCH: short-circuit when there is no piece trigger to start ===\n" +
        ...
        "            const mogoiTriggerName = (trigger as { settings?: { triggerName?: unknown } }).settings\n" +
        "                ?.triggerName\n" +
        "            if (isNil(mogoiTriggerName)) {\n" +
...
// 7. Apply Mogoi-specific stubs and dependency scrubs.

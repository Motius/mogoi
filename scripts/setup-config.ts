...
/**
 * Setup script for M.O.G.O.I. configuration
 *
 * This script helps you:
 * 1. Create the ~/.mogoi directory
...
const CONFIG_DIR = join(homedir(), '.mogoi');
...
async function testProviders(config: any): Promise<void> {
  ...
  try {
    const response = await manager.chat(
      [
        { role: 'system', content: 'You are M.O.G.O.I.' },
...
async function main() {
  console.log('M.O.G.O.I. Configuration Setup\n');
  console.log('================================\n');
  ...
  console.log('  1. Edit config if needed: nano ~/.mogoi/config.yaml');

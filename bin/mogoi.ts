#!/usr/bin/env bun
/**
 * M.O.G.O.I. CLI Entry Point
 *
 * Usage:
 *   mogoi start [--port N] [-d|--detach]   Start the daemon
 ...
 *
 * First-time setup happens in the dashboard at http://localhost:1846
 * after `mogoi start` — there is no longer a CLI wizard.
 */
...
function printHelp(): void {
  console.log(`
${c.cyan('M.O.G.O.I.')} ${c.dim(`v${getVersion()}`)}
...
${c.bold('Start options:')}
  --port <N>        Override daemon port (default: 1846)
  -d, --detach      Run as background daemon
  --no-open         Don't auto-open dashboard in browser
  --data-dir <path> Override data directory (default: ~/.mogoi)
...
${c.bold('Examples:')}
  ...
  mogoi start --port 8080      Start on custom port
  ...
  mogoi revoke <sid>           Revoke a device's access
  mogoi doctor                 Check if everything is working
`);
}
...
async function cmdStart(args: string[]): Promise<void> {
  ...
  if (!_exists(cfgPath)) {
    ...
    console.log(c.dim(`  in ~/.mogoi/config.yaml, open http://localhost:${port ?? 1846}, and`));
  ...
  if (!detach) {
    ...
    console.log(c.cyan('Starting M.O.G.O.I. daemon...'));

    const logPath = getLogPath();

    const daemonArgs = [join(PACKAGE_ROOT, 'bin/mogoi.ts'), 'start', '--no-open'];
    ...
    // `mogoi update`'s restart, and the launchd plist - and because comparing
...
// Callers decide whether that is fatal — `mogoi restart` must not exit here.
async function cmdStop(args: string[] = [], opts: { verb?: string } = {}): Promise<boolean> {
  ...
  try {
    ...
    // case would let the next `mogoi start` take a fresh inode and run a
    ...
    if (!releaseLockIfUnheld()) {
      ...
      console.error(c.dim(
        relaunched
          ? '  A service manager restarted it. Remove autostart first: mogoi uninstall (or disable the service).'
...
function cmdStatus(): void {
  const pid = isLocked();
  if (pid) {
    console.log(`${c.green('●')} MOGOI is ${c.green('running')} (PID ${pid})`);
    ...
    try {
      ...
      console.log(c.dim(`  Dashboard: http://localhost:1846`));
    }

    console.log(c.dim(`  Stop with: mogoi stop`));
  } else {
    console.log(`${c.red('●')} MOGOI is ${c.red('stopped')}`);
    console.log(c.dim(`  Start with: mogoi start`));
  }
}
...
function cmdLogs(args: string[]): void {
  ...
  if (!existsSync(logPath)) {
    console.log(c.yellow('No log file found. Start the daemon first: mogoi start'));
    return;
  }
...
switch (command) {
  ...
  case 'onboard':
    ...
    console.log(c.dim('    1. Run: mogoi start'));
    console.log(c.dim('    2. Open: http://localhost:1846'));
  ...
  default:
    ...
    console.log(c.dim('Run "mogoi help" for usage information.'));
    process.exit(1);
}

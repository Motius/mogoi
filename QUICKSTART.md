# M.O.G.O.I. LLM Providers - Quick Start Guide

Get up and running with M.O.G.O.I. LLM providers in 5 minutes.
...
**Solution**: Add at least one API key to `~/.mogoi/config.yaml`
...
**Solution**: Ensure config exists at `~/.mogoi/config.yaml`
```bash
cp config.example.yaml ~/.mogoi/config.yaml
```
...
1. **Read the Documentation**:
   - `~/mogoi/src/llm/README.md` - LLM providers
   - `~/mogoi/src/config/README.md` - Configuration

2. **Explore Examples**:
   - `~/mogoi/examples/llm-integration.ts`
   - `~/mogoi/src/llm/test.ts`
...
1. **Never commit API keys**: Keep them in `~/.mogoi/config.yaml` only
2. **Use environment variables**: For production deployments
3. **Restrict config permissions**: `chmod 600 ~/.mogoi/config.yaml`
4. **Monitor usage**: Track token usage to control costs
5. **Use fallbacks**: Always configure backup providers

## Getting Help

- Check `~/mogoi/src/llm/README.md` for detailed docs
- Review `~/mogoi/IMPLEMENTATION_SUMMARY.md` for architecture
- Run test file to verify setup: `bun run src/llm/test.ts`
- Check config: `cat ~/.mogoi/config.yaml`
...
You're all set! Start building with M.O.G.O.I. 🚀

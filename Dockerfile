# ─── M.O.G.O.I. Docker Image ──────────────────────────────────────
#
# Multi-stage build for the MOGOI daemon.
...
#
# Build:   docker build -t mogoi .
# Build with version: docker build --build-arg VERSION=0.3.1 -t mogoi .
# Run:     docker run -p 3142:3142 -v mogoi-data:/data -e MOGOI_API_KEY=sk-... mogoi
#
# ─────────────────────────────────────────────────────────────────────
...
RUN mkdir -p ui/public/openwakeword/models ui/public/ort && \
    cp node_modules/openwakeword-wasm-browser/models/melspectrogram.onnx \
       ...
       node_modules/openwakeword-wasm-browser/models/hey_mogoi_v0.1.onnx \
...
# Install mogoi as a global command
# Note: `bun link` can't be used here — it symlinks through /root/.bun/ which
# is inaccessible to the non-root mogoi user. Direct symlink works because
# Bun resolves import.meta.dir through symlinks to the real path (/app/bin).
RUN ln -s /app/bin/mogoi.ts /usr/local/bin/mogoi

# Create non-root user and data directory
RUN groupadd -r mogoi && useradd -r -g mogoi -d /data -s /bin/bash mogoi && \
    mkdir -p /data && chown mogoi:mogoi /data

ENV MOGOI_HOME=/data
ENV NODE_ENV=production
# Signal to `mogoi update` / `mogoi uninstall` that this is a container
# install. Both commands refuse to run here and point the user at the
# correct host-side docker commands.
ENV MOGOI_INSTALL_METHOD=docker
# Durable on-disk marker as a belt-and-suspenders fallback if the env var
# is ever unset (e.g. someone runs `docker exec -e MOGOI_INSTALL_METHOD= ...`).
